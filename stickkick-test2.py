...
class ParachutePowerup: # ... (no change) ...
    ...
    def check_collision(self, player):
        ...
        if player_rect.colliderect(powerup_box_rect):
            ...
            play_sound(loaded_sounds['combo']) # <-- Keep sound calls active
...
def create_explosion(x, y, radius, players, ball): # ... (no change) ...
    ...
    play_sound(loaded_sounds['wall_hit']) # <-- Keep sound calls active
    for p in players:
...
class StickMan: # ... (rest unchanged) ...
    ...
    def jump(self):
        ...
        if can_jump_now:
            ...
            play_sound(loaded_sounds['jump']); # <-- Keep sound calls active
            if was_on_head: play_sound(loaded_sounds['combo']) # <-- Keep sound calls active
    ...
    def fire_rocket(self):
        ...
        play_sound(loaded_sounds['kick']) # <-- Keep sound calls active
    ...
    def update(self, dt, other_player):
        ...
        if was_airborne and is_now_grounded and not self.is_tumbling: play_sound(loaded_sounds['land']) # <-- Keep sound calls active
        intended_vx = self.vx
        if not self.is_kicking:
...
class Ball:
    ...
    def update(self, dt):
        ...
        if hit_shield_this_frame: play_sound(loaded_sounds['wall_hit']) # <-- Keep sound calls active
        ...
        if hit_wall_this_frame and not hit_shield_this_frame: play_sound(loaded_sounds['wall_hit']) # <-- Keep sound calls active
        if self.y + self.radius >= GROUND_Y:
            if self.vy >= 0:
                impact_vy = abs(self.vy); self.y = GROUND_Y - self.radius; self.vy *= -BALL_BOUNCE; self.vx *= 0.9
                if abs(self.vy) < 1: self.vy = 0
                if impact_vy > 1.5: play_sound(loaded_sounds['ball_bounce']) # <-- Keep sound calls active
                hit_ground = True
...
# --- Game Setup ---
pygame.init();
# ** Do NOT initialize mixer here **
# pygame.mixer.pre_init(44100, -16, 2, 512)
# pygame.mixer.init()
# announcement_channel = pygame.mixer.Channel(0) # Initialize later
...
# --- Sound Playing Helpers --- (Keep definitions, they need loaded_sounds)
def play_sound(sound_list):
    if audio_initialized and sound_list: # <-- Check if initialized
    ...
    if audio_initialized and announcement_channel and announcement_queue and not announcement_channel.get_busy():
        ...
        if next_sound:
            ...
            announcement_channel.set_endevent(SOUND_FINISHED_EVENT)
             ...
             # If sound object is None for some reason, still process the event
             pygame.event.post(pygame.event.Event(SOUND_FINISHED_EVENT))

# --- State Variables for Audio ---
audio_initialized = False
loaded_sounds = {}  # Initialize empty, load later
announcement_queue = []
announcement_channel = None # Initialize later
...
# announcement_queue moved to audio state vars
...
# (Keep reset/start functions as they are - they reset game state, not audio state)
...
def start_new_match(): # Full reset for new match
    ...
    # Don't reset announcement_queue here if audio is initialized
    reset_positions()
...
def start_new_game(): # ... (no change) ...
    ...
    active_powerups = []
    if audio_initialized: # Clear queue if audio was already started
        announcement_queue = []
    start_new_match(); print("Starting new game.")

# --- Collision Handling Function (Player-Ball) ---
# (Definition remains the same, calls play_sound which now checks audio_initialized)
...
while running:
    ...
    for event in events:
        ...
        if not audio_initialized and (event.type == pygame.MOUSEBUTTONDOWN or event.type == pygame.KEYDOWN):
            try:
                ...
                # Game can continue without sound, audio_initialized remains False
            # Consume the event that triggered init if it was a gameplay key
            # (Optional, prevents jump/kick on the init click/press)
        ...
        if audio_initialized and event.type == SOUND_FINISHED_EVENT:
            play_next_announcement() # Process sound queue only if audio is ready
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif audio_initialized and match_active: # Only allow game input if audio is ready (or failed init)
        ...
        if event.type == pygame.KEYUP:
             if audio_initialized and match_active: # Only allow game input if audio is ready
                if not player1.is_tumbling:
    ...
    # --- Game Logic (conditional on audio init) ---
    ...
    # --- Normal Game Loop (Audio Initialized) ---
