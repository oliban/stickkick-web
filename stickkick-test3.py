...
TUMBLE_DURATION = 2.5 # Increased tumble duration
...
# --- Power-up Constants ---
POWERUP_TYPES = ["FLIGHT", "ROCKET_LAUNCHER", "BIG_PLAYER", "SUPER_JUMP", "BALL_FREEZE", "SPEED_BOOST", "GOAL_SHIELD", "SHRINK_OPPONENT"]
...
POWERUP_SHRINK_PLAYER_DURATION = 30.0 # Increased duration
POWERUP_SHRINK_PLAYER_SCALE = 0.6
POWERUP_SUPER_JUMP_DURATION = 15.0
POWERUP_SUPER_JUMP_MULTIPLIER = 1.8
POWERUP_BALL_FREEZE_DURATION = 25.0 # Increased duration
...
ROCKET_SPEED = 600 # Increased speed
ROCKET_SIZE = (18, 6)
ROCKET_COLOR = (100, 100, 110)
ROCKET_BLAST_RADIUS_FACTOR = 2.5
ROCKET_EXPLOSION_DURATION = 0.4
ROCKET_EXPLOSION_FORCE = 95.0 # Increased force
ROCKET_PLAYER_UPWARD_BOOST = 20.0 # Increased upward boost
...
# --- Custom Event Types ---
SOUND_FINISHED_EVENT = pygame.USEREVENT + 1 # Keep this

# --- Font Filename ---
# !! IMPORTANT: Replace "MyFont.ttf" with the actual filename of a .ttf font file !!
# !! you have placed in the same directory as this script. !!
FONT_FILENAME = "MyFont.ttf"
...
def draw_scoreboard(surface, p1_score, p2_score, p1_games, p2_games, score_font, name_font, game_score_font, is_goal_active):
    ...
    except Exception as e: # Catch potential errors if fonts aren't loaded
...
def draw_game_scores(surface, scores_list, font):
    ...
    try:
        ...
        print(f"Error drawing game scores: {e}")
def draw_trophy(surface, winner_name, title_font, name_font): # name_font is font_large
    ...
    try:
        ...
        # Rematch Text Font Loading:
        rematch_font_obj = pygame.font.Font(FONT_FILENAME, 36) # Load specifically for rematch text
        rematch_text = "Press R for Rematch"; rematch_surf = rematch_font_obj.render(rematch_text, True, WHITE)
        ...
        print(f"Error drawing trophy: {e}")
...
# --- Class Definitions ---
# ... (Particle, ParachutePowerup, Rocket, Explosion Classes unchanged) ...
class Particle:
...
class ParachutePowerup:
    ...
    def check_collision(self, player):
        ...
        if player_rect.colliderect(powerup_box_rect):
            ...
            play_sound(loaded_sounds.get('combo', [])) # Use .get for safety
...
class StickMan:
    def __init__(self, x, y, facing=1):
        ...
        self.is_flying = False; self.is_big = False; self.is_shrunk = False; self.jump_power = BASE_JUMP_POWER; self.player_speed = BASE_PLAYER_SPEED
    ...
    def apply_powerup(self, powerup_type, other_player=None):
        ...
        if powerup_type == "FLIGHT":
            new_timer = POWERUP_FLIGHT_DURATION
            self.active_powerups["FLIGHT"] = new_timer; self.is_flying = True; print(f"Flight activated/refreshed: {new_timer:.1f}s")
        elif powerup_type == "ROCKET_LAUNCHER":
            new_ammo = current_val + 1; self.active_powerups["ROCKET_LAUNCHER"] = new_ammo; print(f"Rocket Launcher ammo: {new_ammo}")
        elif powerup_type == "BIG_PLAYER":
            new_timer = POWERUP_BIG_PLAYER_DURATION
            self.active_powerups["BIG_PLAYER"] = new_timer; self.is_big = True
            if "SHRUNK" in self.active_powerups: del self.active_powerups["SHRUNK"]; self.is_shrunk = False
            self.calculate_current_sizes(); print(f"Big Player activated/refreshed: {new_timer:.1f}s")
        elif powerup_type == "SUPER_JUMP":
            new_timer = POWERUP_SUPER_JUMP_DURATION
            self.active_powerups["SUPER_JUMP"] = new_timer
            self.jump_power = BASE_JUMP_POWER * POWERUP_SUPER_JUMP_MULTIPLIER; print(f"Super Jump activated/refreshed: {new_timer:.1f}s")
        elif powerup_type == "SPEED_BOOST":
            new_timer = POWERUP_SPEED_BOOST_DURATION
            self.active_powerups["SPEED_BOOST"] = new_timer
            self.player_speed = BASE_PLAYER_SPEED * POWERUP_SPEED_BOOST_MULTIPLIER; print(f"Speed Boost activated/refreshed: {new_timer:.1f}s")
    ...
    def calculate_current_sizes(self):
        scale = 1.0
        if self.is_big: scale = POWERUP_BIG_PLAYER_SCALE
        elif self.is_shrunk: scale = POWERUP_SHRINK_PLAYER_SCALE
        self.head_radius = self.base_head_radius * scale; self.torso_length = self.base_torso_length * scale
        self.limb_width = self.base_limb_width * (1.0 + (scale - 1.0) * 0.5)
        self.upper_arm_length = self.base_upper_arm_length * scale; self.forearm_length = self.base_forearm_length * scale
        self.thigh_length = self.base_thigh_length * scale; self.shin_length = self.base_shin_length * scale
        self.current_nose_length = self.base_nose_length * scale; self.current_nose_width = self.base_nose_width * scale
    def move(self, direction):
        if self.is_tumbling: return
        if not self.is_kicking: self.vx = direction * self.player_speed
    ...
    def jump(self):
        ...
        if can_jump_now:
            ...
            play_sound(loaded_sounds.get('jump', []))
            if was_on_head: play_sound(loaded_sounds.get('combo', []))
    ...
    def fire_rocket(self):
        ...
        play_sound(loaded_sounds.get('kick', []))
    ...
    def update(self, dt, other_player):
        ...
        for p_type, value in list(self.active_powerups.items()):
            if p_type in ["FLIGHT", "BIG_PLAYER", "SHRUNK", "SUPER_JUMP", "SPEED_BOOST"]:
                new_timer = value - dt
                if new_timer <= 0:
        ...
        if was_airborne and is_now_grounded and not self.is_tumbling: play_sound(loaded_sounds.get('land', []))
        intended_vx = self.vx
        if not self.is_kicking:
...
class Ball:
    ...
    def update(self, dt):
        ...
        if p1_shield_active:
            shield_rect = pygame.Rect(0, GOAL_Y_POS, POWERUP_GOAL_SHIELD_WIDTH, GOAL_HEIGHT)
        ...
        if p2_shield_active:
            shield_rect = pygame.Rect(SCREEN_WIDTH - POWERUP_GOAL_SHIELD_WIDTH, GOAL_Y_POS, POWERUP_GOAL_SHIELD_WIDTH, GOAL_HEIGHT)
        ...
        if hit_shield_this_frame: play_sound(loaded_sounds.get('wall_hit', []))
        ...
        if hit_wall_this_frame and not hit_shield_this_frame: play_sound(loaded_sounds.get('wall_hit', []))
        if self.y + self.radius >= GROUND_Y:
            if self.vy >= 0:
                impact_vy = abs(self.vy); self.y = GROUND_Y - self.radius; self.vy *= -BALL_BOUNCE; self.vx *= 0.9
                if abs(self.vy) < 1: self.vy = 0
                if impact_vy > 1.5: play_sound(loaded_sounds.get('ball_bounce', []))
                hit_ground = True
...
# --- Game Setup ---
pygame.init()
# Mixer is NOT initialized here
screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
pygame.display.set_caption("Ciao Kick!")
clock = pygame.time.Clock()

# --- Font Loading (Using Specific File) ---
try:
    font_large = pygame.font.Font(FONT_FILENAME, 50)
    font_medium = pygame.font.Font(FONT_FILENAME, 36)
    font_small = pygame.font.Font(FONT_FILENAME, 28)
    font_timestamp = pygame.font.Font(FONT_FILENAME, 20)
    font_goal = pygame.font.Font(FONT_FILENAME, 80)
    font_start = pygame.font.Font(FONT_FILENAME, 60) # Font for start message
    print("Fonts loaded successfully using:", FONT_FILENAME)
except pygame.error as e:
    print(f"!!! FAILED TO LOAD FONT '{FONT_FILENAME}': {e}")
    print("Ensure the font file is in the same directory as the script.")
    # Attempt to fall back to default font if specific one fails
    try:
        print("Attempting to load default font...")
        font_large = pygame.font.Font(None, 50)
        font_medium = pygame.font.Font(None, 36)
        font_small = pygame.font.Font(None, 28)
        font_timestamp = pygame.font.Font(None, 20)
        font_goal = pygame.font.Font(None, 80)
        font_start = pygame.font.Font(None, 60)
        print("Default font loaded (may not work well in web).")
    except Exception as e_default:
        print(f"!!! FAILED TO LOAD DEFAULT FONT: {e_default}")
        pygame.quit()
        sys.exit("Font loading failed.")


# --- Sound Loading Function (Definition only) ---
...
def load_sounds(sound_dir="sounds"):
    ...
    for name, filenames in sound_files.items():
        ...
        for filename in filenames:
            ...
            try:
                sound = pygame.mixer.Sound(path); sounds[name].append(sound); print(f"Loaded sound: {path}")
    ...
    for i in range(0, 6):
        ...
        try:
            sound = pygame.mixer.Sound(path); sounds['numbers'][i] = sound; print(f"Loaded sound: {path} as number {i}")
    ...
    for player_num in [1, 2]:
        ...
        while True:
            ...
            if os.path.exists(path):
                try:
                    sound = pygame.mixer.Sound(path); sounds[goal_key].append(sound); print(f"Loaded sound: {path}"); i += 1
...
# (Definitions are the same, rely on audio_initialized check)
def play_sound(sound_list):
    if audio_initialized and sound_list:
        try:
...
def queue_sound(sound_list):
    ...
    if audio_initialized and sound_list:
        try:
            ...
            print(f"Error queuing sound: {e}")


def queue_specific_sound(sound_obj):
    global announcement_queue
    if audio_initialized and sound_obj:
        announcement_queue.append(sound_obj)

def play_next_announcement():
    global announcement_queue, announcement_channel
    if audio_initialized and announcement_channel and announcement_queue and not announcement_channel.get_busy():
        try:
            ...
            if next_sound:
                ...
                announcement_channel.set_endevent(SOUND_FINISHED_EVENT)
                 ...
                 pygame.event.post(pygame.event.Event(SOUND_FINISHED_EVENT))
        except Exception as e:
             print(f"Error playing next announcement: {e}")
             pygame.event.post(pygame.event.Event(SOUND_FINISHED_EVENT)) # Ensure queue progresses
...
winner_images = {}
try:
    img_path1 = os.path.join("nils_wins.png")
    img_path2 = os.path.join("harry_wins.png")
    winner_images[1] = pygame.image.load(img_path1).convert_alpha()
    winner_images[2] = pygame.image.load(img_path2).convert_alpha()
...
def reset_positions():
    ...
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0; player1.is_kicking = False; player1.on_other_player_head = False; player1.facing_direction = 1; player1.is_jumping = False; player1.is_tumbling = False; player1.rotation_angle = 0; player1.rotation_velocity = 0
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0; player2.is_kicking = False; player2.on_other_player_head = False; player2.facing_direction = -1; player2.is_jumping = False; player2.is_tumbling = False; player2.rotation_angle = 0; player2.rotation_velocity = 0
    ...
    active_rockets = []; active_explosions = []
def start_new_match():
    global player1_score, player2_score, match_active, match_winner, match_over_timer, match_end_sound_played, announcement_queue, powerup_spawn_timer, active_powerups, ball_freeze_timer, p1_shield_active, p1_shield_timer, p2_shield_active, p2_shield_timer, jackpot_triggered_this_match
    player1_score = 0; player2_score = 0; match_active = True; match_winner = None; match_over_timer = 0.0; match_end_sound_played = False
    if audio_initialized: announcement_queue = [] # Clear only if audio started
    reset_positions()
    player1.active_powerups = {}; player1.is_flying = False; player1.is_big = False; player1.is_shrunk = False; player1.jump_power = BASE_JUMP_POWER; player1.player_speed = BASE_PLAYER_SPEED; player1.calculate_current_sizes()
    player2.active_powerups = {}; player2.is_flying = False; player2.is_big = False; player2.is_shrunk = False; player2.jump_power = BASE_JUMP_POWER; player2.player_speed = BASE_PLAYER_SPEED; player2.calculate_current_sizes()
    active_powerups = []
...
def start_new_game():
    ...
    active_powerups = []
    if audio_initialized: announcement_queue = []
    start_new_match(); print("Starting new game.")
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne):
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad * (player.head_radius / player.base_head_radius))**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 ball.apply_force(kick_x, kick_y, hitter=player); kick_performed = True; kick_pt = local_kick_point; play_sound(loaded_sounds.get('kick', [])); # Use .get for safety before load
    ...
    if dist_head_sq < (ball.radius + head_radius)**2:
        if can_headbutt:
            ...
            ball.apply_force(force_x, force_y, hitter=player); headbutt_cooldown_just_applied = True; headbutt_performed = True; play_sound(loaded_sounds.get('headbutt', [])); # Use .get
            if is_ball_airborne: current_hit_count += 1; score_increase = True
    ...
    if not kick_performed and not headbutt_performed and body_collision_timer == 0:
        ...
        if dist_sq_body < ball.radius**2:
             ...
             if collision_occurred: play_sound(loaded_sounds.get('body_hit', [])) # Use .get
    return score_increase, new_can_headbutt, new_body_collision_timer, kick_pt

# --- Start First Game ---
# start_new_game() # Don't start game logic immediately
...
while running:
    ...
    for event in events:
        ...
        # --- Deferred Audio Initialization Trigger ---
        if not audio_initialized and (event.type == pygame.MOUSEBUTTONDOWN or event.type == pygame.KEYDOWN):
            try:
                print("Attempting to initialize audio...")
                pygame.mixer.pre_init(44100, -16, 2, 512)
                pygame.mixer.init()
                loaded_sounds = load_sounds() # Load sounds now
                announcement_channel = pygame.mixer.Channel(0) # Get channel now
                audio_initialized = True
                start_new_game() # Start the game state AFTER audio init
                print("Audio Initialized Successfully. Game Started.")
            except pygame.error as e:
                print(f"!!! FAILED TO INITIALIZE AUDIO: {e}")
                # Game will continue without sound if init fails, start game state anyway
                audio_initialized = False # Explicitly keep false
                start_new_game()
                print("Continuing without audio. Game Started.")
            # Optional: consume the event that triggered init
            # continue # Uncomment this line if you want the first click/press *only* to init audio

        if audio_initialized and event.type == SOUND_FINISHED_EVENT:
            play_next_announcement()

        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE: running = False
            # Allow debug/rematch keys even before audio init
            if event.key == pygame.K_7: debug_mode = not debug_mode; print(f"Debug Mode {'ACT' if debug_mode else 'DEACT'}IVATED")
            elif game_over and event.key == pygame.K_r: start_new_game() # Rematch needs to work
            # Only process game keys if audio is initialized OR if init failed (game should still be playable silently)
            elif audio_initialized or (not audio_initialized and 'font_start' in locals()): # Check if font loaded as proxy for pre-audio state
                if match_active:
                    if event.key == pygame.K_6: # Allow debug spawn even if audio failed
                        if match_active:
        ...
        if event.type == pygame.KEYUP:
             if audio_initialized or (not audio_initialized and 'font_start' in locals()): # Check if font loaded
                 if match_active:
                    if not player1.is_tumbling:
    ...
    # --- Game Logic (conditional on audio init attempt) ---
    # Use 'font_start' existing as a check that we passed the initial setup phase
    if not audio_initialized and 'font_start' not in locals():
         # This should ideally not happen if font loading is robust
         screen.fill(BLACK)
         # Potentially draw an error message here if possible
         pygame.display.flip()
         continue

    if not audio_initialized and 'start_surf' not in globals(): # Draw "Click to Start" only once
        # --- Draw "Click to Start" ---
        screen.fill(SKY_BLUE)
        start_text = "Click or Press Key to Start"
        try:
            start_surf = font_start.render(start_text, True, TEXT_COLOR)
            start_rect = start_surf.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
            screen.blit(start_surf, start_rect)
        except Exception as e:
            print(f"Error rendering start text: {e}") # Fallback if font failed badly
            pygame.draw.rect(screen, BLACK, (100, 250, 600, 100)) # Simple box
        pygame.display.flip()
        continue # Skip the rest of the game loop until initialized

    # --- Normal Game Loop (Audio Initialized or Init Failed but font loaded) ---
    # (The rest of the game loop logic remains the same as the previous version)
    # ... [ Gamelogic from Handle Game Over State down to pygame.display.flip() ] ...

    # --- Handle Game Over State ---
    if game_over:
        if not game_over_sound_played:
            announcement_queue = []; # Clear queue for game over sounds
            if overall_winner == 1: queue_sound(loaded_sounds.get('nils_wins',[]))
            elif overall_winner == 2: queue_sound(loaded_sounds.get('harry_wins',[]))
            play_next_announcement();
        ...
        draw_trophy(screen, winner_name, font_goal, font_large) # font_large is passed as name_font
        ...
        if winner_image:
            ...
            image_center_y = SCREEN_HEIGHT // 2 + 180 - 40 - 100 // 2 # Align center with trophy cup
    ...
    if match_active:
        ...
        if powerup_spawn_timer <= 0:
            # Check for Super Jackpot (only once per match)
            if not jackpot_triggered_this_match and random.randint(1, 6) == 1:
                print("!!! SUPER JACKPOT !!!")
                play_sound(loaded_sounds.get('super_jackpot', [])) # Use .get
                jackpot_triggered_this_match = True
                for _ in range(8): # Spawn 8 powerups
    ...
    if match_active:
        ...
        if p1_rect.colliderect(p2_rect):
            ...
            if not p1_is_on_p2 and not p2_is_on_p1:
                ...
                if overlap_x > 0:
                    play_sound(loaded_sounds.get('player_bump', [])) # Use .get
        ...
        # Kick Collision with opponent - Apply Big Player Bonus
        ...
        if p1_kick_point and p2_rect.collidepoint(p1_kick_point) and not player2.is_tumbling:
            ...
            play_sound(loaded_sounds.get('body_hit', [])) # Use .get
        ...
        if score_increased_this_frame and last_kick_point and current_hit_count > 0 and current_hit_count % 5 == 0:
            play_sound(loaded_sounds.get('combo', [])) # Use .get
    ...
    if match_active and not ball.is_frozen:
        if ball.x + ball.radius >= GOAL_LINE_X_RIGHT and ball.y > GOAL_Y_POS and not p2_shield_active:
        ...
        elif ball.x - ball.radius <= GOAL_LINE_X_LEFT and ball.y > GOAL_Y_POS and not p1_shield_active:
    ...
    if goal_scored_this_frame: # --- Goal Effects ---
        if scorer == 1: play_sound(loaded_sounds.get('goal_p1', [])); # Use .get
        if player1_score > 0 and player1_score % 5 == 0: play_sound(loaded_sounds.get('combo', [])) # Use .get
        elif scorer == 2: play_sound(loaded_sounds.get('goal_p2', [])); # Use .get
        if player2_score > 0 and player2_score % 5 == 0: play_sound(loaded_sounds.get('combo', [])) # Use .get
        ...
        if winner_found_this_match:
            ...
            if not (p1_games_won >= GAME_WIN_LIMIT or p2_games_won >= GAME_WIN_LIMIT):
                announcement_queue = []; # Clear queue for next match sounds
                p1_games = p1_games_won; p2_games = p2_games_won
                if p1_games > p2_games: queue_sound(loaded_sounds.get('nils_ahead',[])); queue_specific_sound(loaded_sounds.get('numbers',{}).get(p1_games)); queue_specific_sound(loaded_sounds.get('numbers',{}).get(p2_games))
                elif p2_games > p1_games: queue_sound(loaded_sounds.get('harry_ahead',[])); queue_specific_sound(loaded_sounds.get('numbers',{}).get(p2_games)); queue_specific_sound(loaded_sounds.get('numbers',{}).get(p1_games))
                else: queue_specific_sound(loaded_sounds.get('numbers',{}).get(p1_games)); queue_specific_sound(loaded_sounds.get('numbers',{}).get(p2_games))
                play_next_announcement()
    ...
    if p1_shield_active: # P1 Shield on Left Goal
        shield_rect = pygame.Rect(GOAL_LINE_X_LEFT - POWERUP_GOAL_SHIELD_WIDTH // 2, GOAL_Y_POS, POWERUP_GOAL_SHIELD_WIDTH, GOAL_HEIGHT)
    ...
    if p2_shield_active: # P2 Shield on Right Goal
        shield_rect = pygame.Rect(GOAL_LINE_X_RIGHT - POWERUP_GOAL_SHIELD_WIDTH // 2, GOAL_Y_POS, POWERUP_GOAL_SHIELD_WIDTH, GOAL_HEIGHT)
        shield_surf = pygame.Surface(shield_rect.size, pygame.SRCALPHA); shield_surf.fill(shield_color_with_alpha)
        screen.blit(shield_surf, shield_rect.topleft); pygame.draw.rect(screen, WHITE, shield_rect, 1)
    ...
    try:
        ...
        if match_active:
            ...
            powerup_colors = { "FLIGHT": (0,100,200), "ROCKET_LAUNCHER": (200, 50, 50), "BIG_PLAYER": (50, 180, 50), "SUPER_JUMP": (200, 200, 50), "SPEED_BOOST": (255, 150, 0), "GOAL_SHIELD": (100, 180, 255), "SHRUNK": (150, 100, 200)}
            powerup_texts = { "FLIGHT": "Flight: {:.1f}", "ROCKET_LAUNCHER": "ROCKET(x{})", "BIG_PLAYER": "BIG: {:.1f}", "SUPER_JUMP": "JUMP: {:.1f}", "SPEED_BOOST": "SPEED: {:.1f}", "GOAL_SHIELD": "SHIELD: {:.1f}", "SHRUNK": "Shrunk: {:.1f}"}
            ...
            for p_type in sorted(player1.active_powerups.keys()):
                ...
                elif p_type in ["FLIGHT", "BIG_PLAYER", "SHRUNK", "SUPER_JUMP", "SPEED_BOOST"]: text = text_template.format(val)
        ...
        print(f"Error during UI drawing: {e}") # Catch errors if fonts failed
    ...
    if debug_mode:
        try:
            ...
            print(f"Error drawing debug timestamp: {e}")
