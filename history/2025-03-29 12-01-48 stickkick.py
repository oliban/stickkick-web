...
class StickMan: # Added wing attributes and logic
    ...
    def draw(self, screen):
        ...
        # --- Draw Triangle Nose (BEFORE head outline) ---
        ...
        base_x = head_center_int[0] + 2 * self.facing_direction
        nose_points = [(base_x, base_y1), (tip_x, tip_y), (base_x, base_y2)]
        pygame.draw.polygon(screen, NOSE_COLOR, nose_points)
        # --- End Nose ---
        draw_pentagon(screen, BLACK, head_center_int, self.head_radius * 0.6, angle=0.1); draw_pentagon(screen, BLACK, head_center_int, self.head_radius * 0.3, angle=math.pi/5 + 0.1); # Pattern
        ...
        if flight_mode_active:
            ...
            upper_length = 25; upper_width = 15; lower_length = 20; lower_width = 18
            angle_offset_upper = -0.4 ; angle_offset_lower = 0.5
...
while running:
    ...
    # --- Process Announcement Sound Queue (Relies on Event + Initial Trigger) ---
    # Removed the check from the main loop itself.
    # play_next_announcement() # <<< REMOVED FROM HERE
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            announcement_queue = [] # Clear previous
            p1_games = p1_games_won; p2_games = p2_games_won # Use updated game scores
