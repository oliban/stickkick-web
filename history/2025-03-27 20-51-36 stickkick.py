...
GAME_WIN_LIMIT = 5
MATCH_OVER_DURATION = 3.0 # Base duration for match over screen
GOAL_MESSAGE_DURATION = 1.5

# --- Helper Functions ---
# ... (draw_polygon_shape, draw_pentagon, draw_hexagon, normalize, draw_rotated_rectangle, draw_goal_isometric, draw_scoreboard, draw_game_scores, draw_trophy remain the same) ...
...
def draw_trophy(surface, winner_name, title_font, name_font): # Added Rematch Text
    ...
    name_surf = name_font.render(winner_name, True, engraved_color)
    name_rect = name_surf.get_rect(center=base_rect.center)
...
# --- Sound Loading ---
def load_sounds(sound_dir="sounds"): #... (Added numbers dict) ...
    sounds = {}
    ...
    sounds['numbers'] = {} # Load numbers separately
    for i in range(1, 6): # Numbers 1 through 5 (adjust range if needed)
...
match_winner = None; overall_winner = None; match_end_sound_played = False # Flag for match end sounds
...
while running:
    ...
    if goal_scored_this_frame:
        ...
        if player1_score > 0 and player1_score % 5 == 0: play_sound(loaded_sounds['combo']) # Individual 5th goal combo sound
        elif scorer == 2: play_sound(loaded_sounds['goal_p2']);
        if player2_score > 0 and player2_score % 5 == 0: play_sound(loaded_sounds['combo']) # Individual 5th goal combo sound
        ...
        if winner_found_this_match:
            ...
            match_end_sound_played = False # Allow sounds to play for this match end
        ...
        # Don't 'continue' here after goal if match ENDED, allow one frame draw
