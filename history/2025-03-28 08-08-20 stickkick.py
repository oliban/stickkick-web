...
# --- Scoreboard Function (Vertical Layout) ---
...
def draw_game_scores(surface, scores_list, font):
    ...
    for score_tuple in reversed(scores_list):
        ...
        score_surf = font.render(score_str, True, TEXT_COLOR); score_rect = score_surf.get_rect(topleft=(start_x, current_y))
        surface.blit(score_surf, score_rect); current_y += score_rect.height + 1
...
class StickMan: #... (as before) ...
    def __init__(self, x, y, facing=1): # ... (rest of init as before) ...
        ...
        self.kick_timer = 0; self.kick_duration = 18; self.walk_cycle_timer = 0.0; self.head_radius = 12; self.torso_length = 36; self.limb_width = 10; self.upper_arm_length = 12; self.forearm_length = 12; self.thigh_length = 14; self.shin_length = 14;
        self.torso_colors = [P2_COLOR_MAIN, P2_COLOR_WHITE, P2_COLOR_ACCENT]; self.arm_colors = [P2_COLOR_ACCENT, P2_COLOR_MAIN]; self.leg_colors = [P2_COLOR_WHITE, P2_COLOR_ACCENT]
        self.l_upper_arm_angle = 0; self.r_upper_arm_angle = 0; self.l_forearm_angle = 0; self.r_forearm_angle = 0; self.l_thigh_angle = 0; self.r_thigh_angle = 0; self.l_shin_angle = 0; self.r_shin_angle = 0; self.head_pos = (0, 0); self.neck_pos = (0, 0); self.hip_pos = (0, 0); self.shoulder_pos = (0, 0); self.l_elbow_pos = (0, 0); self.r_elbow_pos = (0, 0); self.l_hand_pos = (0, 0); self.r_hand_pos = (0, 0); self.l_knee_pos = (0, 0); self.r_knee_pos = (0, 0); self.l_foot_pos = (0, 0); self.r_foot_pos = (0, 0); self.body_rect = pygame.Rect(0,0,0,0)
...
while running:
    ...
    if match_over_timer > 0:
        ...
        # --- Transition (Wait for timer AND sound queue/channel) ---
    ...
    if match_active:
        # ... (goal detection logic as before) ...
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            if p1_games > p2_games:
                ...
                queue_specific_sound(loaded_sounds['numbers'].get(p1_games))
                 ...
                 queue_specific_sound(loaded_sounds['numbers'].get(p1_games))
                 queue_specific_sound(loaded_sounds['numbers'].get(p2_games))
            play_next_announcement() # Start the sequence
            # --- End Sound Queuing ---
        ...
        else: # Match not won, just reset positions
            reset_positions()
            continue # Skip drawing etc. if match continues
    ...
    # --- Draw UI ---
    draw_scoreboard(screen, player1_score, player2_score, p1_games_won, p2_games_won, font_large, font_medium, font_small, goal_message_timer > 0 or match_over_timer > 0) # Use font_small for game score
    draw_game_scores(screen, game_scores, font_small)
