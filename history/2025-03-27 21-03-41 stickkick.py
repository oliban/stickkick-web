...
GAME_WIN_LIMIT = 5
MATCH_OVER_DURATION = 3.0 # Base visual duration for match over screen
...
def draw_game_scores(surface, scores_list, font): # Corrected Order, Added Winner
    ...
    for score_tuple in reversed(scores_list):
        ...
        # Determine winner based on score (assuming match ended when limit was hit)
        # Note: This might need adjustment if rules change to allow exceeding the limit
...
# --- Sound Loading ---
def load_sounds(sound_dir="sounds"): #... (Handles missing number files) ...
    sounds = {}
...
def reset_positions(): #... (as before) ...
    ...
    # print("Positions reset.") # Less verbose
...
while running:
    ...
    if match_over_timer > 0:
        ...
        if not match_end_sound_played and not game_over:
            # 1. Announce Who's Ahead in Games & Scores (Queue only)
        ...
        # --- Transition (Check timer AND sound queue/channel state) ---
        if match_over_timer <= 0 and not game_over and not announcement_queue and not announcement_channel.get_busy():
             start_new_match()
    ...
    if goal_scored_this_frame:
        ...
        if not winner_found_this_match: continue # Skip drawing etc. only if match continues
