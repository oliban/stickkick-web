...
# --- Power-up Constants ---
POWERUP_SPAWN_INTERVAL_MIN = 15.0 # Min seconds between spawn attempts
POWERUP_SPAWN_INTERVAL_MAX = 30.0 # Max seconds between spawn attempts
POWERUP_SPAWN_CHANCE = 0.5     # Chance (0-1) of spawning when timer is ready
POWERUP_DESCEND_SPEED = 100    # Pixels per second
POWERUP_DRIFT_SPEED = 25       # Sideways drift speed
POWERUP_BOX_SIZE = (30, 25)
POWERUP_CHUTE_COLOR = (255, 165, 0) # Orange
POWERUP_CHUTE_ALT_COLOR = (240, 240, 240) # White/Light Grey panels
POWERUP_BOX_COLOR = (210, 180, 140) # Tan cardboard
POWERUP_FLIGHT_DURATION = 30.0 # Seconds of flight granted

# Star/Goal Explosion Constants <<< MOVED THESE UP
...
# --- Class Definitions ---
class Particle: # Now PARTICLE_SPEED, PARTICLE_LIFESPAN, PARTICLE_SIZE are defined before this class
...
class StickMan: # Added flight attributes, nose randomization, updated jump/draw
    def __init__(self, x, y, facing=1):
        ...
        self.wing_rest_angle_offset = math.pi * 0.1 + (math.pi / 6) # Downward offset
    ...
    def jump(self): # Conditional mid-air jump based on flight status
        can_jump_now = False
        if self.is_flying: # Use self.is_flying now
    ...
    def update(self, dt, other_player): # Added flight timer and wing update
        # --- Flight Timer Countdown ---
        if self.is_flying:
            self.flight_timer -= dt
            if self.flight_timer <= 0: self.is_flying = False; self.flight_timer = 0; print("Flight ended")
        # --- Physics ---
        ...
        if self.is_kicking: # ... (kick animation) ...
    ...
    # Getters
    def get_kick_impact_point(self): #... (as before) ...
    ...
    def draw(self, screen): # Updated head and wings
        ...
        if self.is_flying: # <<< Check player's flight status
            ...
            angle_offset_upper = -0.8; angle_offset_lower = 0.9
...
class Ball:
    ...
    def update(self, dt): # Updated bounce sound timing
...
def play_sound(sound_list): # Plays non-announcement sounds immediately
    if sound_list:
...
powerup = ParachutePowerup() # Create powerup instance
...
def reset_positions(): #... (as before) ...
    ...
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0; player1.is_kicking = False; player1.on_other_player_head = False; player1.facing_direction = 1; player1.is_jumping = False; player1.is_flying = False; player1.flight_timer = 0.0 # Reset flight
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0; player2.is_kicking = False; player2.on_other_player_head = False; player2.facing_direction = -1; player2.is_jumping = False; player2.is_flying = False; player2.flight_timer = 0.0 # Reset flight
...
def start_new_match(): # Reset sound flag and queue, randomize nose
    ...
    player1.randomize_nose(); player2.randomize_nose() # Randomize noses
...
while running:
    ...
    play_next_announcement() # Check if channel is free and play next sound
    ...
    if match_active:
        ...
        ball_hit_ground_this_frame = ball.update(dt); particles = [p for p in particles if p.update(dt)]
        powerup.update(dt) # Update powerup

        # --- Power-up Collection ---
        if powerup.active:
            if powerup.check_collision(player1):
                player1.is_flying = True; player1.flight_timer = POWERUP_FLIGHT_DURATION
                play_sound(loaded_sounds['combo']) # Play some sound for collection
            elif powerup.check_collision(player2):
                player2.is_flying = True; player2.flight_timer = POWERUP_FLIGHT_DURATION
                play_sound(loaded_sounds['combo']) # Play some sound for collection
        ...
        if p1_rect.colliderect(p2_rect): # Body Collision
            ...
            # Crude check to prevent sticking after head jump
            ...
            if not p1_is_on_p2 and not p2_is_on_p1: # Only push if not standing on head
                dx = player2.x - player1.x
                overlap_x = (p1_rect.width / 2 + p2_rect.width / 2) - abs(dx)

                if overlap_x > 0:
                    play_sound(loaded_sounds['player_bump'])
                    push = overlap_x / 2 + 0.1
                    if dx >= 0: # P1 left of P2
                        player1.x -= push; player2.x += push
                        if player1.vx > 0: player1.vx = 0 # Stop P1 moving right
                        if player2.vx < 0: player2.vx = 0 # Stop P2 moving left
                    else: # P1 right of P2
                        player1.x += push; player2.x -= push
                        if player1.vx < 0: player1.vx = 0 # Stop P1 moving left
                        if player2.vx > 0: player2.vx = 0 # Stop P2 moving right

        # Kick Collision with other player
        ...
        if p1_kick_point and p2_rect.collidepoint(p1_kick_point):
            print("P1 kicked P2")
            player2.x += kick_push_amount * player1.facing_direction
        ...
        if p2_kick_point and p1_rect.collidepoint(p2_kick_point):
            print("P2 kicked P1")
            player1.x += kick_push_amount * player2.facing_direction
    ...
    if match_active:
        if ball.x + ball.radius >= GOAL_LINE_X_RIGHT and ball.y > GOAL_Y_POS:
            player1_score += 1; scorer = 1; goal_message_timer = GOAL_MESSAGE_DURATION
            goal_scored_this_frame = True; goal_pos_x = SCREEN_WIDTH
            print(f"GOAL! Player 1 Score: {player1_score}"); screen_flash_timer = SCREEN_FLASH_DURATION
        elif ball.x - ball.radius <= GOAL_LINE_X_LEFT and ball.y > GOAL_Y_POS:
            player2_score += 1; scorer = 2; goal_message_timer = GOAL_MESSAGE_DURATION
            goal_scored_this_frame = True; goal_pos_x = 0
            print(f"GOAL! Player 2 Score: {player2_score}"); screen_flash_timer = SCREEN_FLASH_DURATION

    if goal_scored_this_frame:
        # Play goal sound & combo sound if score is multiple of 5
        if scorer == 1:
            play_sound(loaded_sounds['goal_p1'])
            if player1_score > 0 and player1_score % 5 == 0: play_sound(loaded_sounds['combo'])
        elif scorer == 2:
            play_sound(loaded_sounds['goal_p2'])
            if player2_score > 0 and player2_score % 5 == 0: play_sound(loaded_sounds['combo'])

        # Goal particle effect
        goal_center_y = GOAL_Y_POS + GOAL_HEIGHT / 2
        for _ in range(GOAL_PARTICLE_COUNT):
            particles.append(Particle(goal_pos_x, goal_center_y, colors=GOAL_EXPLOSION_COLORS,
                                       speed_min=GOAL_PARTICLE_SPEED_MIN, speed_max=GOAL_PARTICLE_SPEED_MAX,
                                       lifespan=GOAL_PARTICLE_LIFESPAN))

        # Check for match win
        current_match_limit = DEBUG_MATCH_POINT_LIMIT if debug_mode else MATCH_POINT_LIMIT
        winner_found_this_match = False
        if player1_score >= current_match_limit:
             match_winner = 1; winner_found_this_match = True
             print(f"Player 1 wins the match! (Limit: {current_match_limit})")
        elif player2_score >= current_match_limit:
             match_winner = 2; winner_found_this_match = True
             print(f"Player 2 wins the match! (Limit: {current_match_limit})")
        ...
        if winner_found_this_match:
            ...
            game_scores.insert(0, (player1_score, player2_score)); # Add score to history
            if len(game_scores) > 9: game_scores.pop() # Keep history short
            ...
            if not (p1_games_won >= GAME_WIN_LIMIT or p2_games_won >= GAME_WIN_LIMIT):
                announcement_queue = [] # Clear previous goal announcements
                ...
                if p1_games > p2_games:
                    ...
                    queue_specific_sound(loaded_sounds['numbers'].get(p2_games))
                    queue_specific_sound(loaded_sounds['numbers'].get(p1_games))
                else: # Tied game score
                    queue_specific_sound(loaded_sounds['numbers'].get(p1_games))
                    # queue_sound(some 'tied' sound?) # Optional
                    queue_specific_sound(loaded_sounds['numbers'].get(p2_games))
                play_next_announcement() # Trigger first sound now

            # Check overall game win
            if p1_games_won >= GAME_WIN_LIMIT:
                overall_winner = 1; game_over = True; print("PLAYER 1 WINS THE GAME!")
            elif p2_games_won >= GAME_WIN_LIMIT:
                overall_winner = 2; game_over = True; print("PLAYER 2 WINS THE GAME!")
            # Reset positions immediately after goal if winner found (or continue if reset fails)
            reset_positions()

        else: # No winner yet, just reset positions for next point
            reset_positions()
            # If reset_positions() didn't have 'continue', it would fall through here
            # Adding 'continue' prevents drawing one frame before reset if goal scored
            # This 'continue' might be redundant if reset_positions() is reliable,
            # but safe to keep.
            continue

    # --- Drawing ---
    bg_color = SKY_BLUE
    # Check individual player flight status for background color
    if player1.is_flying or player2.is_flying: bg_color = FLIGHT_MODE_BG_COLOR
    ...
    draw_goal_isometric(screen, GOAL_LINE_X_LEFT, GOAL_Y_POS, GOAL_HEIGHT, -GOAL_DEPTH_X, GOAL_DEPTH_Y, GOAL_POST_THICKNESS, GOAL_COLOR, GOAL_NET_COLOR) # Left Goal (Negative depth)
    draw_goal_isometric(screen, GOAL_LINE_X_RIGHT, GOAL_Y_POS, GOAL_HEIGHT, GOAL_DEPTH_X, GOAL_DEPTH_Y, GOAL_POST_THICKNESS, GOAL_COLOR, GOAL_NET_COLOR) # Right Goal (Positive depth)
    ...
    draw_game_scores(screen, game_scores, font_small) # Draw game history
    ...
    if match_over_timer > 0 and not game_over: # Display "Match Over" message
    ...
    if match_active: # Draw cooldowns only during active match
        ...
        # Draw flight timers if active
        flight_font = font_small
        if player1.is_flying:
            timer_text = f"P1 Flight: {player1.flight_timer:.1f}"; timer_surf = flight_font.render(timer_text, True, (0,100,200)); timer_rect = timer_surf.get_rect(topleft=(10, SCREEN_HEIGHT - 30)); screen.blit(timer_surf, timer_rect)
        if player2.is_flying:
            timer_text = f"P2 Flight: {player2.flight_timer:.1f}"; timer_surf = flight_font.render(timer_text, True, (0,100,200)); timer_rect = timer_surf.get_rect(topright=(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 30)); screen.blit(timer_surf, timer_rect)
