...
FLIGHT_MODE_BG_COLOR = (255, 255, 200) # <<< NEW Light Yellow
...
class StickMan: # Added wings, cap, eyes, nose logic
    def __init__(self, x, y, facing=1):
        ...
        self.nose_randomize_timer = 0.0; self.nose_randomize_interval = 0.5
    ...
    def update(self, dt, other_player): # Added wing animation and nose randomization
        ...
        self.r_wing_upper_angle = self.r_wing_base_angle + 0.4; self.r_wing_lower_angle = self.r_wing_base_angle - 0.6
        # --- Randomize Nose Size ---
        self.nose_randomize_timer -= dt
        if self.nose_randomize_timer <= 0:
            random_factor = random.uniform(1.0, 8.0); self.current_nose_length = self.base_nose_length * random_factor
            self.current_nose_width = self.base_nose_width * random_factor; self.current_nose_width = min(self.current_nose_width, self.current_nose_length * 0.8)
            self.nose_randomize_timer = self.nose_randomize_interval * random.uniform(0.8, 1.2)
        # --- Animation Logic ---
    ...
    def draw(self, screen): # Updated head and wings
        ...
        nose_base_y1 = nose_tip_y - self.current_nose_width / 2
        nose_base_y2 = nose_tip_y + self.current_nose_width / 2
        ...
        # --- Draw Torso/Limbs ---
        # ... (torso/limb drawing code remains the same) ...
...
while running:
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            # --- Queue Match End Sounds (only if game not over) ---
