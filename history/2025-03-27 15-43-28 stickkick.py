...
# Use player1 and player2 variable names
...
p1_can_headbutt = True; p2_can_headbutt = True # Separate headbutt cooldowns
p1_body_collision_timer = 0; p2_body_collision_timer = 0 # Separate body cooldowns

# --- Off-screen Arrow Function ---
def draw_offscreen_arrow(s, ball, p_pos): # p_pos is unused
...
def reset_after_goal():
    global ball_was_on_ground, current_max_height, current_hit_count # Need to reset combo too
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne):
    global current_hit_count # Modifies global hit count
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_LSHIFT: player2.start_charge()
    ...
    if p1_rect.colliderect(p2_rect):
        ...
        if overlap > 0: push = overlap / 2 + 0.1;
    ...
    # Score / Height Tracking Logic (Now maybe less relevant?)
    ...
    display_height_score = int(current_max_height * current_hit_count) # Keep if using combo score display

    # --- Player-Ball Collisions using Helper ---
    p1_score_increase, p1_can_headbutt, p1_body_collision_timer, p1_kick_pt = handle_player_ball_collisions(player1, ball, p1_can_headbutt, p1_body_collision_timer, is_ball_airborne)
    p2_score_increase, p2_can_headbutt, p2_body_collision_timer, p2_kick_pt = handle_player_ball_collisions(player2, ball, p2_can_headbutt, p2_body_collision_timer, is_ball_airborne)
    score_increased_this_frame = p1_score_increase or p2_score_increase
    ...
    player1.draw(screen); player2.draw(screen); ball.draw(screen); draw_offscreen_arrow(screen, ball, (player1.x, player1.y))
    # Scores
