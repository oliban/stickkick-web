...
def draw_scoreboard(surface, p1_score, p2_score, font, is_goal_active):
    ...
    pygame.draw.rect(panel_surf, SCOREBOARD_BORDER_COLOR, panel_surf.get_rect(), 2)
...
class StickMan:
    ...
    def jump(self):
        # Allow jumping off ground or off another player's head
        if not self.is_jumping or self.on_other_player_head:
            self.is_jumping = True
            self.on_other_player_head = False # No longer on head if jumping
            self.vy = JUMP_POWER
            self.walk_cycle_timer = 0
    def start_kick(self):
        if not self.is_kicking:
            self.is_kicking = True; self.kick_timer = 0; self.vx = 0 # Stop horizontal movement

    # --- (Inside StickMan Class) ---
    def update(self, dt, other_player):
        time_ms = pygame.time.get_ticks() # For potential timing use

        # --- Revised Vertical Movement & Platform Checks ---
        was_on_head = self.on_other_player_head # Store state from previous frame
        ...
        dist_x_head = self.x - other_head_pos[0] # Horizontal distance check is done later

        # Check horizontal alignment for potential head landing/standing
        is_aligned_for_head = abs(dist_x_head) < (other_head_radius + HEAD_PLATFORM_RADIUS_BUFFER)

        # --- Apply Gravity OR Handle Standing on Head ---
        if not was_on_head:
            # Apply gravity if not starting on a head
            self.vy += GRAVITY
        elif was_on_head and not is_aligned_for_head:
            # Was on head, but moved off horizontally -> Start falling
            self.on_other_player_head = False
            self.is_jumping = True # Now considered falling
            self.vy += GRAVITY # Apply gravity now that we're falling
        elif was_on_head and is_aligned_for_head:
             # Still on head, maintain vertical position and zero velocity
             self.y = head_top_y
             self.vy = 0
             # Don't apply gravity while stable on head

        # Calculate potential next Y based on current velocity
        # Important: If still on head, vy is 0, so next_y will be head_top_y
        next_y = self.y + self.vy

        # --- Landing Logic ---
        # Check if moving downwards/still and potentially landing
        if self.vy >= 0:
            # Check landing on head
            # Condition: Aligned horizontally, will pass head_top_y, and feet aren't already way below head
            can_land_on_head_now = (
                is_aligned_for_head and
                next_y >= head_top_y and
                self.y < head_top_y + 5 # Feet were slightly above or near head top last frame/step
            )
            if can_land_on_head_now:
                self.y = head_top_y # Snap to head top
                self.vy = 0
                self.is_jumping = False
                self.on_other_player_head = True # Set state
                landed_on_head_this_frame = True
            # Check landing on ground (only if not landing on head this frame)
            elif not landed_on_head_this_frame and next_y >= self.base_y:
                self.y = self.base_y # Snap to ground
                self.vy = 0
                self.is_jumping = False
                self.on_other_player_head = False # Cannot be on head if on ground
                landed_on_ground_this_frame = True
            # Else, still falling (apply calculated position)
            else:
                self.y = next_y
                # Ensure state is correct if falling past everything
                if self.y < self.base_y and not landed_on_head_this_frame:
                     self.on_other_player_head = False # Not on head if just falling
        else:
            # Moving upwards (apply calculated position)
            self.y = next_y
            self.on_other_player_head = False # Cannot be on head if moving upwards

        # Final ground clamp (safety check after all vertical logic)
        ...
        # --- Update horizontal position AFTER vertical state is fully set for the frame ---
        if not self.is_kicking:
            # If standing on head, inherit horizontal velocity (simplified approach)
            if self.on_other_player_head:
                 self.vx = other_player.vx # Match velocity of player below
            # Apply own velocity
            self.x += self.vx
            # Clamp position to screen bounds
            self.x = max(self.limb_width / 2, min(self.x, SCREEN_WIDTH - self.limb_width / 2))
        # If kicking, self.vx is already 0 from start_kick()


        # --- Animation Logic (Calculate angles based on final state) ---
        # Walk Cycle
        ...
        # --- Calculate Joint Positions (based on final X, Y and angles) ---
        ...
        # --- Update Body Rect (based on final joint positions) ---
    ...
    def draw(self, screen):
        ...
        def draw_limb_segment(start_pos, end_pos, length, color):
            ...
            angle = math.atan2(dy, dx); draw_rotated_rectangle(screen, color, (center_x, center_y), draw_length, self.limb_width, angle + math.pi/2) # Add pi/2 for horizontal rectangles
...
while running:
    ...
    # --- Updates --- Order Matters! ---
    # 1. Update players (handles vertical physics, head standing state, animation angles)
    ...
    # --- Player-Player Collision (REVISED BLOCK v4 - Conditional Horizontal) ---
    # Resolve horizontal overlap *after* vertical states are set
    ...
    if p1_rect.colliderect(p2_rect):
        ...
        # Do NOT resolve horizontally if one player is clearly on the other's head
        # Use the state determined by the update() method
        # Check P1 on P2's head
        p1_is_on_p2 = (
            player1.on_other_player_head and
            abs(player1.x - player2.head_pos[0]) < (player2.head_radius + HEAD_PLATFORM_RADIUS_BUFFER) and
            abs(player1.y - (player2.head_pos[1] - player2.head_radius)) < 5 # Check y proximity too
        )
        # Check P2 on P1's head
        p2_is_on_p1 = (
            player2.on_other_player_head and
            abs(player2.x - player1.head_pos[0]) < (player1.head_radius + HEAD_PLATFORM_RADIUS_BUFFER) and
            abs(player2.y - (player1.head_pos[1] - player1.head_radius)) < 5 # Check y proximity too
        )


        # --- Apply Horizontal Resolution ONLY if NOT head-standing ---
        if not p1_is_on_p2 and not p2_is_on_p1:

            # Calculate horizontal overlap using rect edges
            overlap_x = 0
            if player1.x < player2.x: # p1 is left
                overlap_x = p1_rect.right - p2_rect.left
            else: # p1 is right
                overlap_x = p2_rect.right - p1_rect.left

            # Resolve ONLY if there's horizontal overlap
            if overlap_x > 0:
                # Calculate positional push amount
                push = overlap_x / 2 + 0.1

                # Apply ONLY horizontal positional correction
                if player1.x < player2.x: # p1 is left, p2 is right
                    player1.x -= push
                    player2.x += push
                    # Stop horizontal momentum *into* the collision
                ...
                # --- No vertical velocity (vy) modification here ---
        # Else (if p1_is_on_p2 or p2_is_on_p1): Do nothing horizontally.
        # The player on top should stick to the player below (handled by vx inheritance in update).

    # --- (End of Player-Player Collision Block) ---


    # --- Goal Detection & Effects ---
    goal_scored = False; goal_pos_x = -1
    ...
    # --- Trigger Star Explosion (Combo effect) ---
    ...
    if screen_flash_timer > 0:
        ...
        flash_alpha = int(255 * (screen_flash_timer / SCREEN_FLASH_DURATION))
    ...
    if not p1_can_headbutt:
        cooldown_color = (255, 0, 0, 180); head_x, head_y = player1.head_pos; indicator_x = int(head_x); indicator_y = int(head_y + indicator_offset_y); temp_surf = pygame.Surface((cooldown_radius*2, cooldown_radius*2), pygame.SRCALPHA); pygame.draw.circle(temp_surf, cooldown_color, (cooldown_radius, cooldown_radius), cooldown_radius); screen.blit(temp_surf, (indicator_x - cooldown_radius, indicator_y - cooldown_radius))
    if not p2_can_headbutt:
        cooldown_color = (0, 0, 255, 180); head_x, head_y = player2.head_pos; indicator_x = int(head_x); indicator_y = int(head_y + indicator_offset_y); temp_surf = pygame.Surface((cooldown_radius*2, cooldown_radius*2), pygame.SRCALPHA); pygame.draw.circle(temp_surf, cooldown_color, (cooldown_radius, cooldown_radius), cooldown_radius); screen.blit(temp_surf, (indicator_x - cooldown_radius, indicator_y - cooldown_radius))
    # Timestamp
