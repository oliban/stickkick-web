...
DEBUG_BLUE = (0, 0, 255); GOAL_COLOR = (220, 220, 220); GOAL_NET_COLOR = (180, 180, 190)
...
PLAYER_PUSH_FACTOR = 0.5 # How strongly players push each other
...
class StickMan:
    def __init__(self, x, y, facing=1): # Added facing direction
        ...
        self.facing_direction = facing # 1 for right, -1 for left

    def move(self, direction):
        if not self.is_kicking:
            self.vx = direction * PLAYER_SPEED
            if direction != 0: # Update facing direction only when actively moving
    ...
    def release_charge_and_kick(self, ball_x):
        if self.is_charging:
            self.is_charging = False; self.kick_charge_level = self.charge_power; self.charge_power = MIN_CHARGE_POWER; self.vx = 0
            # Kick side depends on ball relative position, not facing direction
            self.kick_side = 'left' if ball_x < self.x else 'right'
            self.start_kick()
    def start_kick(self):
        if not self.is_kicking:
            self.is_kicking = True; self.kick_timer = 0; self.vx = 0
            # Optionally force facing direction towards kick side?
            # self.facing_direction = 1 if self.kick_side == 'right' else -1
    ...
    def draw(self, screen):
        ...
        def draw_limb_segment(start_pos, end_pos, length, color):
            center_x = (start_pos[0] + end_pos[0]) / 2; center_y = (start_pos[1] + end_pos[1]) / 2; dx = end_pos[0] - start_pos[0]; dy = end_pos[1] - start_pos[1]; draw_length = math.hypot(dx, dy);
...
# Create Players
player1 = StickMan(SCREEN_WIDTH // 4, GROUND_Y, facing=1) # Player 1 faces right
player2 = StickMan(SCREEN_WIDTH * 3 // 4, GROUND_Y, facing=-1) # Player 2 faces left
...
player1_score = 0 # Score for player 1 (right goal)
player2_score = 0 # Score for player 2 (left goal)
goal_message_timer = 0; GOAL_MESSAGE_DURATION = 1.5
current_max_height = 0.0; current_hit_count = 0; # These might be less relevant now
...
def draw_offscreen_arrow(s, ball, p_pos): # p_pos is unused now, maybe remove later
...
def reset_after_goal():
    global ball_was_on_ground # Only need this global
    ball.x = SCREEN_WIDTH // 2; ball.y = SCREEN_HEIGHT // 3; ball.vx = 0; ball.vy = 0
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0
    player1.is_kicking = False; player1.is_charging = False
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0
    player2.is_kicking = False; player2.is_charging = False
    ball_was_on_ground = False

# --- Collision Handling Function (Helper) ---
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne):
    """Handles kick, headbutt, and body collisions for one player."""
    global current_hit_count # Need global to modify hit count
    kick_performed = False
    headbutt_performed = False
    ...
    if kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 ball.apply_force(kick_x, kick_y, hitter=player); kick_performed = True # Pass player ref
    ...
    headbutt_cooldown_just_applied = False
    ...
    elif not new_can_headbutt and dist_head_sq > (ball.radius + head_radius + 15)**2: new_can_headbutt = True # Reset cooldown
    ...
    if not kick_performed and not headbutt_performed and body_collision_timer == 0:
        ...
        if dist_sq_body < ball.radius**2 and dist_sq_body > 0:
            ...
            if vel_along_normal < 0:
                ...
                new_body_collision_timer = PLAYER_BODY_COLLISION_FRAMES # Set cooldown
        elif dist_sq_body == 0:
    ...
    return score_increase, new_can_headbutt, new_body_collision_timer, kick_point if kick_performed else None
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_SPACE: player1.start_charge()
            ...
            elif event.key == pygame.K_LSHIFT: player2.start_charge() # Use Left Shift for P2 kick
            # General
        ...
        if event.type == pygame.KEYUP:
            ...
            elif event.key == pygame.K_SPACE: player1.release_charge_and_kick(ball.x)
            # Player 2
            elif event.key == pygame.K_a and player2.vx < 0: player2.stop_move()
            elif event.key == pygame.K_d and player2.vx > 0: player2.stop_move()
            elif event.key == pygame.K_LSHIFT: player2.release_charge_and_kick(ball.x)
    ...
    if p1_rect.colliderect(p2_rect):
        # Simple horizontal push-out based on relative position
        dx = player2.x - player1.x
        overlap = (p1_rect.width / 2 + p2_rect.width / 2) - abs(dx)
        if overlap > 0:
            push = overlap / 2 + 0.1 # Push each player half the overlap + buffer
            if dx > 0: # P2 is right of P1
                player1.x -= push
                player2.x += push
            else: # P1 is right of P2
                player1.x += push
                player2.x -= push
            # Optional: Add slight opposing velocity?
            player1.vx *= -0.1
            player2.vx *= -0.1
    ...
    # Score / Height Tracking Logic (Using combo score now)
    ...
    # display_height_score = int(current_max_height * current_hit_count) # Keep combo score calculation if needed

    # --- Player-Ball Collisions using Helper ---
    p1_scored, p1_can_headbutt, p1_body_collision_timer, p1_kick_pt = handle_player_ball_collisions(player1, ball, p1_can_headbutt, p1_body_collision_timer, is_ball_airborne)
    p2_scored, p2_can_headbutt, p2_body_collision_timer, p2_kick_pt = handle_player_ball_collisions(player2, ball, p2_can_headbutt, p2_body_collision_timer, is_ball_airborne)

    score_increased_this_frame = p1_scored or p2_scored
    last_kick_point = p1_kick_pt if p1_kick_pt else p2_kick_pt # Get the point from whoever kicked last for particles
    ...
    player1.draw(screen) # Draw player 1
    player2.draw(screen) # Draw player 2
    ball.draw(screen); draw_offscreen_arrow(screen, ball, (player1.x, player1.y)) # Arrow based on P1 for now
    # Scores
    score_text = f"{player2_score} - {player1_score}" # Display P2 (left) - P1 (right)
    score_surf = font_large.render(score_text, True, TEXT_COLOR); score_rect = score_surf.get_rect(centerx=SCREEN_WIDTH // 2, top=10); screen.blit(score_surf, score_rect)
    # Draw GOAL! Message
