...
def draw_game_scores(surface, scores_list, font):
    start_x = 10
    start_y = 10 # Below scoreboard area (adjust if scoreboard is very tall)
    title_surf = font.render("Last Games:", True, TEXT_COLOR)
    title_rect = title_surf.get_rect(topleft=(start_x, start_y))
    # Draw behind other UI like scoreboard if needed
    # surface.blit(title_surf, title_rect)
    current_y = title_rect.bottom + 2

    for i, score_tuple in enumerate(scores_list):
        if i >= 9: break
        score_str = f"{i+1}. {score_tuple[0]} - {score_tuple[1]}"
        score_surf = font.render(score_str, True, TEXT_COLOR)
        score_rect = score_surf.get_rect(topleft=(start_x + 10, current_y))
        surface.blit(score_surf, score_rect)
        current_y += score_rect.height + 1
def draw_trophy(surface, winner_name, title_font, name_font):
    cup_color = (255, 215, 0); base_color = (139, 69, 19)
    trophy_center_x = SCREEN_WIDTH // 2; trophy_base_y = SCREEN_HEIGHT // 2 + 100
    cup_width = 80; cup_height = 100; base_width = 120; base_height = 30
    handle_width = 20; handle_height = 60
    base_rect = pygame.Rect(0, 0, base_width, base_height); base_rect.midbottom = (trophy_center_x, trophy_base_y); pygame.draw.rect(surface, base_color, base_rect)
    ...
    cup_rect = pygame.Rect(0, 0, cup_width, cup_height); cup_rect.midbottom = (trophy_center_x, stem_bottom_y); pygame.draw.ellipse(surface, cup_color, cup_rect)
    ...
    pygame.draw.rect(surface, cup_color, handle_left_rect); pygame.draw.rect(surface, cup_color, handle_right_rect)
    title_text = "GAME WINNER!"; title_surf = title_font.render(title_text, True, YELLOW); title_rect = title_surf.get_rect(center=(trophy_center_x, trophy_base_y - 180))
    name_surf = name_font.render(winner_name, True, WHITE); name_rect = name_surf.get_rect(center=(trophy_center_x, trophy_base_y - 130))
    text_bg_rect = title_rect.union(name_rect).inflate(40, 20); bg_surf = pygame.Surface(text_bg_rect.size, pygame.SRCALPHA); bg_surf.fill((0, 0, 100, 200)); surface.blit(bg_surf, text_bg_rect.topleft)
...
# --- Stick Man Class ---
class StickMan: #... (Updated jump method) ...
...
# --- Sound Loading --- (Updated combo list)
        ...
        while True:
            ...
            if os.path.exists(path):
                try:
                    ...
                    print(f"Warning: Could not load sound '{path}': {e}"); break
            else: break
...
player1_score = 0; player2_score = 0 # Current match score
p1_games_won = 0; p2_games_won = 0    # Overall games won
game_scores = []                      # List to store past game results, max 9
match_active = True                   # Is the current match playable?
match_over_timer = 0.0                # Timer to display match winner message
game_over = False                     # Has someone won the overall game?
match_winner = None                   # Who won the last match (1 or 2)
overall_winner = None                 # Who won the game (1 or 2)
...
while running:
    ...
    if game_over:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # if event.key == pygame.K_SPACE: start_new_game() # Optional restart
        ...
        draw_game_scores(screen, game_scores, font_small) # Show final scores
    ...
    if match_over_timer > 0:
        ...
        # Normal drawing continues below to show frozen state + message

    # --- Input (Only if match active) ---
    if match_active:
        for event in pygame.event.get():
            if event.type == pygame.QUIT: running = False
            if event.type == pygame.KEYDOWN:
    ...
    # --- Goal Detection & Effects (Checks 'match_active' inside) ---
    ...
    if goal_scored_this_frame:
        ...
        if player1_score >= MATCH_POINT_LIMIT: match_winner = 1; winner_found_this_match = True; print("Player 1 wins the match!")
        elif player2_score >= MATCH_POINT_LIMIT: match_winner = 2; winner_found_this_match = True; print("Player 2 wins the match!")
        ...
        else: reset_positions() # Only reset pos if match continues
        continue # Skip drawing for this frame after goal logic
    ...
    # --- Draw UI ---
    draw_scoreboard(screen, player1_score, player2_score, font_large, font_medium, goal_message_timer > 0 and not match_active)
    draw_game_scores(screen, game_scores, font_small)
    if goal_message_timer > 0 and match_active: # Only show "GOAL!" during active play
    ...
    if match_over_timer > 0 and not game_over: # Show match winner message (unless game is already over)
