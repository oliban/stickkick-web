...
DEBUG_BLUE = (0, 0, 255)
# --- DEBUG ---
DEBUG_KICK_ANGLES = False # Set to True to print angles to console
...
KICK_RADIUS_FALLING_BONUS = 5 # Slightly increased bonus
...
# --- MORE EXAGGERATED KICK ANIMATION CONSTANTS ---
KICK_THIGH_WINDUP_ANGLE = -math.pi / 6.0  # Even less windup
KICK_THIGH_FOLLOW_ANGLE = math.pi * 0.6   # *** MUCH Stronger forward thigh swing ***
KICK_SHIN_WINDUP_ANGLE = math.pi * 0.6   # Keep shin reasonably tucked
KICK_SHIN_IMPACT_ANGLE = 0.0           # *** Shin almost straight relative to thigh at impact ***
KICK_SHIN_FOLLOW_ANGLE = math.pi * 0.3   # Bend again after impact
...
class StickMan:
    def __init__(self, x, y):
        ...
        self.is_charging = False; self.kick_timer = 0; self.kick_duration = 23 # Shorter duration might feel punchier
    ...
    def update(self, dt):
        ...
        # --- MODIFIED KICK ANIMATION LOGIC with EXAGGERATED ANGLES ---
        if self.is_kicking:
            self.walk_cycle_timer = 0
            self.kick_timer += 1
            progress = min(self.kick_timer / self.kick_duration, 1.0)

            # Define animation phases (can be tweaked)
            windup_end = 0.25 # Faster windup
            impact_start = 0.3
            impact_end = 0.6  # Impact phase
            follow_end = 1.0
            ...
            if progress < windup_end:
                thigh_prog_angle = KICK_THIGH_WINDUP_ANGLE * (progress / windup_end)
                 ...
                 ease_out_factor = 1.0 - follow_progress**1.5 # Slightly faster ease-out
                 thigh_prog_angle = KICK_THIGH_FOLLOW_ANGLE * ease_out_factor

            # Shin Angle (Relative to Thigh)
            if progress < impact_start:
                 # Interpolate towards fully tucked shin
                shin_prog_angle = KICK_SHIN_WINDUP_ANGLE * (progress / impact_start)
            elif progress < impact_end:
                 # Extend shin rapidly towards straight
                 impact_progress = (progress - impact_start) / (impact_end - impact_start)
                 shin_prog_angle = KICK_SHIN_WINDUP_ANGLE + (KICK_SHIN_IMPACT_ANGLE - KICK_SHIN_WINDUP_ANGLE) * impact_progress
            else:
                 # Bend shin again slightly
                 follow_progress = (progress - impact_end) / (follow_end - impact_end)
                 shin_prog_angle = KICK_SHIN_IMPACT_ANGLE + (KICK_SHIN_FOLLOW_ANGLE - KICK_SHIN_IMPACT_ANGLE) * follow_progress

            # --- DEBUG PRINT ---
            if DEBUG_KICK_ANGLES:
                print(f"Kick Prog: {progress:.2f}, Thigh: {math.degrees(thigh_prog_angle):.1f}, Shin: {math.degrees(shin_prog_angle):.1f}")
            # --- END DEBUG ---

            # Apply angles
            if self.kick_side == 'right':
                self.r_thigh_angle = thigh_prog_angle; self.r_shin_angle = shin_prog_angle
                self.l_thigh_angle = -thigh_prog_angle * 0.2; self.l_shin_angle = 0.2
            else:
                self.l_thigh_angle = thigh_prog_angle; self.l_shin_angle = shin_prog_angle
                self.r_thigh_angle = -thigh_prog_angle * 0.2; self.r_shin_angle = 0.2
            self.l_upper_arm_angle = -thigh_prog_angle * 0.1 if self.kick_side == 'right' else thigh_prog_angle * 0.08
            self.r_upper_arm_angle = thigh_prog_angle * 0.08 if self.kick_side == 'right' else -thigh_prog_angle * 0.1
            self.l_forearm_angle = 0.15; self.r_forearm_angle = 0.15
        ...
        # --- END KICK LOGIC ---
             ...
             if is_walking_on_ground:
                walk_sin = math.sin(self.walk_cycle_timer); self.l_upper_arm_angle = RUN_UPPER_ARM_SWING * walk_sin
                self.r_upper_arm_angle = -RUN_UPPER_ARM_SWING * walk_sin; self.l_forearm_angle = RUN_FOREARM_SWING * math.sin(self.walk_cycle_timer - RUN_FOREARM_OFFSET_FACTOR)
                self.r_forearm_angle = -RUN_FOREARM_SWING * math.sin(self.walk_cycle_timer - RUN_FOREARM_OFFSET_FACTOR); self.l_thigh_angle = -LEG_THIGH_SWING * walk_sin
                self.r_thigh_angle = LEG_THIGH_SWING * walk_sin; shin_bend = LEG_SHIN_BEND_WALK * max(0, math.sin(self.walk_cycle_timer + LEG_SHIN_BEND_SHIFT))
                self.l_shin_angle = shin_bend if self.l_thigh_angle < 0 else 0.1; self.r_shin_angle = shin_bend if self.r_thigh_angle < 0 else 0.1
             elif self.is_jumping:
                base_up_angle = JUMP_UPPER_ARM_BASE - self.vy * JUMP_UPPER_ARM_VY_FACTOR; self.l_upper_arm_angle = base_up_angle; self.r_upper_arm_angle = base_up_angle
                base_fore_angle = JUMP_FOREARM_BASE; self.l_forearm_angle = base_fore_angle; self.r_forearm_angle = base_fore_angle
                jump_progress = max(0, min(1, 1 - (self.y / self.base_y))); thigh_tuck = JUMP_THIGH_TUCK * jump_progress; shin_tuck = JUMP_SHIN_TUCK * jump_progress
                self.l_thigh_angle = thigh_tuck; self.r_thigh_angle = thigh_tuck; self.l_shin_angle = shin_tuck; self.r_shin_angle = shin_tuck
             elif self.is_charging:
                 charge_crouch = (self.charge_power - MIN_CHARGE_POWER) / (MAX_CHARGE_POWER - MIN_CHARGE_POWER); squat_angle = math.pi * 0.05 * charge_crouch
                 self.l_thigh_angle = squat_angle; self.r_thigh_angle = squat_angle; self.l_shin_angle = squat_angle * 1.5; self.r_shin_angle = squat_angle * 1.5
                 self.l_upper_arm_angle = squat_angle; self.r_upper_arm_angle = squat_angle; self.l_forearm_angle = math.pi * 0.1; self.r_forearm_angle = math.pi * 0.1
             else:
    ...
    # Getters (Impact window timing might need adjustment)
    def get_kick_impact_point(self):
        if self.is_kicking:
            impact_start = 0.3 # Start when thigh swings forward and shin starts extending
            impact_end = 0.65 # End slightly after max extension
    ...
    # Draw Method (Unchanged - still draws Ciao)
    def draw(self, screen):
        # ... (Ciao drawing code remains the same) ...
        if self.is_charging:
            bar_width = 50; bar_height = 8; bar_x = self.head_pos[0] - bar_width / 2; bar_y = self.head_pos[1] - self.head_radius - bar_height - 5
            fill_ratio = (self.charge_power - MIN_CHARGE_POWER) / (MAX_CHARGE_POWER - MIN_CHARGE_POWER); fill_width = bar_width * max(0, min(1, fill_ratio))
            pygame.draw.rect(screen, BLACK, (bar_x - 1, bar_y - 1, bar_width + 2, bar_height + 2), 1); pygame.draw.rect(screen, YELLOW, (bar_x, bar_y, fill_width, bar_height))
        head_center_int = (int(self.head_pos[0]), int(self.head_pos[1])); pygame.draw.circle(screen, ITALY_WHITE, head_center_int, self.head_radius, 0)
        draw_pentagon(screen, BLACK, head_center_int, self.head_radius * 0.6, angle=0.1); draw_pentagon(screen, BLACK, head_center_int, self.head_radius * 0.3, angle=math.pi/5 + 0.1)
        pygame.draw.circle(screen, BLACK, head_center_int, self.head_radius, 1); torso_segment_height = self.torso_length / 3; current_torso_y = self.neck_pos[1]
        for i in range(3):
            rect_center_x = self.neck_pos[0]; rect_center_y = current_torso_y + torso_segment_height / 2
            draw_rotated_rectangle(screen, self.torso_colors[i], (rect_center_x, rect_center_y), self.limb_width, torso_segment_height, 0); current_torso_y += torso_segment_height
        def draw_limb_segment(start_pos, end_pos, length, color):
            center_x = (start_pos[0] + end_pos[0]) / 2; center_y = (start_pos[1] + end_pos[1]) / 2; dx = end_pos[0] - start_pos[0]; dy = end_pos[1] - start_pos[1]
...
class Ball:
    def __init__(self, x, y, radius):
        self.x = x; self.y = y; self.radius = radius; self.vx = 0; self.vy = 0; self.last_hit_by = None; self.rotation_angle = 0
    def apply_force(self, force_x, force_y, hitter='player'): self.vx += force_x; self.vy += force_y; self.last_hit_by = hitter
    def update(self, dt):
        self.rotation_angle += self.vx * 0.015; self.rotation_angle %= (2 * math.pi); self.vy += GRAVITY; self.vx *= BALL_FRICTION
        self.x += self.vx; self.y += self.vy; hit_ground = False
    ...
    def draw(self, screen):
        # ... (Soccer ball drawing code remains the same) ...
...
def draw_offscreen_arrow(s, ball, p_pos):
    # ... (Arrow drawing code remains the same) ...
...
while running:
    ...
    if is_ball_airborne:
        current_height_pixels = max(0, GROUND_Y - (ball.y + ball.radius)); current_max_height = max(current_max_height, current_height_pixels); ball_was_on_ground = False
    elif ball_hit_ground_this_frame:
        if not ball_was_on_ground:
            final_sequence_score = int(current_max_height * current_hit_count)
            if final_sequence_score > high_score: high_score = final_sequence_score; high_score_max_height = current_max_height; high_score_hit_count = current_hit_count
            current_max_height = 0.0; current_hit_count = 0
        ball_was_on_ground = True
    display_score = int(current_max_height * current_hit_count)

    # Collision & Score (Impact timing check adjusted)
    score_increased_this_frame = False
    kick_point = player.get_kick_impact_point() # Uses updated impact window
    if kick_point:
        ...
        if dist_sq < (ball.radius + effective_kick_rad)**2:
             # Then check if the animation is in the correct impact phase *now*
             # This check doesn't strictly need get_kick_impact_point's timing, but uses its result
             # We re-evaluate progress here to be sure
             progress = player.kick_timer / player.kick_duration
             # Use a slightly generous timing check aligned with when the foot *should* be forward
             if 0.3 < progress < 0.7: # Adjusted impact check window based on animation phases
    ...
    if dist_head_sq < (ball.radius + head_radius)**2:
        if can_headbutt:
            ...
            ball.apply_force(force_x, force_y); can_headbutt = False; headbutt_cooldown_applied = True
            if is_ball_airborne: current_hit_count += 1; score_increased_this_frame = True
    if not headbutt_cooldown_applied:
        if dist_head_sq > (ball.radius + head_radius + 15) ** 2: can_headbutt = True

    # Trigger Star Explosion (Unchanged)
    ...
    if not can_headbutt:
        cooldown_color = (255, 0, 0, 180); cooldown_radius = 5; head_x, head_y = player.head_pos; indicator_x = int(head_x); indicator_y = int(head_y - player.head_radius - cooldown_radius - 2)
        temp_surf = pygame.Surface((cooldown_radius*2, cooldown_radius*2), pygame.SRCALPHA); pygame.draw.circle(temp_surf, cooldown_color, (cooldown_radius, cooldown_radius), cooldown_radius); screen.blit(temp_surf, (indicator_x - cooldown_radius, indicator_y - cooldown_radius))
    pygame.display.flip()
