...
while running:
    ...
    score_increased_this_frame = False # Initialize
    ...
    if not kick_performed_this_frame and not headbutt_performed_this_frame:
        ...
        if dist_sq_body < ball.radius**2 and dist_sq_body > 0: # Check overlap and avoid division by zero
            ...
            if vel_along_normal < 0:
                # Calculate impulse scalar (adjust based on bounce factor)
                ...
                bounce_vx += player.vx * PLAYER_VEL_TRANSFER
                bounce_vy += player.vy * PLAYER_VEL_TRANSFER

                # Enforce minimum bounce speed away from player
                new_vel_mag_sq = bounce_vx**2 + bounce_vy**2
                if new_vel_mag_sq < MIN_BODY_BOUNCE_VEL**2:
                    # If too slow, scale up to minimum speed
                    if new_vel_mag_sq > 0: # Avoid division by zero
                        scale = MIN_BODY_BOUNCE_VEL / math.sqrt(new_vel_mag_sq)
                        bounce_vx *= scale
                        bounce_vy *= scale
                    else: # If somehow zero velocity, just push directly away
        ...
        elif dist_sq_body == 0: # Ball center is exactly on closest point (rare)
             # Simple push-out upwards if stuck inside
             ball.y = player_rect.top - ball.radius - 0.1
             if ball.vy > 0: # If moving down, bounce up slightly
