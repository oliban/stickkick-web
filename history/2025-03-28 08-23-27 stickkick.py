...
# --- Sound Loading ---
# ... (Sound loading function remains the same) ...
...
running = True
while running:
    dt = clock.tick(FPS) / 1000.0; dt = min(dt, 0.1) # <<< dt CALCULATION HERE
    ...
    # This ensures sounds play even if the event system is slightly delayed
    # or if the first sound needed triggering outside the event loop.
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            # Update game scores BEFORE queuing sounds based on the *new* total
            ...
            reset_positions()
            continue # Skip drawing etc. only if match continues
