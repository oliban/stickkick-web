...
MIN_BODY_BOUNCE_VEL = 1.5
PLAYER_BODY_COLLISION_FRAMES = 4
...
# --- Goal Constants ---
GOAL_HEIGHT = 120
GOAL_WIDTH = 10 # The depth/width isn't visually represented much, but used for line check
...
# --- New Helper: Draw Goal ---
def draw_goal(surface, goal_x, goal_y, goal_height, post_thickness, color):
    """Draws a simple goal structure."""
    # Left Post
    pygame.draw.rect(surface, color, (goal_x - post_thickness, goal_y, post_thickness, goal_height))
    # Right Post (if width was defined, this would be offset) - for simplicity, we just use thickness
    pygame.draw.rect(surface, color, (goal_x, goal_y, post_thickness, goal_height)) # Post slightly inside the line
    # Crossbar
    pygame.draw.rect(surface, color, (goal_x - post_thickness, goal_y, post_thickness * 2, post_thickness))
...
class StickMan:
    ...
    def draw(self, screen):
        ...
        def draw_limb_segment(start_pos, end_pos, length, color): center_x = (start_pos[0] + end_pos[0]) / 2; center_y = (start_pos[1] + end_pos[1]) / 2; dx = end_pos[0] - start_pos[0]; dy = end_pos[1] - start_pos[1]; draw_length = math.hypot(dx, dy); if draw_length < 1: draw_length = 1; angle = math.atan2(dy, dx); draw_rotated_rectangle(screen, color, (center_x, center_y), draw_length, self.limb_width, angle)
...
font_goal = pygame.font.Font(None, 80) # Font for GOAL! message

# --- Score & State Variables ---
player_score = 0 # Player defends left, scores right
opponent_score = 0
goal_message_timer = 0 # Timer to show GOAL! message
GOAL_MESSAGE_DURATION = 1.5 # Seconds

# --- Game State Variables (Used before, kept for score display) ---
...
def reset_after_goal():
    """Resets ball and player positions after a goal."""
    ball.x = SCREEN_WIDTH // 2
    ball.y = SCREEN_HEIGHT // 3 # Start ball higher up
    ball.vx = 0
    ball.vy = 0
    player.x = SCREEN_WIDTH // 4 # Reset player to their side
    player.y = GROUND_Y
    player.vx = 0
    player.vy = 0
    player.is_kicking = False # Ensure player isn't stuck mid-kick
    player.is_charging = False
    # Reset score tracking for the new kickoff
    global current_max_height, current_hit_count, ball_was_on_ground
    current_max_height = 0.0
    current_hit_count = 0
    ball_was_on_ground = False # Ball starts airborne
...
while running:
    ...
    # --- Goal Detection ---
    goal_scored = False
    # Check if ball crosses right goal line
    if ball.x + ball.radius >= SCREEN_WIDTH and ball.y > GOAL_Y_POS:
        player_score += 1
        goal_message_timer = GOAL_MESSAGE_DURATION
        goal_scored = True
        print(f"GOAL! Player Score: {player_score}") # Console message
    # Check if ball crosses left goal line
    elif ball.x - ball.radius <= 0 and ball.y > GOAL_Y_POS:
        opponent_score += 1
        goal_message_timer = GOAL_MESSAGE_DURATION
        goal_scored = True
        print(f"GOAL! Opponent Score: {opponent_score}") # Console message

    if goal_scored:
        reset_after_goal()
        # Skip remaining physics/collision for this frame after reset
        continue # Start next loop iteration

    # --- Score / Height Tracking Logic (only if no goal scored) ---
    ...
    display_height_score = int(current_max_height * current_hit_count) # Renamed to avoid conflict
    ...
    if dist_head_sq < (ball.radius + head_radius)**2:
        if can_headbutt:
            ...
            ball.apply_force(force_x, force_y); can_headbutt = False; headbutt_cooldown_applied = True; headbutt_performed_this_frame = True
            if is_ball_airborne: current_hit_count += 1
    if not headbutt_cooldown_applied:
        if dist_head_sq > (ball.radius + head_radius + 15) ** 2: can_headbutt = True

    # --- 3. Player Body Collision ---
    if not kick_performed_this_frame and not headbutt_performed_this_frame and player_body_collision_timer == 0:
        ...
        elif dist_sq_body == 0: ball.y = player_rect.top - ball.radius - 0.1;
             if ball.vy > 0: ball.vy *= -PLAYER_BODY_BOUNCE; player_body_collision_timer = PLAYER_BODY_COLLISION_FRAMES

    # --- Score Increase Check ---
    if kick_performed_this_frame or headbutt_performed_this_frame: score_increased_this_frame = True
    ...
    # Draw Goals (Behind other elements)
    draw_goal(screen, 0, GOAL_Y_POS, GOAL_HEIGHT, GOAL_POST_THICKNESS, GOAL_COLOR)
    draw_goal(screen, SCREEN_WIDTH, GOAL_Y_POS, GOAL_HEIGHT, GOAL_POST_THICKNESS, GOAL_COLOR)

    for p in particles: p.draw(screen)
    player.draw(screen); ball.draw(screen); draw_offscreen_arrow(screen, ball, (player.x, player.y))

    # --- Draw Scores (Player vs Opponent) ---
    score_text = f"{player_score} - {opponent_score}" # New score format
    ...
    # High Score & Session Stats (Keep as before, or remove if confusing)
    if high_score > 0: high_score_text = f"Best Combo: {high_score} ({high_score_max_height:.0f}px * {high_score_hit_count} hits)"
    else: high_score_text = f"Best Combo: {high_score}"
    ...
    if goal_message_timer > 0:
        ...
        bg_surf.fill((WHITE[0], WHITE[1], WHITE[2], 180)) # Semi-transparent white
