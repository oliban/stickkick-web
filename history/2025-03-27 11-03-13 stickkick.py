...
class StickMan:
    ...
    # Draw Method (Corrected nested function)
    def draw(self, screen):
        ...
        def draw_limb_segment(start_pos, end_pos, length, color):
            ...
            if draw_length < 1:
                draw_length = 1 # Avoid zero division/size
        ...
        # Draw limbs using the nested function
...
while running:
    ...
    kick_point = player.get_kick_impact_point() # Uses faster impact window: 0.25 -> 0.6
    if kick_point:
        ...
        if dist_sq < (ball.radius + effective_kick_rad)**2:
             ...
             # *** Adjusted collision check timing ***
             if 0.25 < progress < 0.6: # Match get_kick_impact_point's window
