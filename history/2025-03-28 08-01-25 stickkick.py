...
MATCH_POINT_LIMIT = 5
GAME_WIN_LIMIT = 5
MATCH_OVER_DURATION = 3.0 # Can adjust this visual duration
...
# --- Scoreboard Function (Modified for Game Score) ---
def draw_scoreboard(surface, p1_score, p2_score, p1_games, p2_games, score_font, name_font, game_score_font, is_goal_active):
    ...
    game_score_text = f"Games: {p1_games} - {p2_games}"
    ...
    game_score_surf = game_score_font.render(game_score_text, True, TEXT_COLOR)
    game_score_rect = game_score_surf.get_rect()
    panel_padding_x = 30; panel_padding_y_top = 10; panel_padding_y_bottom = 10; text_spacing = 5
    panel_width = max(score_rect.width, name_rect.width, game_score_rect.width) + panel_padding_x * 2
    panel_height = (name_rect.height + text_spacing + game_score_rect.height + text_spacing + score_rect.height + panel_padding_y_top + panel_padding_y_bottom)
    panel_rect = pygame.Rect(0, 0, panel_width, panel_height)
    panel_rect.centerx = SCREEN_WIDTH // 2; panel_rect.top = 5
    name_rect.centerx = panel_rect.centerx; name_rect.top = panel_rect.top + panel_padding_y_top
    game_score_rect.centerx = panel_rect.centerx; game_score_rect.top = name_rect.bottom + text_spacing
    score_rect.centerx = panel_rect.centerx; score_rect.top = game_score_rect.bottom + text_spacing
    ...
    surface.blit(name_surf, name_rect); surface.blit(game_score_surf, game_score_rect); surface.blit(score_surf, score_rect)
...
def draw_game_scores(surface, scores_list, font):
    ...
    for score_tuple in reversed(scores_list):
        ...
        elif p1s == p2s and p1s >= current_limit: winner_name_str = " (Draw)" # Unlikely but possible
...
# --- Draw Trophy (Added Rematch Text) ---
def draw_trophy(surface, winner_name, title_font, name_font): # Uses name_font for rematch too
...
class StickMan: #... (as before) ...
    def __init__(self, x, y, facing=1): # ... (rest of init as before) ...
        ...
        self.facing_direction = facing; self.on_other_player_head = False
...
announcement_channel = pygame.mixer.Channel(0) # Reserve channel 0
...
def load_sounds(sound_dir="sounds"): #... (as before) ...
    ...
    return sounds
# --- Sound Playing Helpers --- (Using event queue)
...
def play_next_announcement(): # Plays next from queue if channel free
    ...
    if announcement_queue and not announcement_channel.get_busy():
        ...
        else: play_next_announcement() # Skip None and try next immediately
...
game_over_sound_played = False # Flag for game over win sound
announcement_queue = []
...
def start_new_game(): # Reset game_over_sound_played and queue
    ...
    game_over_sound_played = False # Reset game over sound flag
    announcement_queue = []
    start_new_match()
...
while running:
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if game_over and event.key == pygame.K_r: # Rematch only possible when game is over
                start_new_game()
        ...
        if event.type == pygame.KEYUP:
            if match_active: # Only process stop if match is active
    ...
    if game_over:
        # --- Play Game Winner Sound ONCE ---
        if not game_over_sound_played:
            if overall_winner == 1: play_sound(loaded_sounds['nils_wins'])
            elif overall_winner == 2: play_sound(loaded_sounds['harry_wins'])
        ...
        pygame.display.flip(); continue # Skip rest of loop

    # --- Handle Match Over State ---
    if match_over_timer > 0:
        # --- Queue End-of-Match Sounds ONCE ---
        if not match_end_sound_played and not game_over:
            ...
            play_next_announcement() # Trigger the first sound in the queue
        ...
        if match_over_timer <= 0 and not game_over:
             start_new_match()
        # Allow quit handled by general event check outside this block now
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            match_end_sound_played = False # Reset flag to allow sounds for this match end
        ...
        if not winner_found_this_match: continue
    ...
    # --- Draw UI ---
    draw_scoreboard(screen, player1_score, player2_score, p1_games_won, p2_games_won, font_large, font_medium, font_medium, goal_message_timer > 0 or match_over_timer > 0)
    draw_game_scores(screen, game_scores, font_small)
