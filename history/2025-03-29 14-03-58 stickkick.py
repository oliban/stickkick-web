...
# Added is_jumping = True for ground hits
...
class StickMan: # Changed current_powerup to active_powerups set
    def __init__(self, x, y, facing=1):
        ...
        # --- Powerup Attributes ---
        self.active_powerups = set() # <<< Changed to a set
        self.is_flying = False; self.flight_timer = 0.0 # Flight state separate from set presence
    ...
    def stop_move(self): # ... (no change) ...
        if self.is_tumbling: return
        self.vx = 0
    def jump(self): # Check set for FLIGHT
        if self.is_tumbling: return
        can_jump_now = False
        if "FLIGHT" in self.active_powerups: # <<< Check set
        ...
        if can_jump_now:
            ...
            if "FLIGHT" in self.active_powerups: self.start_wing_flap() # <<< Check set

    def start_kick(self): # Check set for ROCKET_LAUNCHER
        if self.is_tumbling: return
        if not self.is_kicking:
            if "ROCKET_LAUNCHER" in self.active_powerups: # <<< Check set
                self.fire_rocket()
            else: self.is_kicking = True; self.kick_timer = 0; self.vx = 0

    def fire_rocket(self): # Remove from set on fire
        global active_rockets
        if "ROCKET_LAUNCHER" not in self.active_powerups: return # <<< Check set
        ...
        self.active_powerups.discard("ROCKET_LAUNCHER") # <<< Remove from set
    ...
    def update(self, dt, other_player): # Check set for powerup logic
        # --- Tumbling State Update ---
        if self.is_tumbling: # ... (no change) ...
        ...
        # --- Powerup Timer Countdown & Logic ---
        if "FLIGHT" in self.active_powerups: # <<< Check set
            if self.is_flying: # Only tick down if currently flying (timer might be reset)
                self.flight_timer -= dt
                if self.flight_timer <= 0:
                    self.active_powerups.discard("FLIGHT") # <<< Remove from set
                    self.is_flying = False; self.flight_timer = 0; print("Flight ended")
        else:
            self.is_flying = False # Ensure not flying if powerup not in set
        ...
        if not self.is_kicking:
            ...
            # Simple friction/drag when tumbling in air
            if self.is_tumbling and not self.on_other_player_head and self.y < self.base_y:
                 self.vx *= 0.99
                 effective_vx = self.vx # Use the possibly reduced vx
            self.x += effective_vx * dt * 60
        ...
        if not self.is_tumbling: # ... (no change inside this block) ...
            ...
            tumble_speed = self.rotation_velocity * 1.5; current_time_ms = pygame.time.get_ticks() # Use current time for variation
    ...
    def get_body_rect(self): return self.body_rect # Collision rect doesn't rotate

    def draw(self, screen): # Check set for drawing powerup items
        # --- Calculate Bounding Box & Offset (unchanged) ---
        ...
        if "ROCKET_LAUNCHER" in self.active_powerups: all_points.append(self.gun_tip_pos) # Include gun if active
        ...
        # --- Draw Head, Torso, Limbs onto Temp Surface (unchanged) ---
        ...
        # --- Draw Wings if FLIGHT is active ---
        if "FLIGHT" in self.active_powerups: # <<< Check set
        ...
        # --- Draw Gun if ROCKET_LAUNCHER is active ---
        if "ROCKET_LAUNCHER" in self.active_powerups: # <<< Check set
            ...
            # Calculate world gun tip pos (needed for laser origin)
        ...
        if self.is_tumbling and self.rotation_angle != 0:
            ...
            screen.blit(rotated_surf, (blit_rect.left - offset_x, blit_rect.top - offset_y)) # Adjust blit pos by negative offset
        else:
            screen.blit(temp_surf, (min_x, min_y)) # Blit normally

        # --- Draw Laser Separately ---
        if "ROCKET_LAUNCHER" in self.active_powerups: # <<< Check set
...
active_powerups = [] # Use list for multiple
...
def reset_positions(): # Reset player powerup set and tumble state
    ...
    # Keep active_powerups list intact here - they don't disappear on goal
    active_rockets = []; active_explosions = []
...
def start_new_game(): # ... (no change) ...
    ...
    active_powerups = [] # <<< Clear falling powerups list for a completely new game
...
while running:
    ...
    if match_active:
        ...
        active_powerups = [p for p in active_powerups if p.active and p.update(dt)] # Simplified update/removal

        particles = [p for p in particles if p.update(dt)]
        active_rockets = [r for r in active_rockets if r.active and not r.update(dt, player_list, ball)]
        active_explosions = [e for e in active_explosions if e.active and e.update(dt)]

        # --- Power-up Collection ---
        collected_powerups_indices = []
        for i, pup in enumerate(active_powerups): # Use enumerate to get index
            collected_this_pup = False
            # Check player 1
            if not collected_this_pup:
                collected_type_p1 = pup.check_collision(player1)
                if collected_type_p1:
                    player1.active_powerups.add(collected_type_p1) # Add to set
            ...
            if not collected_this_pup:
                ...
                if collected_type_p2:
                    ...
                    collected_powerups_indices.append(i)
                    # collected_this_pup = True # Not needed here

        # Remove collected powerups from the main list
        if collected_powerups_indices:
             # Create a new list excluding the collected indices
    ...
    if match_active:
        ...
        if "FLIGHT" in player1.active_powerups:
            ...
            p1_ui_y -= line_height # Move next indicator up
