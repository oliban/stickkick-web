...
DEBUG_BLUE = (0, 0, 255)
# --- TEMP DEBUG COLORS ---
# GOAL_COLOR = (220, 220, 220); GOAL_NET_COLOR = (180, 180, 190) # Original Colors
GOAL_COLOR = (255, 0, 0) # Bright Red for posts/bars
GOAL_NET_COLOR = (0, 255, 0) # Bright Green for net lines
# --- END TEMP DEBUG ---
...
# --- Goal Drawing Function (Restored Logic + INT conversion) ---
def draw_goal_isometric(surface, goal_line_x, goal_y, goal_height, depth_x, depth_y, thickness, post_color, net_color):
    # depth_x is the offset passed from the call (-GOAL_DEPTH_X or +GOAL_DEPTH_X)
    front_top = (goal_line_x, goal_y)
    front_bottom = (goal_line_x, goal_y + goal_height)

    # Calculate back post coordinates using the passed depth_x directly
    ...
    # Convert to integers right before drawing (safer for some Pygame versions/platforms)
    ...
    # Draw the posts and top/bottom bars using integer coords
    ...
    pygame.draw.line(surface, post_color, bb_int, fb_int, thickness)     # Bottom Bar
    pygame.draw.line(surface, post_color, ft_int, fb_int, thickness)     # Front Post

    # Draw simple net lines (crosses for perspective) using integer coords
    ...
    def update(self, dt, other_player):
        ...
        if not self.is_kicking:
            ...
            if self.on_other_player_head:
                effective_vx += other_player.vx # Add velocity of player below
            self.x += effective_vx # Apply combined velocity
...
while running:
    ...
    # --- Draw Goals (Using restored function and correct offsets) ---
