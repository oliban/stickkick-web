...
def reset_after_goal():
    global ball_was_on_ground # Combo score vars removed
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne):
    # Combo score 'current_hit_count' removed from here
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 kick_x = kick_x_base * KICK_FORCE_LEVEL
...
while running:
    ...
    # Ball state needed for collision helper
    is_ball_airborne = not ball.is_on_ground()
    ball_was_on_ground = not is_ball_airborne # Update this *before* collisions modify state potentially
    ...
    score_increased_this_frame = p1_hit or p2_hit # Flag if any scoring hit occurred
    last_kick_point = p1_kick_pt if p1_kick_pt else p2_kick_pt # For particle effect

    # Combo score logic could be re-introduced here if desired,
    # using score_increased_this_frame and is_ball_airborne

    # Trigger Star Explosion (Example: Still uses combo score, needs adjustment if removed)
    # If you removed combo score, you might change this condition or remove particles
    # Example: Trigger on any score increase
    # if score_increased_this_frame and last_kick_point:
    # Example: Or keep combo logic if you re-add current_hit_count tracking
