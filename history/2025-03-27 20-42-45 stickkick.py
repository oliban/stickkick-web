...
MATCH_POINT_LIMIT = 5 # <<< UPDATED Normal match limit
GAME_WIN_LIMIT = 5
...
def draw_game_scores(surface, scores_list, font):
    ...
    for score_tuple in reversed(scores_list): # Show oldest first
        ...
        elif p1s >= DEBUG_MATCH_POINT_LIMIT and p1s > p2s: winner_name_str = " (Nils)" # Check debug limit if normal not met
        elif p2s >= DEBUG_MATCH_POINT_LIMIT and p2s > p1s: winner_name_str = " (Harry)" # Check debug limit if normal not met
...
def draw_trophy(surface, winner_name, title_font, name_font):
    ...
    title_rect = title_surf.get_rect(center=(trophy_center_x, trophy_base_y - 220))
...
while running:
    ...
    if game_over:
        for event in events:
             if event.type == pygame.KEYDOWN:
                 if event.key == pygame.K_ESCAPE: running = False
                 if event.key == pygame.K_r: # <<< Check for R key
                     start_new_game() # <<< Call function to restart
        ...
        draw_trophy(screen, winner_name, font_goal, font_large) # Pass correct fonts
