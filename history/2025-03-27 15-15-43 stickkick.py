...
# Goal Constants
GOAL_HEIGHT = 120; GOAL_WIDTH = 10; GOAL_POST_THICKNESS = 8
...
def draw_goal(surface, goal_x, goal_y, goal_height, post_thickness, color):
    ...
    pygame.draw.rect(surface, color, (goal_x, goal_y, post_thickness, goal_height))
...
player_score = 0; opponent_score = 0; goal_message_timer = 0; GOAL_MESSAGE_DURATION = 1.5
...
def reset_after_goal():
    global current_max_height, current_hit_count, ball_was_on_ground # Declare globals needed
...
while running:
    ...
    # --- Goal Detection ---
    goal_scored = False
    if ball.x + ball.radius >= SCREEN_WIDTH and ball.y > GOAL_Y_POS: player_score += 1; goal_message_timer = GOAL_MESSAGE_DURATION; goal_scored = True; print(f"GOAL! Player Score: {player_score}")
    elif ball.x - ball.radius <= 0 and ball.y > GOAL_Y_POS: opponent_score += 1; goal_message_timer = GOAL_MESSAGE_DURATION; goal_scored = True; print(f"GOAL! Opponent Score: {opponent_score}")
    ...
    if is_ball_airborne: current_height_pixels = max(0, GROUND_Y - (ball.y + ball.radius)); current_max_height = max(current_max_height, current_height_pixels); ball_was_on_ground = False
        ...
        if not ball_was_on_ground: # Just hit ground
            ...
            current_max_height = 0.0 # Reset session
            current_hit_count = 0
    ...
    # High Score / Session Stats (Optional Display)
