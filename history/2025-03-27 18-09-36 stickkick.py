...
import sys
import math
import random # Needed for random sound choice
...
# --- Stick Man Class --- (Added sound trigger checks)
class StickMan:
    ...
    def jump(self):
        if not self.is_jumping or self.on_other_player_head:
            play_sound(loaded_sounds['jump']) # <<< Play jump sound
    ...
    def update(self, dt, other_player):
        ...
        if self.vy > 0 and self.y == self.base_y: self.vy = 0; self.is_jumping = False; # Ensure vy stops on ground
        ...
        if was_airborne and is_now_grounded:
            play_sound(loaded_sounds['land']) # Play general land sound

        # --- Update horizontal position --- (as before)
        intended_vx = self.vx
        if not self.is_kicking:
        ...
        # --- Calculate Joint Positions --- (as before)
...
class Ball:
    ...
    def update(self, dt):
        ...
        hit_ground = False
        hit_wall_this_frame = False # Track wall hit for sound

        # Wall collision check
        if self.x + self.radius >= SCREEN_WIDTH:
            self.x = SCREEN_WIDTH - self.radius; self.vx *= -BALL_BOUNCE * 0.8
            hit_wall_this_frame = True
        elif self.x - self.radius <= 0:
            self.x = self.radius; self.vx *= -BALL_BOUNCE * 0.8
            hit_wall_this_frame = True

        # Play wall sound (only once per frame even if hitting corner)
        if hit_wall_this_frame:
             play_sound(loaded_sounds['wall_hit'])

        # Ground collision check
        if self.y + self.radius >= GROUND_Y:
            if self.vy >= 0: hit_ground = True
            # Optional: Play bounce sound based on velocity before bounce
            # if abs(self.vy) > 1.5: play_sound(loaded_sounds.get('bounce', [])) # Add 'bounce' sound if desired
...
# --- Game Setup ---
pygame.init()
pygame.mixer.pre_init(44100, -16, 2, 512) # Optional buffer settings
pygame.mixer.init()
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne):
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 play_sound(loaded_sounds['kick']) # <<< Kick sound
    ...
    if dist_head_sq < (ball.radius + head_radius)**2:
        if can_headbutt:
            ...
            play_sound(loaded_sounds['headbutt']) # <<< Headbutt sound
            if is_ball_airborne: current_hit_count += 1; score_increase = True
    ...
    if not kick_performed and not headbutt_performed and body_collision_timer == 0:
        ...
        if dist_sq_body < ball.radius**2:
             collision_occurred = False # Flag to play sound only once
             if dist_sq_body > 0:
             ...
             if collision_occurred:
                 play_sound(loaded_sounds['body_hit']) # <<< Body hit sound
...
while running:
    ...
    if p1_rect.colliderect(p2_rect):
        ...
        if not p1_is_on_p2 and not p2_is_on_p1:
            ...
            if overlap_x > 0:
                play_sound(loaded_sounds['player_bump']) # <<< Player bump sound
    ...
    # --- Goal Detection & Effects --- (Added sound trigger)
    ...
    if goal_scored:
        if scorer == 1: play_sound(loaded_sounds['goal_p1']) # <<< Goal sound P1
        elif scorer == 2: play_sound(loaded_sounds['goal_p2']) # <<< Goal sound P2
    ...
    # --- Player-Ball Collisions --- (No changes here, sounds are inside the function)
    ...
    if score_increased_this_frame and last_kick_point and current_hit_count > 0 and current_hit_count % 5 == 0:
        play_sound(loaded_sounds['combo']) # <<< Combo sound
