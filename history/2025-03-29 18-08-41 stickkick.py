...
POWERUP_BALL_FREEZE_DURATION = 25.0
...
ROCKET_EXPLOSION_FORCE = 95.0 # <<< Increased force slightly more again
ROCKET_PLAYER_UPWARD_BOOST = 20.0 # <<< Increased upward boost again
...
class StickMan: # ... (rest of methods mostly unchanged) ...
    def __init__(self, x, y, facing=1):
        # ... (most attributes unchanged) ...
        ...
        # --- Powerup Attributes (Using Dictionary) ---
        self.active_powerups = {} # Format: {"POWERUP_TYPE": value} value = timer/ammo/level
        self.is_flying = False # State flag derived from dict check
        self.is_big = False
    ...
    def apply_powerup(self, powerup_type, other_player=None):
        ...
        if powerup_type == "FLIGHT":
            new_timer = POWERUP_FLIGHT_DURATION # Always reset duration
            self.active_powerups["FLIGHT"] = new_timer; self.is_flying = True
            print(f"Flight activated/refreshed: {new_timer:.1f}s")
        elif powerup_type == "ROCKET_LAUNCHER":
            new_ammo = current_val + 1 # Add one rocket
            self.active_powerups["ROCKET_LAUNCHER"] = new_ammo
            print(f"Rocket Launcher ammo: {new_ammo}")
        elif powerup_type == "BIG_PLAYER":
            new_timer = current_val + POWERUP_BIG_PLAYER_DURATION
            ...
            self.calculate_current_sizes(); print(f"Big Player extended: {new_timer:.1f}s")
        elif powerup_type == "SUPER_JUMP":
            new_timer = current_val + POWERUP_SUPER_JUMP_DURATION
            self.active_powerups["SUPER_JUMP"] = new_timer
            # Simple stacking: add multipliers? Let's increase the base multiplier slightly each time?
            # Or just refresh timer and keep multiplier constant? Let's refresh timer.
            self.jump_power = BASE_JUMP_POWER * POWERUP_SUPER_JUMP_MULTIPLIER
            print(f"Super Jump duration extended: {new_timer:.1f}s")
        elif powerup_type == "SPEED_BOOST":
            new_timer = current_val + POWERUP_SPEED_BOOST_DURATION
            self.active_powerups["SPEED_BOOST"] = new_timer
            self.player_speed = BASE_PLAYER_SPEED * POWERUP_SPEED_BOOST_MULTIPLIER
            print(f"Speed Boost duration extended: {new_timer:.1f}s")
        elif powerup_type == "SHRINK_OPPONENT":
            if other_player: other_player.apply_shrink()
        elif powerup_type == "GOAL_SHIELD":
             # P1 faces right (1), defends left goal. P2 faces left (-1), defends right goal.
             if self.facing_direction == 1:
                 global p1_shield_active, p1_shield_timer
                 p1_shield_timer += POWERUP_GOAL_SHIELD_DURATION
                 p1_shield_active = True; print(f"P1 Shield extended: {p1_shield_timer:.1f}s")
             else:
                 global p2_shield_active, p2_shield_timer
                 p2_shield_timer += POWERUP_GOAL_SHIELD_DURATION
                 p2_shield_active = True; print(f"P2 Shield extended: {p2_shield_timer:.1f}s")
             # Don't add GOAL_SHIELD to player's active_powerups dict
        elif powerup_type == "BALL_FREEZE":
             global ball_freeze_timer
             ball_freeze_timer += POWERUP_BALL_FREEZE_DURATION # Add duration
             ball.is_frozen = True; ball.freeze_effect_timer = 0; print(f"Ball Freeze extended: {ball_freeze_timer:.1f}s")
             ball.vx = 0; ball.vy = 0
             # Don't add BALL_FREEZE to player's active_powerups dict
    ...
    def calculate_current_sizes(self): # ... (no change) ...
        scale = 1.0
    ...
    def update(self, dt, other_player): # ... (no change) ...
        if self.is_tumbling:
        ...
        for p_type, value in list(self.active_powerups.items()):
            if p_type == "FLIGHT": new_timer = value - dt; self.active_powerups["FLIGHT"] = new_timer; self.is_flying = new_timer > 0; if new_timer <= 0: expired_powerups.append(p_type); print("Flight ended")
            elif p_type == "BIG_PLAYER": new_timer = value - dt; self.active_powerups["BIG_PLAYER"] = new_timer; if new_timer <= 0: expired_powerups.append(p_type); self.is_big = False; self.calculate_current_sizes(); print("Big Player ended")
            elif p_type == "SHRUNK": new_timer = value - dt; self.active_powerups["SHRUNK"] = new_timer; if new_timer <= 0: expired_powerups.append(p_type); self.is_shrunk = False; self.calculate_current_sizes(); print("Shrink ended")
            elif p_type == "SUPER_JUMP": new_timer = value - dt; self.active_powerups["SUPER_JUMP"] = new_timer; if new_timer <= 0: expired_powerups.append(p_type); self.jump_power = BASE_JUMP_POWER; print("Super Jump ended")
            elif p_type == "SPEED_BOOST": new_timer = value - dt; self.active_powerups["SPEED_BOOST"] = new_timer; if new_timer <= 0: expired_powerups.append(p_type); self.player_speed = BASE_PLAYER_SPEED; print("Speed Boost ended")
...
def reset_positions(): # Also resets powerup dictionary
    ...
    # Reset Player 1
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0; player1.is_kicking = False; player1.on_other_player_head = False; player1.facing_direction = 1; player1.is_jumping = False; player1.active_powerups = {}; player1.is_flying = False; player1.is_big = False; player1.is_shrunk = False; player1.jump_power=BASE_JUMP_POWER; player1.player_speed=BASE_PLAYER_SPEED; player1.calculate_current_sizes(); player1.is_tumbling = False; player1.rotation_angle = 0; player1.rotation_velocity = 0
    # Reset Player 2
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0; player2.is_kicking = False; player2.on_other_player_head = False; player2.facing_direction = -1; player2.is_jumping = False; player2.active_powerups = {}; player2.is_flying = False; player2.is_big = False; player2.is_shrunk = False; player2.jump_power=BASE_JUMP_POWER; player2.player_speed=BASE_PLAYER_SPEED; player2.calculate_current_sizes(); player2.is_tumbling = False; player2.rotation_angle = 0; player2.rotation_velocity = 0
    # Reset other states
...
while running:
    ...
    if match_active:
        ...
        active_powerups = [p for p in active_powerups if p.update(dt)] # Use update's return
        ...
        # --- Power-up Collection ---
        collected_powerups_indices = [] # ... (unchanged logic for handling multiple powerups) ...
        ...
        if p1_kick_point and p2_rect.collidepoint(p1_kick_point) and not player2.is_tumbling: # Check if opponent is tumbling
            print("P1 kicked P2")
            ...
            player2.vy -= 3 * kick_multiplier # Add some upward kick too
        ...
        if p2_kick_point and p1_rect.collidepoint(p2_kick_point) and not player1.is_tumbling: # Check if opponent is tumbling
            print("P2 kicked P1")
            ...
            player1.vy -= 3 * kick_multiplier # Add some upward kick too
    ...
    if match_active and not ball.is_frozen:
        if ball.x + ball.radius >= GOAL_LINE_X_RIGHT and ball.y > GOAL_Y_POS and not p2_shield_active: # <<< Corrected shield check for P2's goal
        ...
        elif ball.x - ball.radius <= GOAL_LINE_X_LEFT and ball.y > GOAL_Y_POS and not p1_shield_active: # <<< Corrected shield check for P1's goal
    ...
    if goal_scored_this_frame:
        ...
        # Reset positions BUT KEEP powerups/timers/global states
        # Call a modified reset or just reset specific things here?
        # Let's modify reset_positions slightly to NOT reset powerups
        reset_positions() # reset_positions needs adjustment
        continue
    ...
    if p1_shield_active: # P1 defends LEFT goal
        shield_rect = pygame.Rect(GOAL_LINE_X_LEFT - POWERUP_GOAL_SHIELD_WIDTH // 2, GOAL_Y_POS, POWERUP_GOAL_SHIELD_WIDTH, GOAL_HEIGHT) # Centered on line
        shield_surf = pygame.Surface(shield_rect.size, pygame.SRCALPHA)
        shield_surf.fill(shield_color_with_alpha)
        screen.blit(shield_surf, shield_rect.topleft)
        pygame.draw.rect(screen, WHITE, shield_rect, 1)

    if p2_shield_active: # P2 defends RIGHT goal
        shield_rect = pygame.Rect(GOAL_LINE_X_RIGHT - POWERUP_GOAL_SHIELD_WIDTH // 2, GOAL_Y_POS, POWERUP_GOAL_SHIELD_WIDTH, GOAL_HEIGHT) # Centered on line
        shield_surf = pygame.Surface(shield_rect.size, pygame.SRCALPHA)
        shield_surf.fill(shield_color_with_alpha)
        screen.blit(shield_surf, shield_rect.topleft)
        pygame.draw.rect(screen, WHITE, shield_rect, 1)
    ...
    if match_active:
        ...
        powerup_timers = { "FLIGHT": "FLIGHT", "BIG_PLAYER": "BIG_PLAYER", "SUPER_JUMP": "SUPER_JUMP", "SPEED_BOOST": "SPEED_BOOST", "SHRUNK": "SHRUNK"} # Map type to key in dict

        # Player 1 UI
        p1_ui_y = SCREEN_HEIGHT - 30
        for p_type in sorted(player1.active_powerups.keys()):
            val = player1.active_powerups[p_type]
            text_template = powerup_texts.get(p_type, p_type)
            text = ""
            if p_type == "ROCKET_LAUNCHER": text = text_template.format(val) # Show ammo
            elif p_type in powerup_timers: text = text_template.format(val) # Show timer
            else: text = text_template # Just show name
        ...
        for p_type in sorted(player2.active_powerups.keys()):
            val = player2.active_powerups[p_type]
            text_template = powerup_texts.get(p_type, p_type)
            text = ""
            if p_type == "ROCKET_LAUNCHER": text = text_template.format(val)
            elif p_type in powerup_timers: text = text_template.format(val)
