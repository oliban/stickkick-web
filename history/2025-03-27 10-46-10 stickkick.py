...
# --- MODIFIED KICK ANIMATION CONSTANTS ---
# Thigh swings back less, forward more
KICK_THIGH_WINDUP_ANGLE = -math.pi / 5.0 # Less windup
KICK_THIGH_FOLLOW_ANGLE = math.pi * 0.4  # Strong forward thigh swing
# Shin winds up more (tucked), extends almost straight at impact
KICK_SHIN_WINDUP_ANGLE = math.pi * 0.7 # Tucked shin
KICK_SHIN_IMPACT_ANGLE = math.pi * 0.1 # Slightly bent forward relative to thigh at impact
KICK_SHIN_FOLLOW_ANGLE = math.pi * 0.2 # Follow through bend
...
class StickMan:
    def __init__(self, x, y):
        ...
        self.kick_timer = 0; self.kick_duration = 23 # Keep duration relatively short
    ...
    def update(self, dt):
        ...
        if self.is_kicking:
            ...
            windup_end = 0.3
            impact_start = 0.4
            impact_end = 0.6 # Short impact phase
            follow_start = 0.65
            follow_end = 1.0
            ...
            if progress < windup_end:
                ...
                thigh_prog_angle = KICK_THIGH_WINDUP_ANGLE * (progress / windup_end)
                 ...
                 # Swing thigh forward through impact
                 ...
                 # Slow down thigh swing after impact
                 follow_progress = (progress - impact_end) / (follow_end - impact_end)
                 # Ease out: Start slowing down quickly, then less so
                 ease_out_factor = 1.0 - follow_progress**2 # Simple quadratic ease-out
                 thigh_prog_angle = KICK_THIGH_FOLLOW_ANGLE * ease_out_factor # Bring back towards 0 from max angle

            # --- Shin Angle Calculation (Relative to Thigh) ---
            if progress < impact_start:
                # Keep shin tucked during windup and early swing
                shin_prog_angle = KICK_SHIN_WINDUP_ANGLE * (progress / impact_start) # Interpolate towards fully tucked
            elif progress < impact_end:
                # Extend shin rapidly for impact
                impact_progress = (progress - impact_start) / (impact_end - impact_start)
                # Go from tucked (WINDUP) to extended (IMPACT)
                shin_prog_angle = KICK_SHIN_WINDUP_ANGLE + (KICK_SHIN_IMPACT_ANGLE - KICK_SHIN_WINDUP_ANGLE) * impact_progress
            else: # Follow through
                # Bend shin again slightly during follow through
                follow_progress = (progress - impact_end) / (follow_end - impact_end)
                # Go from extended (IMPACT) to slightly bent (FOLLOW)
            ...
            if self.kick_side == 'right':
                ...
                self.r_shin_angle = shin_prog_angle # Shin angle is relative to thigh
                # Non-kicking leg does a small counter-movement
                self.l_thigh_angle = -thigh_prog_angle * 0.2
                self.l_shin_angle = 0.2 # Slightly bent
            else: # Left leg kick
                self.l_thigh_angle = thigh_prog_angle
                self.l_shin_angle = shin_prog_angle
                self.r_thigh_angle = -thigh_prog_angle * 0.2
                self.r_shin_angle = 0.2
        ...
        # Calculate Joint Positions (Logic remains the same)
        ...
        # Shin angle is relative to thigh, so knee position calculation is correct
        ...
        # Foot angle calculation needs world angle of shin (thigh angle + shin angle)
        ...
        # Grounding Logic (Logic remains the same)
        ground = self.base_y
    ...
    def get_kick_impact_point(self):
        if self.is_kicking:
            # Adjust impact window if needed based on new animation timing
            impact_start = 0.4
            impact_end = 0.65 # Slightly extend window?
    ...
    def draw(self, screen):
        ...
        def draw_limb_segment(start_pos, end_pos, length, color):
            ...
            # Draw length should be the actual distance, height is thickness
...
# --- Ball Class (Unchanged from previous) ---
class Ball:
    ...
    def draw(self, screen):
        ...
        pent_size = self.radius * 0.40; hex_size = self.radius * 0.42
        dist_factor = 0.65; num_around = 5; angle_step = 2 * math.pi / num_around
...
def draw_offscreen_arrow(s, ball, p_pos):
    ...
    p1r = (p1[0] * cos_a - p1[1] * sin_a, p1[0] * sin_a + p1[1] * cos_a); p2r = (p2[0] * cos_a - p2[1] * sin_a, p2[0] * sin_a + p2[1] * cos_a); p3r = (p3[0] * cos_a - p3[1] * sin_a, p3[0] * sin_a + p3[1] * cos_a)
...
while running:
    ...
    # Collision & Score (Kick collision check uses get_kick_impact_point with new timing)
    ...
    if kick_point:
        ...
        # Check collision timing within the updated kick impact window
        if dist_sq < (ball.radius + effective_kick_rad)**2:
             # Collision occurs, now check if it's during the valid part of the animation
             progress = player.kick_timer / player.kick_duration
             impact_start_check = 0.4 # Should match get_kick_impact_point start
             impact_end_check = 0.65 # Should match get_kick_impact_point end
             if impact_start_check < progress < impact_end_check:
    ...
    if not can_headbutt:
        cooldown_color = (255, 0, 0, 180); cooldown_radius = 5
        head_x, head_y = player.head_pos; indicator_x = int(head_x); indicator_y = int(head_y - player.head_radius - cooldown_radius - 2)
