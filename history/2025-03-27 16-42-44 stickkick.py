...
class StickMan:
    def __init__(self, x, y, facing=1):
        ...
        # Set default colors (P2) - will be overridden for player1 later
    ...
    def update(self, dt, other_player):
        ...
        # --- Vertical Movement & Platform Checks (Revised Grounding) ---
        landed_on_head_this_frame = False
        landed_on_ground_this_frame = False
        platform_y = self.base_y # Default platform is ground

        # Check for standing on other player's head
        ...
        can_land_on_head = abs(dist_x_head) < platform_radius and self.y >= head_top_y - 5 # Check if roughly above/near head level

        if can_land_on_head:
            platform_y = head_top_y # Potential platform is the head

        # Calculate potential next position
        next_y = self.y + self.vy * dt * 60 # Approximation using dt
        if not self.on_other_player_head: # Apply gravity if not already on head
            self.vy += GRAVITY
            next_y = self.y + self.vy # More direct calculation after gravity

        # Check for landing ON A PLATFORM (Head or Ground)
        if next_y >= platform_y and self.vy >= 0:
            if can_land_on_head and platform_y == head_top_y: # Landed on head
                self.y = platform_y - 1 # Place slightly above head platform
                self.vy = 0
            ...
            # If falling but not meeting landing conditions for current platform, keep falling
            elif self.is_jumping:
                 self.y = next_y

        # Handle falling off a head
        elif self.on_other_player_head and not can_land_on_head:
             self.on_other_player_head = False
             self.is_jumping = True # Start falling
             self.y = next_y # Update position based on current velocity

        # Normal airborne movement
        elif self.is_jumping or self.vy < 0:
            self.y = next_y
        ...
        if self.is_kicking:
            ...
            kick_direction_multiplier = self.facing_direction # 1 if right, -1 if left
            if kick_direction_multiplier == 1: # Facing right
                self.r_thigh_angle = thigh_prog_angle; self.r_shin_angle = shin_prog_angle
                self.l_thigh_angle = -thigh_prog_angle * 0.3; self.l_shin_angle = 0.3
            else: # Facing left
                self.l_thigh_angle = thigh_prog_angle * kick_direction_multiplier # Angle needs to be mirrored
                self.l_shin_angle = shin_prog_angle # Relative angle is the same
                self.r_thigh_angle = -thigh_prog_angle * 0.3 * kick_direction_multiplier
                self.r_shin_angle = 0.3
            # Arms react based on thigh angle magnitude
        ...
        # Calculate Joint Positions (Crucially, use the potentially updated self.y)
    ...
    def get_kick_impact_point(self):
        if self.is_kicking:
            ...
            if impact_start < progress < impact_end:
                # *** Return foot based on facing direction ***
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 kick_x = BASE_KICK_FORCE_X * KICK_FORCE_LEVEL * player.facing_direction # Use facing dir
                 kick_y = BASE_KICK_FORCE_Y * KICK_FORCE_LEVEL
                 if player.vy < 0: kick_y += player.vy * 0.4 # Stronger volley
