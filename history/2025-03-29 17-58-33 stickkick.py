...
POWERUP_SHRINK_PLAYER_DURATION = 15.0
...
POWERUP_GOAL_SHIELD_COLOR = (0, 220, 255, 180) # <<< Brighter cyan, more opaque
POWERUP_GOAL_SHIELD_WIDTH = 12 # <<< Thicker shield
POWERUP_GOAL_SHIELD_PULSE_SPEED = 4.0
POWERUP_GOAL_SHIELD_MIN_ALPHA = 120
POWERUP_GOAL_SHIELD_MAX_ALPHA = 230
# Player Kick Powerup Constants
...
class StickMan: # ... (no change except apply_powerup and size reset) ...
    ...
    def apply_powerup(self, powerup_type, other_player=None):
        ...
        if powerup_type == "FLIGHT":
            self.flight_timer = POWERUP_FLIGHT_DURATION; self.is_flying = True
        elif powerup_type == "BIG_PLAYER":
            self.big_timer = POWERUP_BIG_PLAYER_DURATION; self.is_big = True
            if "SHRUNK" in self.active_powerups: # Cancel shrink if big is picked up
                self.active_powerups.discard("SHRUNK"); self.is_shrunk = False; self.shrink_timer = 0.0
            self.calculate_current_sizes()
        elif powerup_type == "SUPER_JUMP":
            self.super_jump_timer = POWERUP_SUPER_JUMP_DURATION; self.jump_power = BASE_JUMP_POWER * POWERUP_SUPER_JUMP_MULTIPLIER
        elif powerup_type == "SPEED_BOOST":
            self.speed_boost_timer = POWERUP_SPEED_BOOST_DURATION; self.player_speed = BASE_PLAYER_SPEED * POWERUP_SPEED_BOOST_MULTIPLIER
        elif powerup_type == "SHRINK_OPPONENT":
            if other_player: other_player.apply_shrink()
        elif powerup_type == "GOAL_SHIELD": # Set global state
             if self.facing_direction == 1: # Assume P1 is left facing
                 global p1_shield_active, p1_shield_timer
                 p1_shield_active = True; p1_shield_timer = POWERUP_GOAL_SHIELD_DURATION; print("P1 Shield Up!")
             else: # P2 is right facing
                 global p2_shield_active, p2_shield_timer
                 p2_shield_active = True; p2_shield_timer = POWERUP_GOAL_SHIELD_DURATION; print("P2 Shield Up!")
             self.active_powerups.discard("GOAL_SHIELD") # Shield is instant global effect, remove from player set
        elif powerup_type == "BALL_FREEZE": # Set global state
             ...
             ball.vx = 0; ball.vy = 0
             self.active_powerups.discard("BALL_FREEZE") # Instant global effect
    def apply_shrink(self):
         self.active_powerups.add("SHRUNK")
         self.shrink_timer = POWERUP_SHRINK_PLAYER_DURATION; self.is_shrunk = True
         if "BIG_PLAYER" in self.active_powerups: # Cancel big if shrink is applied
             self.active_powerups.discard("BIG_PLAYER"); self.is_big = False; self.big_timer = 0.0
         self.calculate_current_sizes()
    ...
    def update(self, dt, other_player):
        ...
        if self.flight_timer > 0:
            ...
            if self.flight_timer <= 0: self.active_powerups.discard("FLIGHT"); self.is_flying = False; print("Flight ended")
        if self.big_timer > 0:
            self.big_timer -= dt
            if self.big_timer <= 0: self.active_powerups.discard("BIG_PLAYER"); self.is_big = False; self.calculate_current_sizes(); print("Big Player ended")
        if self.shrink_timer > 0:
            self.shrink_timer -= dt
            if self.shrink_timer <= 0: self.active_powerups.discard("SHRUNK"); self.is_shrunk = False; self.calculate_current_sizes(); print("Shrink ended")
        if self.super_jump_timer > 0:
            self.super_jump_timer -= dt
            if self.super_jump_timer <= 0: self.active_powerups.discard("SUPER_JUMP"); self.jump_power = BASE_JUMP_POWER; print("Super Jump ended")
        if self.speed_boost_timer > 0:
            self.speed_boost_timer -= dt
            if self.speed_boost_timer <= 0: self.active_powerups.discard("SPEED_BOOST"); self.player_speed = BASE_PLAYER_SPEED; print("Speed Boost ended")
        ...
        if not self.is_kicking:
            ...
            self.x += effective_vx # Apply velocity
...
# --- Ball Class ---
class Ball: # Updated draw for freeze visual
        ...
        if p1_shield_active:
            ...
            if self.x - self.radius < shield_rect.right and self.x + self.radius > shield_rect.left and \
               self.y + self.radius > shield_rect.top and self.y - self.radius < shield_rect.bottom:
                if self.vx < 0:
                    self.x = shield_rect.right + self.radius; self.vx *= -BALL_BOUNCE; hit_shield_this_frame = True
        if p2_shield_active:
            ...
            if self.x + self.radius > shield_rect.left and self.x - self.radius < shield_rect.right and \
               self.y + self.radius > shield_rect.top and self.y - self.radius < shield_rect.bottom:
                if self.vx > 0:
                    self.x = shield_rect.left - self.radius; self.vx *= -BALL_BOUNCE; hit_shield_this_frame = True
    ...
    def draw(self, screen):
        ...
        # Draw freeze shield visual if active or fading
        if self.is_frozen or self.freeze_effect_timer > 0:
            alpha = 220 # <<< More opaque base alpha for frozen shield
            if self.freeze_effect_timer > 0 and not self.is_frozen: # Fade out effect
                alpha = int(220 * (self.freeze_effect_timer / (POWERUP_BALL_FREEZE_DURATION * 0.1))) # Faster fade out

            freeze_color = (173, 216, 230, max(0, min(255, alpha))) # Light blue ice color
            shield_radius = self.radius + 2 # Slightly larger than the ball
            ...
            # Optional: Add a jagged outline?
            # num_spikes = 12
            # for i in range(num_spikes):
            #     angle = (i / num_spikes) * 2 * math.pi
            #     outer_rad = shield_radius + random.uniform(1, 3)
            #     p1 = (shield_radius + shield_radius * math.cos(angle), shield_radius + shield_radius * math.sin(angle))
            #     p2 = (shield_radius + outer_rad * math.cos(angle + 0.1), shield_radius + outer_rad * math.sin(angle + 0.1))
            #     p3 = (shield_radius + shield_radius * math.cos(angle + 0.2), shield_radius + shield_radius * math.sin(angle + 0.2))
            #     pygame.draw.line(temp_surf, (230, 240, 255, alpha), p1, p2)
...
def reset_positions(): # ... (no change) ...
    ...
    ball.x = SCREEN_WIDTH // 2; ball.y = SCREEN_HEIGHT // 3; ball.vx = 0; ball.vy = 0; ball.is_frozen = False; ball.freeze_effect_timer = 0.0
    ...
    active_rockets = []; active_explosions = []
    ball_freeze_timer = 0.0; p1_shield_active = False; p1_shield_timer = 0.0; p2_shield_active = False; p2_shield_timer = 0.0
...
while running:
    ...
    if ball_freeze_timer > 0:
        ...
        if ball_freeze_timer <= 0:
            ...
            ball.freeze_effect_timer = POWERUP_BALL_FREEZE_DURATION * 0.1 # Start fade out (short duration)
    ...
    if match_active:
        ...
        # Update active powerups FIRST, then filter using the return value of update
        active_powerups = [p for p in active_powerups if p.update(dt)] # <<< Use update's return value
        ...
        kick_push_amount = ball.radius * 1.5; kick_push_vx_base = 5 # Base push velocity
        p1_kick_point = player1.get_kick_impact_point()
        if p1_kick_point and p2_rect.collidepoint(p1_kick_point):
            print("P1 kicked P2")
            kick_multiplier = BIG_PLAYER_KICK_MULTIPLIER if player1.is_big else 1.0 # <<< Check if P1 is big
            kick_push_vx = kick_push_vx_base * kick_multiplier # <<< Apply multiplier
            player2.x += kick_push_amount * player1.facing_direction * kick_multiplier # Also push slightly further
            player2.vx += kick_push_vx * player1.facing_direction
            player2.vy -= 3 * kick_multiplier # Add some upward kick too, stronger if big
            player2.is_jumping = True # Make sure they become airborne
        ...
        if p2_kick_point and p1_rect.collidepoint(p2_kick_point):
            print("P2 kicked P1")
            kick_multiplier = BIG_PLAYER_KICK_MULTIPLIER if player2.is_big else 1.0 # <<< Check if P2 is big
            kick_push_vx = kick_push_vx_base * kick_multiplier # <<< Apply multiplier
            player1.x += kick_push_amount * player2.facing_direction * kick_multiplier
            player1.vx += kick_push_vx * player2.facing_direction
            player1.vy -= 3 * kick_multiplier # Add some upward kick too, stronger if big
            player1.is_jumping = True # Make sure they become airborne
            play_sound(loaded_sounds['body_hit'])
    ...
    if match_active and not ball.is_frozen: # Check ball frozen state
        # P1 scores on right goal (if P2 shield NOT active)
        if ball.x + ball.radius >= GOAL_LINE_X_RIGHT and ball.y > GOAL_Y_POS and not p2_shield_active: # <<< Shield check
        ...
        # P2 scores on left goal (if P1 shield NOT active)
        elif ball.x - ball.radius <= GOAL_LINE_X_LEFT and ball.y > GOAL_Y_POS and not p1_shield_active: # <<< Shield check
    ...
    pulse = (math.sin(current_time * POWERUP_GOAL_SHIELD_PULSE_SPEED) + 1) / 2 # 0 to 1 sine wave
    ...
    if p1_shield_active:
        ...
        shield_color = (POWERUP_GOAL_SHIELD_COLOR[0], POWERUP_GOAL_SHIELD_COLOR[1], POWERUP_GOAL_SHIELD_COLOR[2], shield_alpha) # Use pulsing alpha
        shield_surf.fill(shield_color)
        screen.blit(shield_surf, shield_rect.topleft)
        pygame.draw.rect(screen, WHITE, shield_rect, 1) # Add a thin white border
    ...
    ball.draw(screen) # Ball draw now handles freeze visual
    ...
    if match_active:
        ...
        powerup_timers = { "FLIGHT": "flight_timer", "BIG_PLAYER": "big_timer", "SUPER_JUMP": "super_jump_timer", "SPEED_BOOST": "speed_boost_timer", "SHRUNK": "shrink_timer", "GOAL_SHIELD": "p1_shield_timer"} # Need specific timer vars for global ones
        # Player 1 UI
        p1_ui_y = SCREEN_HEIGHT - 30
        for p_type in sorted(list(player1.active_powerups)): # Sort for consistent order
            text = powerup_texts.get(p_type, p_type)
            timer_attr = powerup_timers.get(p_type)
            if timer_attr: text = text.format(getattr(player1, timer_attr, 0.0))
            ...
            p1_ui_y -= line_height
        # Add P1 Shield UI if globally active
        if p1_shield_active:
            text = powerup_texts.get("GOAL_SHIELD", "SHIELD: {:.1f}").format(p1_shield_timer)
            color = powerup_colors.get("GOAL_SHIELD", WHITE)
            surf = powerup_font.render(text, True, color); rect = surf.get_rect(bottomleft=(10, p1_ui_y)); screen.blit(surf, rect)
            p1_ui_y -= line_height

        # Player 2 UI
        p2_ui_y = SCREEN_HEIGHT - 30
        for p_type in sorted(list(player2.active_powerups)): # Sort for consistent order
            text = powerup_texts.get(p_type, p_type)
            timer_attr = powerup_timers.get(p_type)
            if timer_attr: text = text.format(getattr(player2, timer_attr, 0.0))
            color = powerup_colors.get(p_type, WHITE)
            surf = powerup_font.render(text, True, color); rect = surf.get_rect(bottomright=(SCREEN_WIDTH - 10, p2_ui_y)); screen.blit(surf, rect)
            p2_ui_y -= line_height
        # Add P2 Shield UI if globally active
        if p2_shield_active:
            text = powerup_texts.get("GOAL_SHIELD", "SHIELD: {:.1f}").format(p2_shield_timer)
            color = powerup_colors.get("GOAL_SHIELD", WHITE)
            surf = powerup_font.render(text, True, color); rect = surf.get_rect(bottomright=(SCREEN_WIDTH - 10, p2_ui_y)); screen.blit(surf, rect)
            p2_ui_y -= line_height
