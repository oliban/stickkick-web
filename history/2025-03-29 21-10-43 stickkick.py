...
class StickMan: # ... (most methods unchanged) ...
    ...
    def apply_powerup(self, powerup_type, other_player=None): # Logic for stacking/refreshing
        current_val = self.active_powerups.get(powerup_type, 0)
        if powerup_type == "FLIGHT":
            new_timer = POWERUP_FLIGHT_DURATION # Always refresh timer
            ...
            new_timer = POWERUP_BIG_PLAYER_DURATION # Refresh timer
            ...
            new_timer = POWERUP_SUPER_JUMP_DURATION # Refresh timer
            ...
            new_timer = POWERUP_SPEED_BOOST_DURATION # Refresh timer
    ...
    def update(self, dt, other_player):
        ...
        for p_type, value in list(self.active_powerups.items()):
            if p_type in ["FLIGHT", "BIG_PLAYER", "SHRUNK", "SUPER_JUMP", "SPEED_BOOST"]: # Timed powerups
                new_timer = value - dt
                if new_timer <= 0:
                    expired_powerups.append(p_type)
                    # Reset specific effects on expiry
                    ...
                    self.active_powerups[p_type] = new_timer # Update remaining time
            # ROCKET_LAUNCHER handled by fire_rocket
...
def reset_positions(): # Now preserves powerup dicts, resets player state only
    ...
    # Don't reset ball.is_frozen or freeze timer here, let it expire naturally
    ...
    # Keep player1.active_powerups and player2.active_powerups
    # Keep shield timers/active states
    ...
    active_rockets = []; active_explosions = []
def start_new_match(): # Full reset for a new match in the game
    global player1_score, player2_score, match_active, match_winner, match_over_timer, match_end_sound_played, announcement_queue, powerup_spawn_timer, active_powerups, ball_freeze_timer, p1_shield_active, p1_shield_timer, p2_shield_active, p2_shield_timer
    player1_score = 0; player2_score = 0; match_active = True; match_winner = None; match_over_timer = 0.0; match_end_sound_played = False
    announcement_queue = []; reset_positions() # Resets positions, actions, tumble
    # Explicitly clear/reset all powerup states for a new match
    ...
    active_powerups = [] # Clear falling powerups
    ball_freeze_timer = 0.0; ball.is_frozen = False; ball.freeze_effect_timer = 0.0 # Reset ball freeze
    p1_shield_active = False; p1_shield_timer = 0.0 # Reset shields
    p2_shield_active = False; p2_shield_timer = 0.0
    player1.randomize_nose(); player2.randomize_nose()
    powerup_spawn_timer = random.uniform(POWERUP_SPAWN_INTERVAL_MIN, POWERUP_SPAWN_INTERVAL_MAX)
    print("Starting new match.")
def start_new_game(): # Resets game score, then starts new match (which does full reset)
...
while running:
    ...
    if match_active:
        ...
        active_powerups = [p for p in active_powerups if p.update(dt)] # <<< Uses update's return value now
    ...
    if match_active and not ball.is_frozen:
        # P1 scores on right goal (P2's goal) - Check P2 shield
        ...
        # P2 scores on left goal (P1's goal) - Check P1 shield
    ...
    if goal_scored_this_frame:
        ...
        # Reset positions but keep powerups/global states active
        reset_positions()
        continue
    ...
    if match_active:
        cooldown_radius = 5; indicator_offset_y = - player1.head_radius - cooldown_radius - 2 # Adjust based on current radius?
        ...
        # Timed powerups stored directly on player instance now have specific timer names
        ...
        for p_type in sorted(player1.active_powerups.keys()):
            ...
            elif p_type in powerup_timers: text = text_template.format(val) # Use dict value for timer
