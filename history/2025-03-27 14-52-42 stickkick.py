...
# Collision Specific
PLAYER_BODY_BOUNCE = 0.65 # Slightly more bounce off body
PLAYER_VEL_TRANSFER = 0.25 # Slightly more velocity transfer
MIN_BODY_BOUNCE_VEL = 1.5 # Minimum bounce speed to prevent sticking
...
class StickMan:
    def __init__(self, x, y):
        ...
        self.l_foot_pos = (0, 0); self.r_foot_pos = (0, 0); self.body_rect = pygame.Rect(0,0,0,0)
    ...
    def update(self, dt):
        ...
        body_width = self.limb_width * 1.5; self.body_rect.width = body_width; self.body_rect.height = self.hip_pos[1] - self.neck_pos[1]; self.body_rect.centerx = int(self.hip_pos[0]); self.body_rect.top = int(self.neck_pos[1])
...
class Ball:
    ...
    def draw(self, screen):
        ...
        for i in range(num_around):
            ...
            if i % 2 == 0:
                 draw_hexagon(screen, BLACK, shape_center, hex_size, angle + self.rotation_angle * 0.5, width=1)
            else:
                draw_pentagon(screen, BLACK, shape_center, pent_size, angle + self.rotation_angle * 0.5)
...
while running:
    ...
    # Score / Height Tracking Logic (Corrected indentation)
    ...
    if is_ball_airborne:
        ...
        if not ball_was_on_ground: # Just hit ground
            ...
            if final_sequence_score > high_score: # Update high score details
                high_score = final_sequence_score; high_score_max_height = current_max_height; high_score_hit_count = current_hit_count
            # Reset MUST happen after checking high score, but always when hitting ground
    ...
    kick_performed_this_frame = False
    headbutt_performed_this_frame = False
    score_increased_this_frame = False # Initialize here
    ...
    # 2. Headbutt Collision (Corrected Indentation)
    ...
    if dist_head_sq < (ball.radius + head_radius)**2:
        if can_headbutt:
            ...
            if player.vy < 0: # If player is moving up
                force_y -= abs(player.vy) * HEADBUTT_VY_MULTIPLIER # Add boost
            ...
            headbutt_performed_this_frame = True
            if is_ball_airborne:
                current_hit_count += 1
    ...
    # --- 3. Player Body Collision (REVISED LOGIC) ---
    if not kick_performed_this_frame and not headbutt_performed_this_frame:
        ...
        delta_x = ball.x - closest_x
        delta_y = ball.y - closest_y
        dist_sq_body = delta_x**2 + delta_y**2

        if dist_sq_body < ball.radius**2 and dist_sq_body > 0: # Check overlap and avoid division by zero
            distance = math.sqrt(dist_sq_body)
            overlap = ball.radius - distance
            collision_normal_x = delta_x / distance
            collision_normal_y = delta_y / distance

            # Push-out (move slightly more than overlap to prevent sticking)
            push_amount = overlap + 0.1
            ball.x += collision_normal_x * push_amount
            ball.y += collision_normal_y * push_amount

            # Bounce Velocity Calculation (using reflection formula)
            rel_vx = ball.vx - player.vx
            rel_vy = ball.vy - player.vy
            vel_along_normal = rel_vx * collision_normal_x + rel_vy * collision_normal_y

            if vel_along_normal < 0: # Only bounce if moving towards each other
                impulse_scalar = -(1 + PLAYER_BODY_BOUNCE) * vel_along_normal
                bounce_vx = impulse_scalar * collision_normal_x
                bounce_vy = impulse_scalar * collision_normal_y
                bounce_vx += player.vx * PLAYER_VEL_TRANSFER # Add player velocity
                bounce_vy += player.vy * PLAYER_VEL_TRANSFER

                new_vel_mag_sq = bounce_vx**2 + bounce_vy**2
                if new_vel_mag_sq < MIN_BODY_BOUNCE_VEL**2:
                    if new_vel_mag_sq > 0:
                        scale = MIN_BODY_BOUNCE_VEL / math.sqrt(new_vel_mag_sq)
                        bounce_vx *= scale; bounce_vy *= scale
                    else:
                        bounce_vx = collision_normal_x * MIN_BODY_BOUNCE_VEL
                        bounce_vy = collision_normal_y * MIN_BODY_BOUNCE_VEL

                ball.vx = bounce_vx
                ball.vy = bounce_vy

        elif dist_sq_body == 0: # Ball center is exactly on closest point
             ball.y = player_rect.top - ball.radius - 0.1 # Push up
             if ball.vy > 0: ball.vy *= -PLAYER_BODY_BOUNCE # Bounce if moving down
    # --- END BODY COLLISION REVISION ---

    # --- Score Increase Check ---
    if kick_performed_this_frame or headbutt_performed_this_frame:
        score_increased_this_frame = True
