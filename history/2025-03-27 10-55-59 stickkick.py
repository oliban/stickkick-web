...
# --- Get Timestamp ---
# Capture the time when the script is initially run
...
KICK_THIGH_WINDUP_ANGLE = -math.pi / 6.0
KICK_THIGH_FOLLOW_ANGLE = math.pi * 0.6
KICK_SHIN_WINDUP_ANGLE = math.pi * 0.6
KICK_SHIN_IMPACT_ANGLE = 0.0
KICK_SHIN_FOLLOW_ANGLE = math.pi * 0.3
...
# --- Stick Man Class (Ciao - Using exaggerated kick angles) ---
class StickMan:
    def __init__(self, x, y):
        ...
        self.is_charging = False; self.kick_timer = 0; self.kick_duration = 23
    ...
    def update(self, dt):
        ...
        # --- Kick Animation Logic with Exaggerated Angles ---
        if self.is_kicking:
            self.walk_cycle_timer = 0
            self.kick_timer += 1
            progress = min(self.kick_timer / self.kick_duration, 1.0)
            windup_end = 0.25; impact_start = 0.3; impact_end = 0.6; follow_end = 1.0
    ...
    def get_kick_impact_point(self):
        if self.is_kicking:
            impact_start = 0.3; impact_end = 0.65; progress = self.kick_timer / self.kick_duration
...
font_timestamp = pygame.font.Font(None, 20) # Smaller font for timestamp
...
while running:
    ...
    if kick_point:
        ...
        if dist_sq < (ball.radius + effective_kick_rad)**2:
             ...
             if 0.3 < progress < 0.7: # Match get_kick_impact_point timing roughly
    ...
    timestamp_rect = timestamp_surf.get_rect(bottomright=(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 10)) # Position bottom right with padding
