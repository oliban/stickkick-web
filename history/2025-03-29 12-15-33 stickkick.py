...
class StickMan: # Added wings, cap, eyes
    def __init__(self, x, y, facing=1):
        ...
        if facing == 1: self.cap_color = (50, 50, 50) # Dark Grey Cap for P1 (adjust logic if colors change)
        else: self.cap_color = ITALY_RED      # Red Cap for P2
    ...
    def update(self, dt, other_player): # Added wing animation logic
    ...
    def draw(self, screen): # Updated head and wings
        ...
        nose_length = self.head_radius * 0.8 ; nose_width = self.head_radius * 0.5
        ...
        if flight_mode_active:
            ...
            def create_wing_poly(attach_point, angle, length, width):
                ...
                p5 = (attach_point[0] + length*0.2 * cos_a - width*0.2 * cos_w, attach_point[1] + length*0.2 * sin_a - width*0.2 * sin_w) # Curve back in
                return [(int(p[0]), int(p[1])) for p in [p1, p2, p3, p4, p5]] # Use 5 points
            l_attach_upper = (upper_attach_x - 3, upper_attach_y) # Upper wing attachment
            l_attach_lower = (lower_attach_x - 3, lower_attach_y) # Lower wing attachment
            ...
            r_attach_upper = (upper_attach_x + 3, upper_attach_y)
            r_attach_lower = (lower_attach_x + 3, lower_attach_y)
...
while running:
    ...
    # --- Process Announcement Sound Queue (Triggered by event only now) ---
    # Removed call from here
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            if not game_over: # Don't queue match sounds if game just ended
                announcement_queue = []
            ...
            # Check overall game win AFTER updating scores
