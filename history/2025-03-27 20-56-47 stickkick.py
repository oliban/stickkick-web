...
# pygame.mixer.set_num_channels(16) # Optional: Increase if needed
announcement_channel = pygame.mixer.Channel(0) # Use channel 0 for announcements
screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT)); pygame.display.set_caption("Ciao Kick!"); clock = pygame.time.Clock()

# --- Sound Loading --- (Handles missing number files)
def load_sounds(sound_dir="sounds"):
    ...
    sounds['numbers'] = {}
    for i in range(0, 6): # Load 0.wav to 5.wav
        ...
        except pygame.error as e: print(f"Warning: Could not load number sound '{path}': {e}"); sounds['numbers'][i] = None # Store None if missing
...
announcement_queue = [] # Queue for announcement sounds
...
def start_new_match(): # Reset sound flag and queue
    ...
    announcement_queue = [] # Clear queue
    reset_positions()
    print("Starting new match.")
def start_new_game(): # Clear queue
    global p1_games_won, p2_games_won, game_scores, game_over, overall_winner, announcement_queue
...
while running:
    ...
    # --- Handle Match Over State (Plays queued sounds) ---
    if match_over_timer > 0:
        # --- Queue End-of-Match Sounds ONCE ---
        if not match_end_sound_played and not game_over:
            if match_winner == 1: queue_sound(loaded_sounds['nils_wins'])
            elif match_winner == 2: queue_sound(loaded_sounds['harry_wins'])
            # Queue 'ahead' and scores
            ...
            if p1_games > p2_games:
                ...
                queue_specific_sound(loaded_sounds['numbers'].get(p1_games))
                 ...
                 queue_specific_sound(loaded_sounds['numbers'].get(p1_games))
                 queue_specific_sound(loaded_sounds['numbers'].get(p2_games))
            match_end_sound_played = True # Sounds are now queued
    ...
    if announcement_queue and not announcement_channel.get_busy():
        next_sound = announcement_queue.pop(0) # Get the first sound
        if next_sound: # Check if sound object is valid (not None from missing file)
            print(f"Playing announcement: {next_sound}") # Debug print
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            match_end_sound_played = False # <<< Reset flag to allow sounds to play
        ...
        # Don't 'continue' here after goal if match ENDED
