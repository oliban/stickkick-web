    ...
    def update(self, dt):
        ...
        if self.y + self.radius >= GROUND_Y:
            if self.vy >= 0:
                ...
                # Play sound after physics, based on impact velocity
...
loaded_sounds = load_sounds() # Load sounds
...
while running:
    ...
    if match_over_timer > 0:
        # --- Queue End-of-Match Sounds ONCE (if not already done) ---
        if not match_end_sound_played and not game_over:
            announcement_queue = [] # Clear previous just in case
    ...
    # --- Process Announcement Sound Queue (Runs every frame) --- <<< Still needed as backup/trigger
    play_next_announcement()

    # --- Updates (Only if match active) ---
    if match_active:
        # ... (Rest of game logic: player updates, ball updates, collisions, etc.) ...
