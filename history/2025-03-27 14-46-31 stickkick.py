...
class Ball:
    ...
    def draw(self, screen):
        ...
        for i in range(num_around):
            ...
            # Alternate between hexagons and maybe more pentagons
            if i % 2 == 0:
...
while running:
    ...
    if not kick_performed_this_frame and not headbutt_performed_this_frame:
        ...
        if dist_sq_body < ball.radius**2:
            ...
            if distance == 0: collision_normal = (0, -1); overlap = ball.radius + (player_rect.height / 2)
            else: collision_normal = ((ball.x - closest_x) / distance, (ball.y - closest_y) / distance)
            ball.x += collision_normal[0] * overlap; ball.y += collision_normal[1] * overlap
            if abs(collision_normal[0]) > abs(collision_normal[1]): ball.vx *= -PLAYER_BODY_BOUNCE; ball.vx += player.vx * PLAYER_VEL_TRANSFER
            else: ball.vy *= -PLAYER_BODY_BOUNCE; ball.vy += player.vy * PLAYER_VEL_TRANSFER

    # --- Score Increase Check ---
    if kick_performed_this_frame or headbutt_performed_this_frame:
        score_increased_this_frame = True # Set the general flag
