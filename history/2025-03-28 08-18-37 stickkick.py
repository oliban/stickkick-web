...
# --- Get Timestamp ---
GENERATION_TIMESTAMP = datetime.now().strftime("%Y-%m-%d %H:%M:%S") # Keep this here
...
# --- Helper Functions ---
# ... (All helper functions remain the same, including drawing) ...
...
def draw_scoreboard(surface, p1_score, p2_score, p1_games, p2_games, score_font, name_font, game_score_font, is_goal_active): #... (Updated version)
...
class StickMan: #... (as before) ...
    ...
    def jump(self): # ... (as before) ...
        ...
        if not self.is_jumping or was_on_head: play_sound(loaded_sounds['jump']);
...
while running:
    ...
    # --- Handle Match Over State (Queue sounds on goal detect, timer starts next match) ---
    ...
    # --- Process Announcement Sound Queue (Runs every frame as backup/trigger) ---
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            match_end_sound_played = False # Reset flag to allow queuing for this match end
            game_scores.insert(0, (player1_score, player2_score));
            if len(game_scores) > 9: game_scores.pop()

            # Use pending game scores for announcement logic
            pending_p1_games = p1_games_won + (1 if match_winner == 1 else 0)
            pending_p2_games = p2_games_won + (1 if match_winner == 2 else 0)

            # --- Queue Match End Sounds ---
            announcement_queue = [] # Clear previous
            if pending_p1_games > pending_p2_games:
                queue_sound(loaded_sounds['nils_ahead'])
                queue_specific_sound(loaded_sounds['numbers'].get(pending_p1_games))
                queue_specific_sound(loaded_sounds['numbers'].get(pending_p2_games))
            elif pending_p2_games > pending_p1_games:
