...
class StickMan: # Added wing attributes and logic
    def __init__(self, x, y, facing=1):
        ...
        self.wing_flap_magnitude = math.pi * 0.4 # More flap
        self.wing_upper_lobe_size = (30, 22); self.wing_lower_lobe_size = (28, 25)
        # --- Head Feature Colors ---
        self.eye_color = BLACK
        if facing == 1: self.cap_color = (50, 50, 50) # Dark Grey Cap for P1
    ...
    def draw(self, screen): # Updated head and wings
        ...
        if flight_mode_active:
            ...
            l_attach_upper = (upper_attach_x - 4, upper_attach_y)
            l_attach_lower = (lower_attach_x - 4, lower_attach_y)
            ...
            r_attach_upper = (upper_attach_x + 4, upper_attach_y)
            r_attach_lower = (lower_attach_x + 4, lower_attach_y)
...
while running:
    ...
    if game_over:
        if not game_over_sound_played:
            announcement_queue = []; # Clear queue before game over sound
    ...
    # Removed redundant check from main loop
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            if not game_over: # Only queue if game isn't over
                announcement_queue = []
