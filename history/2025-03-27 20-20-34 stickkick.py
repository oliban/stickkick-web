...
BASE_KICK_FORCE_X = 15; BASE_KICK_FORCE_Y = -10 # Normal Kick Y
...
# --- Debug Mode ---
debug_mode = False
DEBUG_BG_COLOR = (128, 0, 128) # Purple
DEBUG_MATCH_POINT_LIMIT = 3
DEBUG_BASE_KICK_FORCE_Y = -2 # Lower kick angle
...
def draw_game_scores(surface, scores_list, font): # Corrected Order
...
def draw_trophy(surface, winner_name, title_font, name_font): # Moved Down, Engraved
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne): # Added debug_mode access
    global current_hit_count, debug_mode
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 kick_y = (DEBUG_BASE_KICK_FORCE_Y if debug_mode else BASE_KICK_FORCE_Y) * KICK_FORCE_LEVEL # <<< Conditional Y force
...
while running:
    ...
    if game_over:
        for event in events:
             ...
             # if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE: start_new_game()
    ...
    if goal_scored_this_frame:
        ...
        current_match_limit = DEBUG_MATCH_POINT_LIMIT if debug_mode else MATCH_POINT_LIMIT # Check debug limit
        ...
        continue # Skip drawing etc. for this frame after goal logic
