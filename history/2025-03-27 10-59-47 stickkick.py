...
# --- Get Timestamp ---
GENERATION_TIMESTAMP = datetime.now().strftime("%Y-%m-%d %H:%M:%S") # Capture time
...
DEBUG_BLUE = (0, 0, 255)
# --- DEBUG ---
DEBUG_KICK_ANGLES = False # Set to True to print angles
...
# --- VIVID KICK ANIMATION CONSTANTS ---
KICK_THIGH_WINDUP_ANGLE = -math.pi / 4.5  # More windup back
KICK_THIGH_FOLLOW_ANGLE = math.pi * 0.7   # *** Really swing thigh forward ***
KICK_SHIN_WINDUP_ANGLE = math.pi * 0.75  # *** Tuck shin sharply ***
KICK_SHIN_IMPACT_ANGLE = -math.pi * 0.05 # *** Extend slightly past straight ***
KICK_SHIN_FOLLOW_ANGLE = math.pi * 0.5   # *** Bend more sharply after follow through ***
...
class StickMan:
    def __init__(self, x, y):
        self.x = x; self.y = y; self.base_y = y; self.width = 20; self.height = 80
        self.vx = 0; self.vy = 0; self.is_jumping = False; self.is_kicking = False
        self.is_charging = False; self.kick_timer = 0; self.kick_duration = 20 # *** Faster kick overall ***
        self.charge_power = MIN_CHARGE_POWER; self.kick_charge_level = MIN_CHARGE_POWER
        self.walk_cycle_timer = 0.0; self.kick_side = 'right'; self.head_radius = 12
        self.torso_length = 36; self.limb_width = 10; self.upper_arm_length = 12
        self.forearm_length = 12; self.thigh_length = 14; self.shin_length = 14
        self.torso_colors = [ITALY_GREEN, ITALY_WHITE, ITALY_RED]
        self.arm_colors = [ITALY_RED, ITALY_GREEN]; self.leg_colors = [ITALY_WHITE, ITALY_RED]
        self.l_upper_arm_angle = 0; self.r_upper_arm_angle = 0; self.l_forearm_angle = 0
        self.r_forearm_angle = 0; self.l_thigh_angle = 0; self.r_thigh_angle = 0
        self.l_shin_angle = 0; self.r_shin_angle = 0; self.head_pos = (0, 0)
        self.neck_pos = (0, 0); self.hip_pos = (0, 0); self.shoulder_pos = (0, 0)
        self.l_elbow_pos = (0, 0); self.r_elbow_pos = (0, 0); self.l_hand_pos = (0, 0)
        self.r_hand_pos = (0, 0); self.l_knee_pos = (0, 0); self.r_knee_pos = (0, 0)
        self.l_foot_pos = (0, 0); self.r_foot_pos = (0, 0)
    # Methods (move, stop_move, etc. - unchanged)
    ...
    def start_charge(self):
        if not self.is_kicking and not self.is_charging: self.is_charging = True; self.charge_power = MIN_CHARGE_POWER
    ...
    def update(self, dt):
        ...
        # --- VIVID KICK ANIMATION LOGIC ---
        if self.is_kicking:
            self.walk_cycle_timer = 0
            self.kick_timer += 1
            progress = min(self.kick_timer / self.kick_duration, 1.0)

            # *** Adjusted Timing Phases for Faster Kick ***
            windup_end = 0.20   # Shorter windup
            impact_start = 0.25 # Start extending shin earlier
            impact_end = 0.50   # *** Faster extension/impact phase ***
            follow_end = 1.0

            # Thigh Angle (Using new constants)
            if progress < windup_end:
                thigh_prog_angle = KICK_THIGH_WINDUP_ANGLE * (progress / windup_end)
            elif progress < impact_end:
                 ...
                 ease_out_factor = 1.0 - follow_progress**1.5
                 thigh_prog_angle = KICK_THIGH_FOLLOW_ANGLE * ease_out_factor

            # Shin Angle (Using new constants, faster extension)
            ...
            # Apply angles (Increased counter-movement)
            if self.kick_side == 'right':
                self.r_thigh_angle = thigh_prog_angle; self.r_shin_angle = shin_prog_angle
                self.l_thigh_angle = -thigh_prog_angle * 0.3; self.l_shin_angle = 0.3 # More reaction
            else:
                self.l_thigh_angle = thigh_prog_angle; self.l_shin_angle = shin_prog_angle
                self.r_thigh_angle = -thigh_prog_angle * 0.3; self.r_shin_angle = 0.3 # More reaction
            # Stronger arm reaction
            self.l_upper_arm_angle = -thigh_prog_angle * 0.15 if self.kick_side == 'right' else thigh_prog_angle * 0.12
            self.r_upper_arm_angle = thigh_prog_angle * 0.12 if self.kick_side == 'right' else -thigh_prog_angle * 0.15
        ...
        # --- END KICK LOGIC ---
        else: # Normal walking/jumping/idle angles (Unchanged)
             if is_walking_on_ground: walk_sin = math.sin(self.walk_cycle_timer); self.l_upper_arm_angle = RUN_UPPER_ARM_SWING * walk_sin; self.r_upper_arm_angle = -RUN_UPPER_ARM_SWING * walk_sin; self.l_forearm_angle = RUN_FOREARM_SWING * math.sin(self.walk_cycle_timer - RUN_FOREARM_OFFSET_FACTOR); self.r_forearm_angle = -RUN_FOREARM_SWING * math.sin(self.walk_cycle_timer - RUN_FOREARM_OFFSET_FACTOR); self.l_thigh_angle = -LEG_THIGH_SWING * walk_sin; self.r_thigh_angle = LEG_THIGH_SWING * walk_sin; shin_bend = LEG_SHIN_BEND_WALK * max(0, math.sin(self.walk_cycle_timer + LEG_SHIN_BEND_SHIFT)); self.l_shin_angle = shin_bend if self.l_thigh_angle < 0 else 0.1; self.r_shin_angle = shin_bend if self.r_thigh_angle < 0 else 0.1
             ...
             elif self.is_charging: charge_crouch = (self.charge_power - MIN_CHARGE_POWER) / (MAX_CHARGE_POWER - MIN_CHARGE_POWER); squat_angle = math.pi * 0.05 * charge_crouch; self.l_thigh_angle = squat_angle; self.r_thigh_angle = squat_angle; self.l_shin_angle = squat_angle * 1.5; self.r_shin_angle = squat_angle * 1.5; self.l_upper_arm_angle = squat_angle; self.r_upper_arm_angle = squat_angle; self.l_forearm_angle = math.pi * 0.1; self.r_forearm_angle = math.pi * 0.1
        ...
        # Calculate Joint Positions (Unchanged)
        current_y = self.y; current_x = self.x; wobble_offset = 0; total_leg_visual_height = self.thigh_length + self.shin_length
        self.hip_pos = (current_x, current_y - total_leg_visual_height); upper_body_x = current_x + wobble_offset
        self.neck_pos = (upper_body_x, self.hip_pos[1] - self.torso_length); self.head_pos = (upper_body_x, self.neck_pos[1] - self.head_radius)
        self.shoulder_pos = self.neck_pos; l_elbow_x = self.shoulder_pos[0] + self.upper_arm_length * math.sin(self.l_upper_arm_angle)
        l_elbow_y = self.shoulder_pos[1] + self.upper_arm_length * math.cos(self.l_upper_arm_angle); self.l_elbow_pos = (l_elbow_x, l_elbow_y)
        l_hand_angle_world = self.l_upper_arm_angle + self.l_forearm_angle; l_hand_x = self.l_elbow_pos[0] + self.forearm_length * math.sin(l_hand_angle_world)
        l_hand_y = self.l_elbow_pos[1] + self.forearm_length * math.cos(l_hand_angle_world); self.l_hand_pos = (l_hand_x, l_hand_y)
        r_elbow_x = self.shoulder_pos[0] + self.upper_arm_length * math.sin(self.r_upper_arm_angle); r_elbow_y = self.shoulder_pos[1] + self.upper_arm_length * math.cos(self.r_upper_arm_angle)
        self.r_elbow_pos = (r_elbow_x, r_elbow_y); r_hand_angle_world = self.r_upper_arm_angle + self.r_forearm_angle
        r_hand_x = self.r_elbow_pos[0] + self.forearm_length * math.sin(r_hand_angle_world); r_hand_y = self.r_elbow_pos[1] + self.forearm_length * math.cos(r_hand_angle_world)
        self.r_hand_pos = (r_hand_x, r_hand_y); l_knee_x = self.hip_pos[0] + self.thigh_length * math.sin(self.l_thigh_angle)
        l_knee_y = self.hip_pos[1] + self.thigh_length * math.cos(self.l_thigh_angle); self.l_knee_pos = (l_knee_x, l_knee_y)
        l_foot_angle_world = self.l_thigh_angle + self.l_shin_angle; l_foot_x = self.l_knee_pos[0] + self.shin_length * math.sin(l_foot_angle_world)
        l_foot_y = self.l_knee_pos[1] + self.shin_length * math.cos(l_foot_angle_world); r_knee_x = self.hip_pos[0] + self.thigh_length * math.sin(self.r_thigh_angle)
        r_knee_y = self.hip_pos[1] + self.thigh_length * math.cos(self.r_thigh_angle); self.r_knee_pos = (r_knee_x, r_knee_y)
        r_foot_angle_world = self.r_thigh_angle + self.r_shin_angle; r_foot_x = self.r_knee_pos[0] + self.shin_length * math.sin(r_foot_angle_world)
        ...
        if self.y >= ground and self.vy >= 0:
             if not self.is_kicking:
                 self.y = ground; self.is_jumping = False; self.vy = 0
                 l_knee_x = self.hip_pos[0] + self.thigh_length * math.sin(self.l_thigh_angle); l_knee_y = self.hip_pos[1] + self.thigh_length * math.cos(self.l_thigh_angle)
                 l_foot_angle_world = self.l_thigh_angle + self.l_shin_angle; l_foot_x = l_knee_x + self.shin_length * math.sin(l_foot_angle_world)
                 r_knee_x = self.hip_pos[0] + self.thigh_length * math.sin(self.r_thigh_angle); r_knee_y = self.hip_pos[1] + self.thigh_length * math.cos(self.r_thigh_angle)
                 r_foot_angle_world = self.r_thigh_angle + self.r_shin_angle; r_foot_x = r_knee_x + self.shin_length * math.sin(r_foot_angle_world)
                 self.l_foot_pos = (l_foot_x, ground); self.r_foot_pos = (r_foot_x, ground)
        else: self.l_foot_pos = (l_foot_x, l_foot_y); self.r_foot_pos = (r_foot_x, r_foot_y)

    # Getters (Adjusted impact window)
    def get_kick_impact_point(self):
        if self.is_kicking:
            impact_start = 0.25 # Align with faster animation start
            impact_end = 0.6   # Align with faster animation end
    ...
    # Draw Method (Unchanged - Ciao)
    def draw(self, screen):
        # ... (Ciao drawing code remains the same) ...
        if self.is_charging: bar_width = 50; bar_height = 8; bar_x = self.head_pos[0] - bar_width / 2; bar_y = self.head_pos[1] - self.head_radius - bar_height - 5; fill_ratio = (self.charge_power - MIN_CHARGE_POWER) / (MAX_CHARGE_POWER - MIN_CHARGE_POWER); fill_width = bar_width * max(0, min(1, fill_ratio)); pygame.draw.rect(screen, BLACK, (bar_x - 1, bar_y - 1, bar_width + 2, bar_height + 2), 1); pygame.draw.rect(screen, YELLOW, (bar_x, bar_y, fill_width, bar_height))
        ...
        def draw_limb_segment(start_pos, end_pos, length, color): center_x = (start_pos[0] + end_pos[0]) / 2; center_y = (start_pos[1] + end_pos[1]) / 2; dx = end_pos[0] - start_pos[0]; dy = end_pos[1] - start_pos[1]; draw_length = math.hypot(dx, dy); angle = math.atan2(dy, dx); if draw_length < 1: draw_length = 1; draw_rotated_rectangle(screen, color, (center_x, center_y), draw_length, self.limb_width, angle)
...
class Ball:
    ...
    def update(self, dt): self.rotation_angle += self.vx * 0.015; self.rotation_angle %= (2 * math.pi); self.vy += GRAVITY; self.vx *= BALL_FRICTION; self.x += self.vx; self.y += self.vy; hit_ground = False; if self.y + self.radius >= GROUND_Y:
        if self.vy >= 0: hit_ground = True; self.y = GROUND_Y - self.radius; self.vy *= -BALL_BOUNCE; self.vx *= 0.9; if abs(self.vy) < 1: self.vy = 0
        ...
        if abs(self.vx) < 0.1 and self.y + self.radius >= GROUND_Y - 1: self.vx = 0; return hit_ground
    def is_on_ground(self): return self.y + self.radius >= GROUND_Y - 0.5
    def draw(self, screen): center_tuple = (int(self.x), int(self.y)); pygame.draw.circle(screen, WHITE, center_tuple, self.radius); pent_size = self.radius * 0.40; hex_size = self.radius * 0.42; dist_factor = 0.65; num_around = 5; angle_step = 2 * math.pi / num_around; draw_pentagon(screen, BLACK, center_tuple, pent_size, self.rotation_angle); for i in range(num_around): angle = self.rotation_angle + (i * angle_step) + angle_step / 2; shape_center_x = center_tuple[0] + self.radius * dist_factor * math.cos(angle); shape_center_y = center_tuple[1] + self.radius * dist_factor * math.sin(angle); shape_center = (shape_center_x, shape_center_y); if i % 2 == 0: draw_hexagon(screen, BLACK, shape_center, hex_size, angle + self.rotation_angle * 0.5, width=1); else: draw_pentagon(screen, BLACK, shape_center, pent_size, angle + self.rotation_angle * 0.5); pygame.draw.circle(screen, BLACK, center_tuple, self.radius, 1)
...
high_score_max_height = 0.0; high_score_hit_count = 0
ball_was_on_ground = True; particles = []; can_headbutt = True

# --- Off-screen Arrow Function (Unchanged) ---
def draw_offscreen_arrow(s, ball, p_pos): ar_sz = 15; pad = 25; is_off = False; tx, ty = ball.x, ball.y; ax = max(pad, min(ball.x, SCREEN_WIDTH - pad)); ay = max(pad, min(ball.y, SCREEN_HEIGHT - pad)); if ball.x < 0 or ball.x > SCREEN_WIDTH: ax = pad if ball.x < 0 else SCREEN_WIDTH - pad; is_off = True; if ball.y < 0 or ball.y > SCREEN_HEIGHT: ay = pad if ball.y < 0 else SCREEN_HEIGHT - pad; is_off = True; if not is_off: return; ang = math.atan2(ty - ay, tx - ax); p1 = (ar_sz, 0); p2 = (-ar_sz / 2, -ar_sz / 2); p3 = (-ar_sz / 2, ar_sz / 2); cos_a, sin_a = math.cos(ang), math.sin(ang); p1r = (p1[0] * cos_a - p1[1] * sin_a, p1[0] * sin_a + p1[1] * cos_a); p2r = (p2[0] * cos_a - p2[1] * sin_a, p2[0] * sin_a + p2[1] * cos_a); p3r = (p3[0] * cos_a - p3[1] * sin_a, p3[0] * sin_a + p3[1] * cos_a); pts = [(ax + p1r[0], ay + p1r[1]), (ax + p2r[0], ay + p2r[1]), (ax + p3r[0], ay + p3r[1])]; pygame.draw.polygon(s, ARROW_RED, [(int(p[0]), int(p[1])) for p in pts])
...
while running:
    ...
    if is_ball_airborne: current_height_pixels = max(0, GROUND_Y - (ball.y + ball.radius)); current_max_height = max(current_max_height, current_height_pixels); ball_was_on_ground = False
    elif ball_hit_ground_this_frame:
        if not ball_was_on_ground: final_sequence_score = int(current_max_height * current_hit_count);
            if final_sequence_score > high_score: high_score = final_sequence_score; high_score_max_height = current_max_height; high_score_hit_count = current_hit_count; current_max_height = 0.0; current_hit_count = 0
        ball_was_on_ground = True
    display_score = int(current_max_height * current_hit_count)

    # Collision & Score (Using adjusted impact window)
    score_increased_this_frame = False
    kick_point = player.get_kick_impact_point() # Uses updated impact window: 0.25 -> 0.6
    if kick_point:
        ...
        if dist_sq < (ball.radius + effective_kick_rad)**2:
             ...
             # *** Adjusted collision check timing to better match animation ***
             if 0.25 < progress < 0.6: # Check if foot is forward during this part
                 kick_x_base = BASE_KICK_FORCE_X
                 if player.kick_side == "left": kick_x_base = -kick_x_base
                 kick_x = kick_x_base * player.kick_charge_level; kick_y = BASE_KICK_FORCE_Y * player.kick_charge_level
    ...
    if dist_head_sq < (ball.radius + head_radius)**2:
        if can_headbutt: force_y = -HEADBUTT_UP_FORCE;
            if player.vy < 0: force_y -= abs(player.vy) * HEADBUTT_VY_MULTIPLIER; force_x = player.vx * HEADBUTT_PLAYER_VX_FACTOR - dist_x_head * HEADBUTT_POS_X_FACTOR; ball.apply_force(force_x, force_y); can_headbutt = False; headbutt_cooldown_applied = True
            if is_ball_airborne: current_hit_count += 1; score_increased_this_frame = True
    ...
    if score_increased_this_frame and kick_point and current_hit_count > 0 and current_hit_count % 5 == 0:
        ...
        for _ in range(num_kick_particles): particle_x = kick_point[0] + random.uniform(-5, 5); particle_y = kick_point[1] + random.uniform(-5, 5); particles.append(Particle(particle_x, particle_y))
    ...
    # Cooldown
    if not can_headbutt: cooldown_color = (255, 0, 0, 180); cooldown_radius = 5; head_x, head_y = player.head_pos; indicator_x = int(head_x); indicator_y = int(head_y - player.head_radius - cooldown_radius - 2); temp_surf = pygame.Surface((cooldown_radius*2, cooldown_radius*2), pygame.SRCALPHA); pygame.draw.circle(temp_surf, cooldown_color, (cooldown_radius, cooldown_radius), cooldown_radius); screen.blit(temp_surf, (indicator_x - cooldown_radius, indicator_y - cooldown_radius))
    # Timestamp
