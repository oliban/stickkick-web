...
# --- Helper Functions ---
# ... (draw polygon, pentagon, hexagon, normalize, draw_rotated_rectangle, draw_goal_isometric unchanged) ...
...
def draw_trophy(surface, winner_name, title_font, name_font): # Changed title/rematch positions
    ...
    title_rect = title_surf.get_rect(center=(trophy_center_x, trophy_base_y - 220 - 200)) # <<< Moved up 200px
    ...
    # Draw Rematch Text - Moved to Top
...
while running:
    ...
    if match_active:
        ...
        active_powerups = [p for p in active_powerups if p.update(dt)] # Uses update's return value
        ...
        for i, pup in enumerate(active_powerups):
            ...
            if not collected_this_pup and not player1.is_tumbling:
                ...
                if collected_type_p1:
                    ...
                    # Trigger immediate spawn ONLY for BALL_FREEZE
                    if collected_type_p1 == "BALL_FREEZE":
                         new_powerup = ParachutePowerup(); new_powerup.spawn(); active_powerups.append(new_powerup)
                         powerup_spawn_timer = random.uniform(POWERUP_SPAWN_INTERVAL_MIN, POWERUP_SPAWN_INTERVAL_MAX) # Reset normal timer too
