...
GOAL_Y_POS = GROUND_Y - GOAL_HEIGHT # Y-coordinate of the crossbar
...
GOAL_LINE_X_LEFT = GOAL_POST_THICKNESS // 2
...
# --- New Helper: Draw Isometric Goal ---
def draw_goal_isometric(surface, goal_line_x, goal_y, goal_height, depth_x, depth_y, thickness, post_color, net_color):
    ...
    # Draw Back Structure First (lines)
    ...
    pygame.draw.line(surface, post_color, back_bottom, front_bottom, thickness) # Bottom Side Bar

    # Draw Front Structure (lines)
    ...
    # Draw Simple Net Lines (diagonal lines for side/back)
    pygame.draw.line(surface, net_color, front_top, back_bottom, 1)
    pygame.draw.line(surface, net_color, front_bottom, back_top, 1)
    # Add a few vertical lines on the back plane for effect
    num_net_lines_back = 4
    for i in range(1, num_net_lines_back):
        lerp_factor = i / num_net_lines_back
        y_pos = back_top_y + (back_bottom_y - back_top_y) * lerp_factor
        pygame.draw.line(surface, net_color, (back_x, back_top_y), (back_x, y_pos), 1) # From top down
        pygame.draw.line(surface, net_color, (back_x, y_pos), (back_bottom[0], back_bottom[1]), 1) # From mid to bottom
...
while running:
    ...
    # --- Goal Detection ---
    goal_scored = False
    if ball.x + ball.radius >= GOAL_LINE_X_RIGHT and ball.y > GOAL_Y_POS: player_score += 1; goal_message_timer = GOAL_MESSAGE_DURATION; goal_scored = True; print(f"GOAL! Player Score: {player_score}")
    elif ball.x - ball.radius <= GOAL_LINE_X_LEFT and ball.y > GOAL_Y_POS: opponent_score += 1; goal_message_timer = GOAL_MESSAGE_DURATION; goal_scored = True; print(f"GOAL! Opponent Score: {opponent_score}")
    ...
    if not kick_performed_this_frame and not headbutt_performed_this_frame and player_body_collision_timer == 0:
        ...
        if dist_sq_body < ball.radius**2 and dist_sq_body > 0:
            ...
            if vel_along_normal < 0:
             ...
             if ball.vy > 0: # Bounce if moving down
                 ball.vy *= -PLAYER_BODY_BOUNCE
                 player_body_collision_timer = PLAYER_BODY_COLLISION_FRAMES # Set cooldown here too
    # --- END BODY COLLISION ---
