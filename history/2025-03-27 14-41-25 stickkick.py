...
# Collision Specific
PLAYER_BODY_BOUNCE = 0.6 # How much ball bounces off player body
PLAYER_VEL_TRANSFER = 0.2 # How much player velocity affects ball bounce
...
KICK_THIGH_WINDUP_ANGLE = -math.pi / 4.5
KICK_THIGH_FOLLOW_ANGLE = math.pi * 0.7
KICK_SHIN_WINDUP_ANGLE = math.pi * 0.75
KICK_SHIN_IMPACT_ANGLE = -math.pi * 0.05
...
class StickMan:
    def __init__(self, x, y):
        ...
        self.is_charging = False; self.kick_timer = 0; self.kick_duration = 20
        ...
        # Add a rect for body collision
        self.body_rect = pygame.Rect(0,0,0,0) # Initialize, will be updated
    ...
    def update(self, dt):
        ...
        if self.is_kicking:
            ...
            if self.kick_timer >= self.kick_duration: self.is_kicking = False; self.kick_timer = 0; self.r_thigh_angle = 0; self.l_thigh_angle = 0; self.r_shin_angle = 0; self.l_shin_angle = 0; self.l_upper_arm_angle = 0; self.r_upper_arm_angle = 0; self.l_forearm_angle = 0; self.r_forearm_angle = 0; self.kick_charge_level = MIN_CHARGE_POWER
        # --- END KICK LOGIC ---
        else: # Normal walking/jumping/idle angles
        ...
        # --- Update Body Collision Rect ---
        # Simple vertical rect from neck to hip, centered horizontally
        body_width = self.limb_width * 1.5 # Slightly wider than limbs
        self.body_rect.width = body_width
        self.body_rect.height = self.hip_pos[1] - self.neck_pos[1]
        self.body_rect.centerx = int(self.hip_pos[0]) # Use hip X as center
    ...
    def draw(self, screen):
        ...
        # Optional: Draw the body rect for debugging
        # pygame.draw.rect(screen, (255, 0, 255), self.body_rect, 1)


# --- Ball Class (Corrected formatting) ---
class Ball:
...
# --- Off-screen Arrow Function (Corrected formatting) ---
...
while running:
    ...
    # Updates
    player.update(dt) # Player position/state updated first
    ball_hit_ground_this_frame = ball.update(dt) # Then ball physics
    particles = [p for p in particles if p.update(dt)] # Then particles
    ...
    if is_ball_airborne: current_height_pixels = max(0, GROUND_Y - (ball.y + ball.radius)); current_max_height = max(current_max_height, current_height_pixels); ball_was_on_ground = False
        ...
        if not ball_was_on_ground: final_sequence_score = int(current_max_height * current_hit_count);
            ...
            current_max_height = 0.0; current_hit_count = 0 # Reset session
        ball_was_on_ground = True
    display_score = int(current_max_height * current_hit_count)

    # --- Collision Detection & Resolution ---
    kick_performed_this_frame = False # Flag to prevent body collision if kick connects
    headbutt_performed_this_frame = False # Flag for headbutt

    # 1. Kick Collision (Applies force)
    ...
    if kick_point:
        ...
        if dist_sq < (ball.radius + effective_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 kick_performed_this_frame = True # Set flag
                 if is_ball_airborne: current_hit_count += 1
                 # Note: score_increased_this_frame is set below for *any* hit

    # 2. Headbutt Collision (Applies force)
    ...
    headbutt_cooldown_applied = False # Renamed to avoid conflict
    if dist_head_sq < (ball.radius + head_radius)**2:
        if can_headbutt:
            ...
            can_headbutt = False; headbutt_cooldown_applied = True # Use local flag for cooldown logic
            headbutt_performed_this_frame = True # Set flag
            if is_ball_airborne: current_hit_count += 1
    # Headbutt cooldown reset logic
    if not headbutt_cooldown_applied:
        if dist_head_sq > (ball.radius + head_radius + 15) ** 2:
            can_headbutt = True

    # --- 3. Player Body Collision (Bounce/Push-out) ---
    # Only check body collision if kick or headbutt didn't *just* happen
    if not kick_performed_this_frame and not headbutt_performed_this_frame:
        player_rect = player.get_body_rect()
        # Find closest point on player rect to ball center
        closest_x = max(player_rect.left, min(ball.x, player_rect.right))
        closest_y = max(player_rect.top, min(ball.y, player_rect.bottom))
        # Check distance
        dist_sq_body = (ball.x - closest_x)**2 + (ball.y - closest_y)**2
        if dist_sq_body < ball.radius**2:
            # Collision detected!
            distance = math.sqrt(dist_sq_body)
            overlap = ball.radius - distance
            if distance == 0: # Avoid division by zero if center is inside
                 collision_normal = (0, -1) # Default push upwards
                 overlap = ball.radius + (player_rect.height / 2) # Approximate overlap
            else:
                 collision_normal = ((ball.x - closest_x) / distance, (ball.y - closest_y) / distance)

            # Push-out based on normal
            ball.x += collision_normal[0] * overlap
            ball.y += collision_normal[1] * overlap

            # Bounce - Reflect velocity along the normal (simplified)
            # Determine if it's more horizontal or vertical collision
            if abs(collision_normal[0]) > abs(collision_normal[1]): # More horizontal collision
                 ball.vx *= -PLAYER_BODY_BOUNCE
                 # Add some player velocity
                 ball.vx += player.vx * PLAYER_VEL_TRANSFER
                 # Dampen vertical slightly if hitting side? Optional.
                 # ball.vy *= 0.95
            else: # More vertical collision
                 ball.vy *= -PLAYER_BODY_BOUNCE
                 # Add some player velocity
                 ball.vy += player.vy * PLAYER_VEL_TRANSFER
                 # Dampen horizontal slightly if hitting top/bottom? Optional.
                 # ball.vx *= 0.95

            # Prevent ball getting stuck inside player after bounce
            # Re-check closest point after push-out and velocity change, ensure separation
            # (This can get complex, simple push-out is often enough)


    # --- Score Increase Check (Applies to Kick OR Headbutt) ---
    if kick_performed_this_frame or headbutt_performed_this_frame:
        score_increased_this_frame = True # Set the general flag if any hit occured
