...
class StickMan:
    def __init__(self, x, y, facing=1):
        ...
        self.kick_timer = 0; self.kick_duration = 20; # Kick duration remains
    ...
    def jump(self):
        if not self.is_jumping:
            ...
            # No need to check is_charging

    # Removed start_charge and release_charge_and_kick

    # Modified start_kick
    def start_kick(self, ball_x): # Takes ball_x now
        if not self.is_kicking:
            self.is_kicking = True
            self.kick_timer = 0
            self.vx = 0 # Stop moving when kicking
            # Determine kick side based on ball position at the moment kick is initiated
            self.kick_side = 'left' if ball_x < self.x else 'right'
            # Optionally force facing direction?
            # self.facing_direction = 1 if self.kick_side == 'right' else -1

    def update(self, dt):
        time_ms = pygame.time.get_ticks()
        # Removed charging logic check
        ...
        if self.is_kicking:
            ...
            if self.kick_timer >= self.kick_duration: self.is_kicking = False; self.kick_timer = 0; self.r_thigh_angle = 0; self.l_thigh_angle = 0; self.r_shin_angle = 0; self.l_shin_angle = 0; self.l_upper_arm_angle = 0; self.r_upper_arm_angle = 0; self.l_forearm_angle = 0; self.r_forearm_angle = 0 # Removed kick_charge_level reset
        else:
    ...
    # Draw Method (Removed Charge Bar)
    def draw(self, screen):
        # Removed charge bar drawing
...
# Combo score variables removed for simplicity in 2P mode
...
def reset_after_goal():
    global ball_was_on_ground # Only need this global now
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne):
    kick_performed = False; headbutt_performed = False; score_increase = False; kick_pt = None # kick_pt needed for particles
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 kick_x = kick_x_base * KICK_FORCE_LEVEL # <<< Use fixed force level
                 kick_y = BASE_KICK_FORCE_Y * KICK_FORCE_LEVEL # <<< Use fixed force level
                 if player.vy < 0: kick_y += player.vy * 0.3 # Volley boost still applies
                 ball.apply_force(kick_x, kick_y, hitter=player); kick_performed = True; kick_pt = local_kick_point
                 if is_ball_airborne: score_increase = True # Used for particles only now
    ...
    if dist_head_sq < (ball.radius + head_radius)**2:
        if can_headbutt:
            ...
            if is_ball_airborne: score_increase = True
...
while running:
    ...
    # Event Handling (Modified for Kick Triggers)
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_DOWN: player1.start_kick(ball.x) # <<< P1 Kick Trigger
            # Player 2
            elif event.key == pygame.K_a: player2.move(-1)
            elif event.key == pygame.K_d: player2.move(1)
            elif event.key == pygame.K_w: player2.jump()
            elif event.key == pygame.K_s: player2.start_kick(ball.x) # <<< P2 Kick Trigger
            # General
    ...
    # Combo Score Logic (Simplified)
    is_ball_airborne = not ball.is_on_ground()
    # Reset hit count immediately when ball hits ground after being airborne
    ...
    # The helper now increments current_hit_count internally
    ...
    score_increased_this_frame = p1_hit or p2_hit # If either player made contact that counts
    last_kick_point = p1_kick_pt if p1_kick_pt else p2_kick_pt # For particle effect origin
    ...
    player1.draw(screen); player2.draw(screen); ball.draw(screen); draw_offscreen_arrow(screen, ball, None) # p_pos not needed
    # Scores
