...
POWERUP_SPAWN_INTERVAL_MIN = 20.0 # Adjusted Min seconds
POWERUP_SPAWN_INTERVAL_MAX = 45.0 # Adjusted Max seconds
POWERUP_SPAWN_CHANCE = 0.6     # Increased chance slightly
...
POWERUP_FLIGHT_DURATION = 15.0 # Reduced duration

# --- Rocket Launcher Constants ---
ROCKET_SPEED = 450
ROCKET_SIZE = (18, 6)
ROCKET_COLOR = (100, 100, 110)
ROCKET_FUSE_TIME = 3.0 # seconds
ROCKET_BLAST_RADIUS_FACTOR = 2.5 # Multiplier of ball radius
ROCKET_EXPLOSION_DURATION = 0.4 # seconds
ROCKET_EXPLOSION_FORCE = 60.0
ROCKET_EXPLOSION_COLOR = (255, 100, 0, 180) # Semi-transparent orange
GUN_COLOR = (70, 80, 90)
GUN_SIZE = (25, 8)
GUN_ANIM_SPEED = 4.0
GUN_ANIM_MAGNITUDE = 0.25 # Radians
...
class ParachutePowerup: # Added powerup_type
    def __init__(self):
        ...
        self.powerup_type = None # <<< Stores the type of powerup inside
    def spawn(self):
        self.active = True
        self.powerup_type = random.choice(POWERUP_TYPES) # <<< Randomly select type
        ...
        print(f"Powerup spawned: {self.powerup_type} at ({self.x:.0f}, {self.y:.0f}) with drift {self.vx:.1f}")
    def update(self, dt):
    ...
    def check_collision(self, player): # Returns the type of powerup on collision
        if not self.active: return None
        player_rect = player.get_body_rect(); powerup_box_rect = self.get_box_rect()
        if player_rect.colliderect(powerup_box_rect):
            collected_type = self.powerup_type
            self.active = False; print(f"Powerup collected: {collected_type}")
            play_sound(loaded_sounds['combo']) # Play pickup sound
            return collected_type # <<< Return the type
        return None
    def draw(self, screen): #... (visuals unchanged) ...
        if not self.active: return
        ...
        for i in range(num_panels):
            ...
            except ValueError: pass # Avoid potential errors with tiny polygons
...
class Rocket: # <<< NEW CLASS
    def __init__(self, x, y, vx, vy, owner_player):
        self.x = x; self.y = y; self.vx = vx; self.vy = vy
        self.width, self.height = ROCKET_SIZE
        self.owner = owner_player # Reference to the player who fired it
        self.active = True
        self.fuse_timer = ROCKET_FUSE_TIME
        self.angle = math.atan2(vy, vx)
    def update(self, dt, players, ball):
        if not self.active: return False # Return value indicates if explosion happened
        self.x += self.vx * dt; self.y += self.vy * dt
        self.fuse_timer -= dt
        self.angle = math.atan2(self.vy, self.vx) # Update angle based on velocity

        # Collision checks
        exploded = False
        # Screen bounds
        if self.x < 0 or self.x > SCREEN_WIDTH or self.y < 0 or self.y > SCREEN_HEIGHT: exploded = True
        # Ground
        if self.y + self.height / 2 > GROUND_Y: self.y = GROUND_Y - self.height / 2; exploded = True
        # Fuse
        if self.fuse_timer <= 0: exploded = True
        # Players (excluding owner) & Ball
        rocket_rect = self.get_rect()
        for p in players:
            if p != self.owner and p.get_body_rect().colliderect(rocket_rect): exploded = True; break
        if not exploded and pygame.Rect(ball.x - ball.radius, ball.y - ball.radius, ball.radius * 2, ball.radius * 2).colliderect(rocket_rect):
            exploded = True
    ...
    def draw(self, screen):
        ...
        # Simple flame effect
        flame_length = random.uniform(5, 10)
        flame_angle = self.angle + math.pi # Opposite direction
        flame_x = self.x + math.cos(flame_angle) * (self.width / 2 + 2)
        flame_y = self.y + math.sin(flame_angle) * (self.width / 2 + 2)
        end_flame_x = flame_x + math.cos(flame_angle) * flame_length
        end_flame_y = flame_y + math.sin(flame_angle) * flame_length
        pygame.draw.line(screen, random.choice([YELLOW, STAR_ORANGE]), (int(flame_x), int(flame_y)), (int(end_flame_x), int(end_flame_y)), 3)

class Explosion: # <<< NEW CLASS
    def __init__(self, x, y, max_radius):
        self.x = x; self.y = y; self.max_radius = max_radius
        self.duration = ROCKET_EXPLOSION_DURATION
    ...
    def update(self, dt):
        ...
        # Calculate radius (expand quickly, shrink slower - or just expand)
        progress = 1.0 - (self.timer / self.duration) # 0 to 1
        # Simple linear expansion:
        self.current_radius = self.max_radius * progress
        # Sine expansion/contraction (looks better):
...
def create_explosion(x, y, radius, players, ball): # Handles force application and adds to list
    global active_explosions
    print(f"Explosion created at ({x:.0f}, {y:.0f}) with radius {radius:.0f}")
    play_sound(loaded_sounds['wall_hit']) # Use wall hit or add specific explosion sound
    ...
    for p in players:
        ...
        if dist_sq < radius**2 and dist_sq > 0:
            ...
            force_magnitude = ROCKET_EXPLOSION_FORCE * (1.0 - (dist / radius)) # Stronger closer
            push_vec_x = (p.x - x) / dist
            push_vec_y = (p.y - y) / dist # Push slightly upwards too
            p.vx += push_vec_x * force_magnitude
            p.vy += push_vec_y * force_magnitude * 0.8 - 5 # Add some upward boost

    # Apply force to ball
    dist_sq = (ball.x - x)**2 + (ball.y - y)**2
    if dist_sq < radius**2 and dist_sq > 0:
        dist = math.sqrt(dist_sq)
        force_magnitude = ROCKET_EXPLOSION_FORCE * (1.0 - (dist / radius))
        push_vec_x = (ball.x - x) / dist
        push_vec_y = (ball.y - y) / dist
        ball.apply_force(push_vec_x * force_magnitude, push_vec_y * force_magnitude - 5, hitter='explosion') # Add upward boost

    # Create visual explosion object
    active_explosions.append(Explosion(x, y, radius))

class StickMan: # Added powerup state, gun logic
    def __init__(self, x, y, facing=1):
        ...
        # --- Powerup Attributes ---
        self.current_powerup = None # Can be "FLIGHT" or "ROCKET_LAUNCHER" or None
        self.is_flying = False; self.flight_timer = 0.0
        self.gun_anim_timer = random.uniform(0, 2 * math.pi) # Random start phase
        self.gun_angle_offset = 0.0
    ...
    def jump(self):
        can_jump_now = False
        if self.current_powerup == "FLIGHT": # Check specific powerup
    ...
    def start_kick(self): # Modified to handle firing rocket
    ...
    def fire_rocket(self):
        ...
        play_sound(loaded_sounds['kick']) # Use kick or add rocket fire sound

        # Calculate start position and velocity based on gun position/angle
        gun_angle = self.gun_angle_offset # Base angle is horizontal
        start_x = self.shoulder_pos[0] + (GUN_SIZE[0] / 2 + 5) * self.facing_direction
        start_y = self.shoulder_pos[1] + math.sin(gun_angle) * 10 # Approx gun height

        rocket_vx = ROCKET_SPEED * self.facing_direction
        rocket_vy = ROCKET_SPEED * math.sin(gun_angle) * 0.3 # Slight vertical based on gun angle

        active_rockets.append(Rocket(start_x, start_y, rocket_vx, rocket_vy, self))
        self.current_powerup = None # Rocket is used up
    ...
    def update(self, dt, other_player): # Added gun animation timer
        # --- Powerup Timer Countdown ---
        if self.current_powerup == "FLIGHT":
            self.flight_timer -= dt
            if self.flight_timer <= 0:
                self.current_powerup = None
                self.is_flying = False; self.flight_timer = 0; print("Flight ended")
        else:
            self.is_flying = False # Ensure not flying if powerup isn't flight

        # --- Gun Animation Timer ---
        if self.current_powerup == "ROCKET_LAUNCHER":
            self.gun_anim_timer += dt * GUN_ANIM_SPEED
            self.gun_angle_offset = math.sin(self.gun_anim_timer) * GUN_ANIM_MAGNITUDE * self.facing_direction

        # --- Physics ---
        # ... (rest of physics logic remains the same) ...
        ...
        if self.current_powerup == "FLIGHT": # Check powerup type
        ...
        # --- Body Animation ---
        # ... (kick, walk, jump animation logic remains the same) ...
        ...
        # ... (joint calculation logic remains the same) ...
        ...
        # --- Update Body Rect ---
        # ... (body rect update remains the same) ...
    ...
    # Draw Method
    def draw(self, screen): # Added gun drawing
        ...
        nose_tip_y = head_center_y + self.head_radius * 0.1
        nose_base_x = head_center_x + (self.head_radius * 0.3) * self.facing_direction
        ...
        # --- Draw Wings (Only if flying) ---
        if self.current_powerup == "FLIGHT": # Check powerup type
             # ... (wing drawing logic unchanged) ...
            upper_attach_x = self.shoulder_pos[0]; upper_attach_y = self.shoulder_pos[1] + 5
            lower_attach_x = self.hip_pos[0]; lower_attach_y = self.hip_pos[1] - 5
        ...
        # --- Draw Gun (Only if holding rocket launcher) ---
        if self.current_powerup == "ROCKET_LAUNCHER":
            gun_attach_x = self.shoulder_pos[0] + 5 * self.facing_direction
            gun_attach_y = self.shoulder_pos[1] + 10
            gun_angle = self.gun_angle_offset # Base angle is 0 (horizontal) + animation
            gun_center_x = gun_attach_x + (GUN_SIZE[0] / 2) * math.cos(gun_angle) * self.facing_direction
            gun_center_y = gun_attach_y + (GUN_SIZE[0] / 2) * math.sin(gun_angle)
            # Determine actual angle for draw_rotated_rectangle based on facing direction
            draw_angle = gun_angle if self.facing_direction == 1 else gun_angle + math.pi
            draw_rotated_rectangle(screen, GUN_COLOR, (gun_center_x, gun_center_y), GUN_SIZE[0], GUN_SIZE[1], draw_angle)
...
# --- Sound Loading ---
# ... (load_sounds definition as before, maybe add rocket sounds later) ...
...
def play_next_announcement(): # ... (no change) ...
    ...
    if announcement_queue and not announcement_channel.get_busy():
        ...
        else: pygame.event.post(pygame.event.Event(SOUND_FINISHED_EVENT)) # Ensure queue continues if sound is None
...
player_list = [player1, player2] # Handy list for iteration
...
active_rockets = [] # <<< List for active rockets
active_explosions = [] # <<< List for active explosions
...
def reset_positions():
    ...
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0; player1.is_kicking = False; player1.on_other_player_head = False; player1.facing_direction = 1; player1.is_jumping = False; player1.current_powerup = None; player1.is_flying = False; player1.flight_timer = 0.0 # Reset powerup
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0; player2.is_kicking = False; player2.on_other_player_head = False; player2.facing_direction = -1; player2.is_jumping = False; player2.current_powerup = None; player2.is_flying = False; player2.flight_timer = 0.0 # Reset powerup
    ...
    powerup.active = False # Deactivate spawned powerup
    active_rockets = [] # Clear projectiles
    active_explosions = [] # Clear explosions
...
def start_new_match(): #... (no change) ...
    ...
    player1.randomize_nose(); player2.randomize_nose(); print("Starting new match.")
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne): #... (no change) ...
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             if player.kick_duration <= 0: progress = 1.0 # Avoid division by zero if kick is instant
    ...
    if not kick_performed and not headbutt_performed and body_collision_timer == 0:
        ...
        if dist_sq_body < ball.radius**2:
             ...
             elif dist_sq_body == 0: # Inside rectangle exactly
...
while running:
    ...
    # ... (event loop mostly unchanged, handles quit, debug, flight toggle, game over restart, player movement keys) ...
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # if event.key == pygame.K_6: flight_mode_active = not flight_mode_active; print(f"Flight Mode {'ACT' if flight_mode_active else 'DEACT'}IVATED") # Removed flight toggle key
            if game_over and event.key == pygame.K_r: start_new_game()
            if match_active: # Only process game keys if match is active
                if event.key == pygame.K_a: player1.move(-1)
                elif event.key == pygame.K_d: player1.move(1)
                elif event.key == pygame.K_w: player1.jump()
                elif event.key == pygame.K_s: player1.start_kick() # Kick / Fire Rocket
                ...
                elif event.key == pygame.K_DOWN: player2.start_kick() # Kick / Fire Rocket
    ...
    if game_over:
        # ... (game over logic and drawing unchanged) ...
        ...
        bg_color = DEBUG_BG_COLOR if debug_mode else SKY_BLUE # No flight bg color
    ...
    # --- Power-up Spawning ---
    if not powerup.active and match_active:
        powerup_spawn_timer -= dt
        if powerup_spawn_timer <= 0:
            if random.random() < POWERUP_SPAWN_CHANCE: powerup.spawn()
    ...
    if match_active:
        ...
        # Update Players, Ball, Powerup
        player1.update(dt, player2); player2.update(dt, player1)
        ball_hit_ground_this_frame = ball.update(dt)
        powerup.update(dt)

        # Update Particles, Rockets, Explosions
        particles = [p for p in particles if p.update(dt)]
        rockets_exploded_this_frame = []
        for r in active_rockets:
            if r.update(dt, player_list, ball): # Pass players and ball for collision
                rockets_exploded_this_frame.append(r)
        active_rockets = [r for r in active_rockets if r.active] # Remove exploded/inactive
        active_explosions = [e for e in active_explosions if e.update(dt)] # Update and remove finished explosions


        # --- Power-up Collection ---
        if powerup.active:
            collected_type_p1 = powerup.check_collision(player1)
            ...
            else: # Only check P2 if P1 didn't collect
                collected_type_p2 = powerup.check_collision(player2)
        ...
        # --- Player Collisions ---
        # ... (Player-Player body and kick collision logic unchanged) ...
    ...
    if match_active:
        # ... (goal detection logic unchanged) ...
    ...
    if goal_scored_this_frame:
        # ... (goal sound, particle, match/game win checks, announcement queuing unchanged) ...
        ...
        # Reset positions AFTER checking for winner and queuing sounds
        reset_positions()
        # Skip rest of the loop for this frame as state is reset
        continue


    # --- Drawing ---
    bg_color = SKY_BLUE # Base color
    if debug_mode: bg_color = DEBUG_BG_COLOR # Override for debug
    screen.fill(bg_color)

    pygame.draw.rect(screen, GRASS_GREEN, (0, GROUND_Y, SCREEN_WIDTH, SCREEN_HEIGHT - GROUND_Y))
    draw_goal_isometric(screen, GOAL_LINE_X_LEFT, GOAL_Y_POS, GOAL_HEIGHT, -GOAL_DEPTH_X, GOAL_DEPTH_Y, GOAL_POST_THICKNESS, GOAL_COLOR, GOAL_NET_COLOR) # Left Goal
    draw_goal_isometric(screen, GOAL_LINE_X_RIGHT, GOAL_Y_POS, GOAL_HEIGHT, GOAL_DEPTH_X, GOAL_DEPTH_Y, GOAL_POST_THICKNESS, GOAL_COLOR, GOAL_NET_COLOR) # Right Goal
    ...
    powerup.draw(screen) # Draw powerup parachute/box
    for r in active_rockets: r.draw(screen) # Draw rockets
    for e in active_explosions: e.draw(screen) # Draw explosions
    player1.draw(screen); player2.draw(screen) # Draw players (includes wings/gun if active)
    ball.draw(screen); draw_offscreen_arrow(screen, ball, None)

    # --- Draw UI ---
    # ... (Scoreboard, game scores, Goal! message, Match Over message unchanged) ...
    ...
    if match_active: # Draw cooldowns & powerup indicators only during active match
        ...
        # Draw powerup indicators / timers
        powerup_font = font_small
        if player1.current_powerup == "FLIGHT":
            timer_text = f"P1 Flight: {player1.flight_timer:.1f}"; timer_surf = powerup_font.render(timer_text, True, (0,100,200)); timer_rect = timer_surf.get_rect(topleft=(10, SCREEN_HEIGHT - 30)); screen.blit(timer_surf, timer_rect)
        elif player1.current_powerup == "ROCKET_LAUNCHER":
            powerup_text = "P1 ROCKET"; powerup_surf = powerup_font.render(powerup_text, True, (200, 50, 50)); powerup_rect = powerup_surf.get_rect(topleft=(10, SCREEN_HEIGHT - 30)); screen.blit(powerup_surf, powerup_rect)

        if player2.current_powerup == "FLIGHT":
            timer_text = f"P2 Flight: {player2.flight_timer:.1f}"; timer_surf = powerup_font.render(timer_text, True, (0,100,200)); timer_rect = timer_surf.get_rect(topright=(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 30)); screen.blit(timer_surf, timer_rect)
        elif player2.current_powerup == "ROCKET_LAUNCHER":
            powerup_text = "P2 ROCKET"; powerup_surf = powerup_font.render(powerup_text, True, (200, 50, 50)); powerup_rect = powerup_surf.get_rect(topright=(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 30)); screen.blit(powerup_surf, powerup_rect)
