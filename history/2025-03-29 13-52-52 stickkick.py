...
ROCKET_EXPLOSION_FORCE = 85.0 # <<< Increased explosion force
ROCKET_PLAYER_UPWARD_BOOST = 8.0 # <<< Specific upward boost for players hit
...
# LASER_COLOR = (255, 0, 0, 220) # <<< More opaque red (using hex now)
LASER_COLOR_HEX = "#C70E20" # <<< Specific red color
LASER_ALPHA = 220 # <<< Alpha component for the laser
LASER_COLOR = pygame.Color(LASER_COLOR_HEX)
LASER_COLOR.a = LASER_ALPHA
LASER_LENGTH = 200 # <<< Shorter laser
LASER_WIDTH = 1
...
class ParachutePowerup: # Now just a visual container
    def __init__(self):
        self.x = -100; self.y = -100; self.vy = POWERUP_DESCEND_SPEED; self.vx = 0
        self.width, self.height = POWERUP_BOX_SIZE; self.active = False
        self.chute_radius = 35
        self.powerup_type = None # Set during spawn
        self.id = random.randint(1, 1000000) # Unique ID for removal from list
    def spawn(self):
    ...
    def update(self, dt):
        ...
        if self.y - self.chute_radius > SCREEN_HEIGHT: self.active = False # Deactivate if off-screen
    ...
    def check_collision(self, player):
        ...
        if player_rect.colliderect(powerup_box_rect):
            # Deactivation now happens in the main loop after returning type
            print(f"Powerup collected: {self.powerup_type}")
            play_sound(loaded_sounds['combo'])
            return self.powerup_type # Return the type
        return None
...
class Rocket: # Added head collision check
    ...
    def update(self, dt, players, ball):
        ...
        rocket_center_x, rocket_center_y = self.x, self.y

        # Player collision (body OR head, excluding owner)
        for p in players:
            if p == self.owner: continue # Skip self

            # Check body collision
            if p.get_body_rect().colliderect(rocket_rect):
                exploded = True; break

            # Check head collision (circle vs point)
            if not exploded:
                head_pos, head_radius = p.get_head_position_radius()
                dist_sq_head = (rocket_center_x - head_pos[0])**2 + (rocket_center_y - head_pos[1])**2
                # Simple check: rocket center within head radius + half rocket width
        ...
        if exploded:
            self.active = False
            create_explosion(self.x, self.y, ball.radius * ROCKET_BLAST_RADIUS_FACTOR, players, ball)
            return True
        return False
    def get_rect(self):
        # A simple bounding box is sufficient for collision detection here
...
class Explosion: # ... (no change) ...
    ...
    def update(self, dt):
        if not self.active: return False # Return activity status
        self.timer -= dt
...
def create_explosion(x, y, radius, players, ball):
    ...
    for p in players:
        ...
        if dist_sq < radius**2 and dist_sq > 0:
            ...
            p.vy += push_vec_y * force_magnitude * 0.8 - ROCKET_PLAYER_UPWARD_BOOST # Use specific player boost
    ...
    if dist_sq < radius**2 and dist_sq > 0:
        ...
        ball.apply_force(push_vec_x * force_magnitude, push_vec_y * force_magnitude - ROCKET_BALL_UPWARD_BOOST, hitter='explosion') # Use specific ball boost

    active_explosions.append(Explosion(x, y, radius))

class StickMan: # Added tumbling state and draw rotation
    def __init__(self, x, y, facing=1):
        ...
        self.rotation_angle = 0.0 # Current angle in radians
    ...
    def start_tumble(self):
        if not self.is_tumbling:
             ...
             # Maybe disable kicking/moving input temporarily?
             self.is_kicking = False
             self.kick_timer = 0
             # print(f"Player {1 if self.facing_direction==1 else 2} starting tumble!")

    def move(self, direction):
        if self.is_tumbling: return # Disable movement while tumbling
    ...
    def stop_move(self):
        ...
        self.vx = 0
    def jump(self):
        if self.is_tumbling: return # Disable jumping while tumbling
        can_jump_now = False
    ...
    def start_kick(self):
        if self.is_tumbling: return # Disable kicking/firing while tumbling
    ...
    def update(self, dt, other_player):
        # --- Tumbling State Update ---
        if self.is_tumbling:
            self.tumble_timer -= dt
            if self.tumble_timer <= 0:
                self.is_tumbling = False
                self.tumble_timer = 0.0
                # Smoothly stop rotation? Or snap? Let's snap for now.
                self.rotation_angle = 0.0
                self.rotation_velocity = 0.0
                print(f"Player {1 if self.facing_direction==1 else 2} finished tumble.")
            else:
                # Apply rotation and damping
                self.rotation_angle += self.rotation_velocity * dt
                self.rotation_velocity *= (PLAYER_TUMBLE_DAMPING ** (dt * 60)) # Apply damping based on dt
        ...
        # --- Physics ---
        # Gravity applies even when tumbling
        ...
        elif was_on_head and is_aligned_for_head: self.y = head_top_y; self.vy = 0 # Stay on head if aligned
        # Allow bouncing off head even when tumbling
        ...
        if self.vy >= 0:
            ...
            if can_land_on_head_now:
                ...
                if self.is_tumbling: self.vy *= -0.3 # Dampen bounce if tumbling
                ...
                if self.is_tumbling: self.rotation_velocity *= 0.8; self.vx *= 0.8 # Dampen on ground hit while tumbling
            else: self.y = next_y;
        ...
        if self.y > self.base_y and not self.on_other_player_head: self.y = self.base_y; # Clamp to ground
        if self.vy > 0 and self.y == self.base_y: self.vy = 0; self.is_jumping = False; # Stop gravity accumulation on ground
        ...
        # Horizontal movement (allow sliding while tumbling)
        intended_vx = self.vx
        if not self.is_kicking: # Allow movement application even if tumbling, just not player-controlled input
            effective_vx = intended_vx
            if self.on_other_player_head: effective_vx += other_player.vx # Inherit speed if on head
            self.x += effective_vx * dt * 60 # Apply velocity (adjust for dt if needed, assume fixed FPS for now)
            self.x = max(self.limb_width / 2, min(self.x, SCREEN_WIDTH - self.limb_width / 2)) # Clamp X position

        # --- Wing Animation Update ---
        if self.current_powerup == "FLIGHT": # ... (wing animation unchanged) ...
        ...
        # --- Body Animation (Skip if tumbling) ---
        if not self.is_tumbling:
            # ... (existing kick, walk, jump animation logic) ...
        ...
        else: # If tumbling, maybe make limbs flail?
        ...
        # --- Update Body Rect (Based on un-rotated state for collision) ---
        # Body rect for collisions should probably NOT rotate with the visual tumble
    ...
    def draw(self, screen): # Apply rotation using temporary surface
        # --- Calculate Bounding Box for Drawing ---
        # Find min/max x/y of all calculated joint positions to size the temp surface
        all_points = [self.head_pos, self.neck_pos, self.hip_pos, self.shoulder_pos,
                      self.l_elbow_pos, self.r_elbow_pos, self.l_hand_pos, self.r_hand_pos,
                      self.l_knee_pos, self.r_knee_pos, self.l_foot_pos, self.r_foot_pos,
                      self.gun_tip_pos] # Include gun tip if relevant
        ...
        # Add wing bounds if flying
        if self.current_powerup == "FLIGHT":
             min_x = min(min_x, self.shoulder_pos[0] - self.wing_upper_lobe_size[0] - 10)
             max_x = max(max_x, self.shoulder_pos[0] + self.wing_upper_lobe_size[0] + 10)
             min_y = min(min_y, self.shoulder_pos[1] - self.wing_upper_lobe_size[1] - 10)
             max_y = max(max_y, self.hip_pos[1] + self.wing_lower_lobe_size[1] + 10)

        surf_width = max(1, int(max_x - min_x))
        surf_height = max(1, int(max_y - min_y))
        temp_surf = pygame.Surface((surf_width, surf_height), pygame.SRCALPHA)

        # --- Calculate Offset for Drawing on Temp Surface ---
        offset_x = -min_x
        offset_y = -min_y

        # --- Draw Everything Relative to Offset ---
        ...
        torso_start_pos = offset_pos(self.neck_pos)
        torso_segment_height = self.torso_length / 3; current_torso_y = torso_start_pos[1]
        ...
        def draw_limb_segment_offset(start_pos, end_pos, length, color):
            o_start = offset_pos(start_pos); o_end = offset_pos(end_pos)
            center_x = (o_start[0] + o_end[0]) / 2; center_y = (o_start[1] + o_end[1]) / 2
        ...
        if self.current_powerup == "FLIGHT": # Draw wings on temp surf
             ...
             def create_wing_poly_offset(attach_point, angle, length, width): # Modified to work on offset coords
        ...
        if self.current_powerup == "ROCKET_LAUNCHER": # Draw gun on temp surf
            ...
            # Calculate world gun tip pos (needed for laser origin even if rotated)
            world_gun_attach_x = self.shoulder_pos[0] + 5 * self.facing_direction
            world_gun_attach_y = self.shoulder_pos[1] + 10
            world_gun_center_x = world_gun_attach_x + (GUN_SIZE[0] / 2) * math.cos(gun_world_angle)
            world_gun_center_y = world_gun_attach_y + (GUN_SIZE[0] / 2) * math.sin(gun_world_angle)
            tip_offset = GUN_SIZE[0] / 2
            self.gun_tip_pos = (world_gun_center_x + tip_offset * math.cos(gun_world_angle),
                                world_gun_center_y + tip_offset * math.sin(gun_world_angle))

        # --- Rotate Surface if Tumbling ---
        if self.is_tumbling and self.rotation_angle != 0:
            rotated_surf = pygame.transform.rotate(temp_surf, -math.degrees(self.rotation_angle)) # Pygame rotation is counter-clockwise
            # Get new rect and calculate blit position to keep center correct
            blit_rect = rotated_surf.get_rect(center = offset_pos(self.hip_pos)) # Rotate around hip
            screen.blit(rotated_surf, blit_rect.topleft + pygame.Vector2(-offset_x, -offset_y))
        else:
            # Blit normally if not tumbling
            screen.blit(temp_surf, (min_x, min_y))

        # --- Draw Laser Separately (Not part of rotated surface) ---
        if self.current_powerup == "ROCKET_LAUNCHER":
            base_angle = 0 if self.facing_direction == 1 else math.pi
            laser_world_angle = base_angle + self.gun_angle_offset
            laser_end_x = self.gun_tip_pos[0] + LASER_LENGTH * math.cos(laser_world_angle);
            laser_end_y = self.gun_tip_pos[1] + LASER_LENGTH * math.sin(laser_world_angle)
            try:
                 # Draw directly on main screen now
                 pygame.draw.aaline(screen, LASER_COLOR, self.gun_tip_pos, (laser_end_x, laser_end_y))
            except ValueError: pass
...
# powerup = ParachutePowerup() # <<< Now using a list
active_powerups = [] # <<< List for active powerups
...
def reset_positions(): # Don't clear powerups list, reset player tumble state
    ...
    # powerup.active = False # <<< Removed - powerups persist
    active_rockets = []; active_explosions = [] # Clear projectiles/explosions
...
def start_new_game(): # Clear active powerups on new game
    ...
    active_powerups = [] # <<< Clear powerups list for a completely new game
...
while running:
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_7: debug_mode = not debug_mode; print(f"Debug Mode {'ACT' if debug_mode else 'DEACT'}IVATED")
            elif event.key == pygame.K_6: # DEBUG SPAWN
                if match_active:
                    print("DEBUG: Forcing powerup spawn.")
                    new_powerup = ParachutePowerup()
                    new_powerup.spawn()
                    active_powerups.append(new_powerup)
                    powerup_spawn_timer = random.uniform(POWERUP_SPAWN_INTERVAL_MIN, POWERUP_SPAWN_INTERVAL_MAX) # Reset timer too
                else: print("DEBUG: Match inactive, cannot force spawn.")
            elif game_over and event.key == pygame.K_r: start_new_game()
            elif match_active: # Only process game keys if match is active
                # Player 1 Controls (if not tumbling)
                ...
                # Player 2 Controls (if not tumbling)
        ...
        if event.type == pygame.KEYUP:
             if match_active:
                # Stop movement only if not tumbling and corresponding key is released
                if not player1.is_tumbling:
    ...
    # --- Power-up Spawning ---
    if match_active: # Only spawn during active match
        powerup_spawn_timer -= dt
        if powerup_spawn_timer <= 0:
            new_powerup = ParachutePowerup()
            new_powerup.spawn()
            active_powerups.append(new_powerup) # Add to the list
            powerup_spawn_timer = random.uniform(POWERUP_SPAWN_INTERVAL_MIN, POWERUP_SPAWN_INTERVAL_MAX) # Reset timer
    ...
    if match_active:
        ...
        # Update powerups and remove inactive
        for p in active_powerups[:]: # Iterate copy for safe removal
            p.update(dt)
            if not p.active:
        ...
        active_explosions = [e for e in active_explosions if e.update(dt)] # Update and remove finished

        # --- Power-up Collection (Iterate through list) ---
        ...
        for i, pup in enumerate(active_powerups):
            ...
            if collected_type_p1:
                if collected_type_p1 == "FLIGHT":
                    player1.flight_timer = POWERUP_FLIGHT_DURATION # Reset timer
                    player1.is_flying = True # Ensure flying is active
                player1.current_powerup = collected_type_p1 # Assign/overwrite
                collected_powerups_indices.append(i)
                continue # Only one player collects one powerup

            collected_type_p2 = pup.check_collision(player2)
            if collected_type_p2:
                if collected_type_p2 == "FLIGHT":
                    player2.flight_timer = POWERUP_FLIGHT_DURATION # Reset timer
                    player2.is_flying = True # Ensure flying is active
                player2.current_powerup = collected_type_p2 # Assign/overwrite
                collected_powerups_indices.append(i)

        # Remove collected powerups (iterate backwards to avoid index issues)
        for i in sorted(collected_powerups_indices, reverse=True):
            if 0 <= i < len(active_powerups): # Safety check
                 active_powerups.pop(i)
    ...
    if goal_scored_this_frame: # ... (unchanged) ...
        ...
        reset_positions(); continue # Always reset and continue after goal
    ...
    for pup in active_powerups: pup.draw(screen) # Draw all active powerups
