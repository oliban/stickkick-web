...
# --- Helper Functions ---
# ... (draw_polygon_shape, draw_pentagon, draw_hexagon, normalize, draw_rotated_rectangle, draw_goal_isometric, draw_scoreboard remain the same) ...
...
class Ball: # ... (Updated update method) ...
    ...
    def update(self, dt):
        ...
        if self.y + self.radius >= GROUND_Y:
            if self.vy >= 0:
                hit_ground = True
                if abs(self.vy) > 1.5: # Play sound only on significant bounce
                    play_sound(loaded_sounds['ball_bounce'])
...
def load_sounds(sound_dir="sounds"):
    ...
    sound_files = {
        ...
        "combo": ["combo_sparkle1.wav", "combo_sparkle2.wav", "combo_sparkle3.wav", "combo_sparkle4.wav"], # <<< UPDATED
...
while running:
    ...
    # --- Event Handling (CONTROLS SWAPPED) ---
    for event in pygame.event.get():
        if event.type == pygame.QUIT: running = False
        if event.type == pygame.KEYDOWN:
            # Player 1 (Black/White) Controls: ASDW
            ...
            # Player 2 (Italian) Controls: Arrow Keys
    ...
    goal_scored = False; goal_pos_x = -1; scorer = 0 # ... (rest as before) ...
