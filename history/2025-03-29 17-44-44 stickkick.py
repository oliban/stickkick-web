...
# Physics
GRAVITY = 0.5; BASE_PLAYER_SPEED = 4; BASE_JUMP_POWER = -11 # <<< Base values
...
# --- Power-up Constants ---
POWERUP_TYPES = ["FLIGHT", "ROCKET_LAUNCHER", "BIG_PLAYER", "SUPER_JUMP", "BALL_FREEZE", "SPEED_BOOST", "GOAL_SHIELD", "SHRINK_OPPONENT"] # <<< Added new types
POWERUP_SPAWN_INTERVAL_MIN = 15.0 # Reduced interval for more action
POWERUP_SPAWN_INTERVAL_MAX = 30.0 # Reduced interval
...
POWERUP_BIG_PLAYER_DURATION = 20.0 # <<< New duration
POWERUP_BIG_PLAYER_SCALE = 1.75    # <<< New scale factor
POWERUP_SHRINK_PLAYER_DURATION = 15.0 # <<< New duration
POWERUP_SHRINK_PLAYER_SCALE = 0.6    # <<< New scale factor
POWERUP_SUPER_JUMP_DURATION = 15.0 # <<< New duration
POWERUP_SUPER_JUMP_MULTIPLIER = 1.8 # <<< New multiplier
POWERUP_BALL_FREEZE_DURATION = 3.0 # <<< New duration
POWERUP_SPEED_BOOST_DURATION = 10.0 # <<< New duration
POWERUP_SPEED_BOOST_MULTIPLIER = 1.6 # <<< New multiplier
POWERUP_GOAL_SHIELD_DURATION = 10.0 # <<< New duration
POWERUP_GOAL_SHIELD_COLOR = (100, 180, 255, 150) # Semi-transparent light blue
POWERUP_GOAL_SHIELD_WIDTH = 10
...
ROCKET_PLAYER_UPWARD_BOOST = 18.0
...
class StickMan: # Added powerup handling, size scaling, tumbling
    def __init__(self, x, y, facing=1):
        ...
        self.is_big = False; self.big_timer = 0.0
        self.is_shrunk = False; self.shrink_timer = 0.0
        self.jump_power = BASE_JUMP_POWER; self.super_jump_timer = 0.0
        self.player_speed = BASE_PLAYER_SPEED; self.speed_boost_timer = 0.0
    ...
    # --- Methods for applying/removing powerup effects ---
    def apply_powerup(self, powerup_type, other_player=None):
        self.active_powerups.add(powerup_type)
        if powerup_type == "FLIGHT":
            self.flight_timer = POWERUP_FLIGHT_DURATION
            self.is_flying = True
        elif powerup_type == "BIG_PLAYER":
            self.big_timer = POWERUP_BIG_PLAYER_DURATION
            self.is_big = True
            self.is_shrunk = False # Cancel shrink
            self.shrink_timer = 0.0
            self.calculate_current_sizes()
        elif powerup_type == "SUPER_JUMP":
            self.super_jump_timer = POWERUP_SUPER_JUMP_DURATION
            self.jump_power = BASE_JUMP_POWER * POWERUP_SUPER_JUMP_MULTIPLIER
        elif powerup_type == "SPEED_BOOST":
            self.speed_boost_timer = POWERUP_SPEED_BOOST_DURATION
            self.player_speed = BASE_PLAYER_SPEED * POWERUP_SPEED_BOOST_MULTIPLIER
        elif powerup_type == "SHRINK_OPPONENT":
            if other_player:
                other_player.apply_shrink() # Apply shrink to the other player
        # Note: BALL_FREEZE and GOAL_SHIELD are handled globally/in main loop

    def apply_shrink(self): # Called on the player being shrunk
         self.active_powerups.add("SHRUNK") # Add a status effect type
         self.shrink_timer = POWERUP_SHRINK_PLAYER_DURATION
         self.is_shrunk = True
         self.is_big = False # Cancel big
         self.big_timer = 0.0
         self.calculate_current_sizes()
    ...
    def calculate_current_sizes(self):
        ...
        self.head_radius = self.base_head_radius * scale
        self.torso_length = self.base_torso_length * scale
        self.limb_width = self.base_limb_width # Keep limb width constant? Or scale? Let's scale slightly.
        self.limb_width = self.base_limb_width * (1.0 + (scale - 1.0) * 0.5) # Scale width less drastically
        self.upper_arm_length = self.base_upper_arm_length * scale
        self.forearm_length = self.base_forearm_length * scale
        self.thigh_length = self.base_thigh_length * scale
        self.shin_length = self.base_shin_length * scale
        self.current_nose_length = self.base_nose_length * scale
        self.current_nose_width = self.base_nose_width * scale

        # Adjust base_y when big/small to keep feet near ground
        visual_height = self.head_radius * 2 + self.torso_length + self.thigh_length + self.shin_length
        # Maybe adjust base_y based on shin+thigh length change? This is tricky.
        # Let's keep base_y for physics, but visual joints will adapt based on lengths.
        # The physics 'feet' position might be slightly off visually when scaled.

    def move(self, direction):
        if self.is_tumbling: return
        if not self.is_kicking: self.vx = direction * self.player_speed # Use current speed
    ...
    def jump(self):
        ...
        if can_jump_now:
            ...
            self.vy = self.jump_power; self.walk_cycle_timer = 0 # Use current jump power
    ...
    def update(self, dt, other_player):
        ...
        # --- Powerup Timer Countdowns & Expiry ---
        if self.flight_timer > 0:
            self.flight_timer -= dt
            if self.flight_timer <= 0:
                self.active_powerups.discard("FLIGHT"); self.is_flying = False; print("Flight ended")
        if self.big_timer > 0:
            self.big_timer -= dt
            if self.big_timer <= 0:
                self.active_powerups.discard("BIG_PLAYER"); self.is_big = False; self.calculate_current_sizes(); print("Big Player ended")
        if self.shrink_timer > 0:
            self.shrink_timer -= dt
            if self.shrink_timer <= 0:
                self.active_powerups.discard("SHRUNK"); self.is_shrunk = False; self.calculate_current_sizes(); print("Shrink ended")
        if self.super_jump_timer > 0:
            self.super_jump_timer -= dt
            if self.super_jump_timer <= 0:
                self.active_powerups.discard("SUPER_JUMP"); self.jump_power = BASE_JUMP_POWER; print("Super Jump ended")
        if self.speed_boost_timer > 0:
            self.speed_boost_timer -= dt
            if self.speed_boost_timer <= 0:
                 self.active_powerups.discard("SPEED_BOOST"); self.player_speed = BASE_PLAYER_SPEED; print("Speed Boost ended")

        # Update gun animation if rocket launcher is held
        ...
        is_aligned_for_head = abs(dist_x_head) < (other_head_radius + self.head_radius + HEAD_PLATFORM_RADIUS_BUFFER) # Use current radius
        ...
        if not self.is_kicking:
            ...
            self.x += effective_vx # Apply velocity based on dt handled later potentially, or assume fixed FPS
        ...
        if not self.is_tumbling: # ... (unchanged) ...
        ...
        # --- Calculate Joint Positions (using current sizes) ---
        current_y = self.y; current_x = self.x
        # Use current limb lengths
        ...
        self.head_pos = (upper_body_x, self.neck_pos[1] - self.head_radius) # Use current radius
        ...
        # --- Update Body Rect (using current sizes) ---
        body_width = self.limb_width * 1.5 # Use current limb width for visual rect width
        self.body_rect.width = int(body_width)
        self.body_rect.height = max(1, int(self.hip_pos[1] - self.neck_pos[1])) # Height based on joints
        self.body_rect.centerx = int(self.hip_pos[0]) # Centered on hip
        self.body_rect.top = int(self.neck_pos[1]) # Top at neck
    ...
    def get_kick_impact_point(self): # ... (no change) ...
        ...
        return None
    def get_head_position_radius(self): return self.head_pos, self.head_radius # Use current radius
    def get_body_rect(self): return self.body_rect # Use current rect
    ...
    def draw(self, screen): # ... (no change) ...
        ...
        pygame.draw.circle(temp_surf, ITALY_WHITE, head_center_int, int(self.head_radius), 0) # Use current radius
        eye_offset_x = self.head_radius * 0.35 * self.facing_direction; eye_offset_y = -self.head_radius * 0.1; eye_radius = int(max(1, 3 * (self.head_radius / self.base_head_radius))) # Scale eyes
        ...
        def draw_limb_segment_offset(start_pos, end_pos, length, color, limb_w): # Pass width
            ...
            angle = math.atan2(dy, dx); draw_rotated_rectangle(temp_surf, color, (center_x, center_y), draw_length, limb_w, angle + math.pi/2) # Use passed width
...
class Ball: # Added freeze logic
    ...
    def update(self, dt):
        ...
        # Shield Collision (check before walls/ground)
        if p1_shield_active:
            ...
            if self.x - self.radius < shield_rect.right and self.x + self.radius > shield_rect.left and \
               self.y + self.radius > shield_rect.top and self.y - self.radius < shield_rect.bottom:
                if self.vx < 0: # Moving towards shield
                    self.x = shield_rect.right + self.radius
                    self.vx *= -BALL_BOUNCE # Bounce off
                    hit_shield_this_frame = True
        if p2_shield_active:
            ...
            if self.x + self.radius > shield_rect.left and self.x - self.radius < shield_rect.right and \
               self.y + self.radius > shield_rect.top and self.y - self.radius < shield_rect.bottom:
                if self.vx > 0: # Moving towards shield
                    self.x = shield_rect.left - self.radius
                    self.vx *= -BALL_BOUNCE # Bounce off
                    hit_shield_this_frame = True

        if hit_shield_this_frame: play_sound(loaded_sounds['wall_hit']) # Use wall hit sound for shield
    ...
    def draw(self, screen): # Added freeze visual
        ...
        if self.is_frozen or self.freeze_effect_timer > 0:
            alpha = 150 # Base alpha for frozen
            if self.freeze_effect_timer > 0 and not self.is_frozen: # Fade out effect after unfreeze
                alpha = int(150 * (self.freeze_effect_timer / (POWERUP_BALL_FREEZE_DURATION * 0.5))) # Fade over half the freeze duration

            freeze_color = (180, 220, 255, max(0, min(255, alpha))) # Light blue with alpha
            temp_surf = pygame.Surface((self.radius * 2, self.radius * 2), pygame.SRCALPHA)
            pygame.draw.circle(temp_surf, freeze_color, (self.radius, self.radius), self.radius)
...
def reset_positions(): # Reset new powerup states
    global ball_was_on_ground, current_hit_count, p1_can_headbutt, p2_can_headbutt, p1_body_collision_timer, p2_body_collision_timer, active_rockets, active_explosions, ball_freeze_timer, p1_shield_active, p1_shield_timer, p2_shield_active, p2_shield_timer
    ball.x = SCREEN_WIDTH // 2; ball.y = SCREEN_HEIGHT // 3; ball.vx = 0; ball.vy = 0; ball.is_frozen = False; ball.freeze_effect_timer = 0.0 # Reset ball freeze
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0; player1.is_kicking = False; player1.on_other_player_head = False; player1.facing_direction = 1; player1.is_jumping = False; player1.active_powerups = set(); player1.is_flying = False; player1.flight_timer = 0.0; player1.is_tumbling = False; player1.rotation_angle = 0; player1.rotation_velocity = 0; player1.is_big = False; player1.is_shrunk = False; player1.big_timer=0.0; player1.shrink_timer=0.0; player1.jump_power=BASE_JUMP_POWER; player1.super_jump_timer=0.0; player1.player_speed=BASE_PLAYER_SPEED; player1.speed_boost_timer=0.0; player1.calculate_current_sizes() # Reset player state
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0; player2.is_kicking = False; player2.on_other_player_head = False; player2.facing_direction = -1; player2.is_jumping = False; player2.active_powerups = set(); player2.is_flying = False; player2.flight_timer = 0.0; player2.is_tumbling = False; player2.rotation_angle = 0; player2.rotation_velocity = 0; player2.is_big = False; player2.is_shrunk = False; player2.big_timer=0.0; player2.shrink_timer=0.0; player2.jump_power=BASE_JUMP_POWER; player2.super_jump_timer=0.0; player2.player_speed=BASE_PLAYER_SPEED; player2.speed_boost_timer=0.0; player2.calculate_current_sizes() # Reset player state
    ...
    active_rockets = []; active_explosions = []
    ball_freeze_timer = 0.0 # Reset global powerup states
    p1_shield_active = False; p1_shield_timer = 0.0
    p2_shield_active = False; p2_shield_timer = 0.0
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne): # ... (no change) ...
    ...
    if local_kick_point:
        ...
        # Use current head radius for kick radius scaling? Maybe too complex. Stick to base.
        eff_kick_rad = KICK_RADIUS_NORMAL + (KICK_RADIUS_FALLING_BONUS if ball.vy > BALL_FALLING_VELOCITY_THRESHOLD else 0)
        if dist_sq < (ball.radius + eff_kick_rad * (player.head_radius / player.base_head_radius))**2: # Scale kick radius slightly
    ...
    if dist_head_sq < (ball.radius + head_radius)**2: # Use current head radius
        if can_headbutt:
...
while running:
    ...
    if ball_freeze_timer > 0:
        ...
        if ball_freeze_timer <= 0:
            ...
            ball.freeze_effect_timer = POWERUP_BALL_FREEZE_DURATION * 0.5 # Start fade out
            print("Ball un-frozen")
    ...
    if match_active:
        ...
        ball_hit_ground_this_frame = ball.update(dt) # Ball update now handles freeze check

        # Update active powerups FIRST, then filter
        for pup in active_powerups:
            pup.update(dt)
        active_powerups = [p for p in active_powerups if p.active]
        ...
        for i, pup in enumerate(active_powerups):
            ...
            # Check player 1
            if not collected_this_pup and not player1.is_tumbling: # Cannot collect while tumbling
                collected_type_p1 = pup.check_collision(player1)
                if collected_type_p1:
                    player1.apply_powerup(collected_type_p1, other_player=player2) # Pass opponent for shrink
                    collected_powerups_indices.append(i); collected_this_pup = True
                    # Handle global effects immediately
                    if collected_type_p1 == "BALL_FREEZE":
                         ball.is_frozen = True; ball_freeze_timer = POWERUP_BALL_FREEZE_DURATION; ball.freeze_effect_timer = 0; print("Ball Frozen!")
                         ball.vx = 0; ball.vy = 0 # Stop ball immediately
                    elif collected_type_p1 == "GOAL_SHIELD":
                         p1_shield_active = True; p1_shield_timer = POWERUP_GOAL_SHIELD_DURATION; print("P1 Shield Up!")

            # Check player 2 only if P1 didn't collect THIS powerup
            if not collected_this_pup and not player2.is_tumbling: # Cannot collect while tumbling
                collected_type_p2 = pup.check_collision(player2)
                if collected_type_p2:
                    player2.apply_powerup(collected_type_p2, other_player=player1) # Pass opponent for shrink
                    collected_powerups_indices.append(i)
                    # Handle global effects immediately
                    if collected_type_p2 == "BALL_FREEZE":
                         ball.is_frozen = True; ball_freeze_timer = POWERUP_BALL_FREEZE_DURATION; ball.freeze_effect_timer = 0; print("Ball Frozen!")
                         ball.vx = 0; ball.vy = 0 # Stop ball immediately
                    elif collected_type_p2 == "GOAL_SHIELD":
                         p2_shield_active = True; p2_shield_timer = POWERUP_GOAL_SHIELD_DURATION; print("P2 Shield Up!")


        # Remove collected powerups from the main list
    ...
    if match_active and not ball.is_frozen: # Cannot score if ball frozen
    ...
    if goal_scored_this_frame: # ... (goal handling unchanged) ...
    ...
    if p1_shield_active:
        shield_rect = pygame.Rect(0, GOAL_Y_POS, POWERUP_GOAL_SHIELD_WIDTH, GOAL_HEIGHT)
        shield_surf = pygame.Surface(shield_rect.size, pygame.SRCALPHA)
        alpha = int(150 * (p1_shield_timer / POWERUP_GOAL_SHIELD_DURATION)) # Fade out
        shield_surf.fill((POWERUP_GOAL_SHIELD_COLOR[0], POWERUP_GOAL_SHIELD_COLOR[1], POWERUP_GOAL_SHIELD_COLOR[2], alpha))
        screen.blit(shield_surf, shield_rect.topleft)
    if p2_shield_active:
        shield_rect = pygame.Rect(SCREEN_WIDTH - POWERUP_GOAL_SHIELD_WIDTH, GOAL_Y_POS, POWERUP_GOAL_SHIELD_WIDTH, GOAL_HEIGHT)
        shield_surf = pygame.Surface(shield_rect.size, pygame.SRCALPHA)
        alpha = int(150 * (p2_shield_timer / POWERUP_GOAL_SHIELD_DURATION)) # Fade out
        shield_surf.fill((POWERUP_GOAL_SHIELD_COLOR[0], POWERUP_GOAL_SHIELD_COLOR[1], POWERUP_GOAL_SHIELD_COLOR[2], alpha))
        screen.blit(shield_surf, shield_rect.topleft)
    ...
    if match_active:
        ...
        # Powerup Indicators (Stacked) - Define colors for new ones
        powerup_font = font_small; line_height = powerup_font.get_height() + 2
        powerup_colors = { "FLIGHT": (0,100,200), "ROCKET_LAUNCHER": (200, 50, 50), "BIG_PLAYER": (50, 180, 50), "SUPER_JUMP": (200, 200, 50), "SPEED_BOOST": (255, 150, 0), "GOAL_SHIELD": (100, 180, 255), "SHRUNK": (150, 100, 200)} # Added colors for new powerups
        powerup_texts = { "FLIGHT": "Flight: {:.1f}", "ROCKET_LAUNCHER": "ROCKET", "BIG_PLAYER": "BIG: {:.1f}", "SUPER_JUMP": "JUMP: {:.1f}", "SPEED_BOOST": "SPEED: {:.1f}", "GOAL_SHIELD": "SHIELD: {:.1f}", "SHRUNK": "Shrunk: {:.1f}"}
        powerup_timers = { "FLIGHT": "flight_timer", "BIG_PLAYER": "big_timer", "SUPER_JUMP": "super_jump_timer", "SPEED_BOOST": "speed_boost_timer", "SHRUNK": "shrink_timer"}

        # Player 1 UI
        p1_ui_y = SCREEN_HEIGHT - 30
        for p_type in player1.active_powerups:
            text = powerup_texts.get(p_type, p_type) # Get base text
            timer_attr = powerup_timers.get(p_type)
            if timer_attr: # Format with timer if applicable
                 text = text.format(getattr(player1, timer_attr, 0.0))
            color = powerup_colors.get(p_type, WHITE)
        ...
        for p_type in player2.active_powerups:
            text = powerup_texts.get(p_type, p_type) # Get base text
            timer_attr = powerup_timers.get(p_type)
            if timer_attr: # Format with timer if applicable
                 text = text.format(getattr(player2, timer_attr, 0.0))
            color = powerup_colors.get(p_type, WHITE)
            surf = powerup_font.render(text, True, color); rect = surf.get_rect(bottomright=(SCREEN_WIDTH - 10, p2_ui_y)); screen.blit(surf, rect)
            p2_ui_y -= line_height

        # Global Effect Indicators (Optional)
        if ball.is_frozen:
            freeze_text = "BALL FROZEN: {:.1f}".format(ball_freeze_timer)
            freeze_surf = powerup_font.render(freeze_text, True, (180, 220, 255)); freeze_rect = freeze_surf.get_rect(midbottom=(SCREEN_WIDTH // 2, SCREEN_HEIGHT - 10)); screen.blit(freeze_surf, freeze_rect)
