...
def reset_positions(): # Keep powerups active
    ...
    if ball.is_frozen: ball.freeze_effect_timer = POWERUP_BALL_FREEZE_DURATION * 0.1 # Start fade if frozen
    # Reset only dynamic states
...
def start_new_match(): # Full reset for new match
    ...
    # Explicitly clear/reset all powerup states
...
while running:
    ...
    if match_active:
        ...
        active_powerups = [p for p in active_powerups if p.update(dt)] # Filter based on update return
        ...
        # --- Power-up Collection ---
        collected_powerups_indices = [] # ... (unchanged collection logic including BALL_FREEZE trigger) ...
        ...
        # --- Player Collisions ---
        # ... (player-player body collision unchanged) ...
    ...
    if goal_scored_this_frame: # --- Goal Effects ---
        ...
        # Reset positions, keeping powerups active
        reset_positions()
        continue
    ...
    # --- Draw UI ---
    # ... (UI drawing logic handles dict values correctly now) ...
    ...
    if match_active:
        ...
        for p_type in sorted(player1.active_powerups.keys()):
            ...
            elif p_type in ["FLIGHT", "BIG_PLAYER", "SUPER_JUMP", "SPEED_BOOST", "SHRUNK"]: text = text_template.format(val)
