...
class StickMan: #... (as before) ...
    ...
    def get_kick_impact_point(self): # Fixed UnboundLocalError risk
        ...
        if self.is_kicking:
            if self.kick_duration <= 0: return None # Avoid division by zero
...
while running:
    ...
    if match_over_timer > 0:
        ...
        # Sounds play independently via queue and event
        if match_over_timer <= 0 and not game_over:
             start_new_match()

    # --- Process Announcement Sound Queue ---
    # This attempts to play the next sound if the channel is free,
    # acting as a starter/backup for the event system.
