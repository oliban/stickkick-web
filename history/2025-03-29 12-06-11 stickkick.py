...
FLIGHT_MODE_BG_COLOR = (100, 100, 110) # Gray
...
class StickMan: # Added wing attributes and logic
    def __init__(self, x, y, facing=1):
        ...
        self.l_wing_angle = math.pi + self.wing_rest_angle_offset; self.r_wing_angle = -self.wing_rest_angle_offset
        self.wing_flap_timer = 0.0; self.wing_flap_duration = 0.25; self.wing_flapping = False
        self.wing_upper_lobe_size = (30, 22); self.wing_lower_lobe_size = (18, 18)
        # --- Head Feature Colors ---
        self.eye_color = BLACK
        if P1_COLOR_MAIN == (220, 220, 220): self.cap_color = (50, 50, 50) # Dark Grey for P1
        else: self.cap_color = ITALY_RED # Red for P2
    ...
    def jump(self): # Conditional mid-air jump based on FLIGHT mode
        can_jump_now = False
        ...
        if can_jump_now:
            ...
            if flight_mode_active: self.start_wing_flap() # Flap only if flight mode is on
    def start_kick(self):
    ...
    def update(self, dt, other_player):
        ...
        wing_rest_angle_offset = self.wing_rest_angle_offset # Use value from init
        wing_flap_down_angle_offset = math.pi * 0.4
    ...
    def draw(self, screen):
        ...
        # --- Draw Head Features ---
        cap_height = self.head_radius * 0.8; cap_width = self.head_radius * 1.8
        cap_rect = pygame.Rect(0, 0, cap_width, cap_height); cap_rect.center = (head_center_x, head_center_y - self.head_radius * 0.5)
        pygame.draw.ellipse(screen, self.cap_color, cap_rect)
        brim_width = self.head_radius * 1.2; brim_height = self.head_radius * 0.4
        brim_x = head_center_x + (self.head_radius * 0.5) * self.facing_direction; brim_y = cap_rect.centery + cap_height * 0.1
        brim_rect = pygame.Rect(0, 0, brim_width, brim_height); brim_rect.center = (brim_x, brim_y); pygame.draw.rect(screen, self.cap_brim_color, brim_rect)
        # Head Fill (Draw OVER cap base, UNDER brim/eyes/nose/outline)
        pygame.draw.circle(screen, ITALY_WHITE, head_center_int, self.head_radius, 0)
        # Eyes
        eye_offset_x = self.head_radius * 0.4 * self.facing_direction; eye_offset_y = -self.head_radius * 0.1; eye_radius = 2
        eye_pos_x = int(head_center_x + eye_offset_x); eye_y = int(head_center_y + eye_offset_y)
        pygame.draw.circle(screen, self.eye_color, (eye_pos_x, eye_y - 2), eye_radius); pygame.draw.circle(screen, self.eye_color, (eye_pos_x, eye_y + 2), eye_radius)
        # Triangle Nose
        nose_length = self.head_radius * 0.5 ; nose_width = self.head_radius * 0.3
        tip_x = head_center_x + nose_length * self.facing_direction; tip_y = head_center_y + self.head_radius * 0.3
        base_y1 = tip_y - nose_width / 2; base_y2 = tip_y + nose_width / 2; base_x = head_center_x + 2 * self.facing_direction
        nose_points = [(int(base_x), int(base_y1)), (int(tip_x), int(tip_y)), (int(base_x), int(base_y2))]
        ...
        # --- Draw Wings (Only in Flight Mode) using Polygons ---
        if flight_mode_active:
            wing_attach_x = self.shoulder_pos[0]; wing_attach_y = self.shoulder_pos[1] + 3
            ...
            angle_offset_upper = -0.5; angle_offset_lower = 0.6
            def create_wing_poly(attach_point, base_angle, length, width, angle_offset):
                angle = base_angle + angle_offset; cos_a = math.cos(angle); sin_a = math.sin(angle)
                cos_w = math.cos(angle + math.pi/2); sin_w = math.sin(angle + math.pi/2)
                p1 = attach_point
                p2 = (attach_point[0] + length * cos_a + width/2 * cos_w, attach_point[1] + length * sin_a + width/2 * sin_w)
                p3 = (attach_point[0] + length * 0.8 * cos_a, attach_point[1] + length * 0.8 * sin_a)
                p4 = (attach_point[0] + length * cos_a - width/2 * cos_w, attach_point[1] + length * sin_a - width/2 * sin_w)
                p_inner_mid = (attach_point[0] + length*0.2 * cos_a, attach_point[1] + length*0.2 * sin_a)
                return [(int(p[0]), int(p[1])) for p in [p1, p2, p3, p4, p_inner_mid]]
            l_attach = (wing_attach_x - 3, wing_attach_y)
            l_upper_poly = create_wing_poly(l_attach, self.l_wing_angle, upper_length, upper_width, angle_offset_upper)
            l_lower_poly = create_wing_poly(l_attach, self.l_wing_angle, lower_length, lower_width, angle_offset_lower)
            ...
            r_attach = (wing_attach_x + 3, wing_attach_y)
            r_upper_poly = create_wing_poly(r_attach, self.r_wing_angle, upper_length, upper_width, -angle_offset_upper)
            r_lower_poly = create_wing_poly(r_attach, self.r_wing_angle, lower_length, lower_width, -angle_offset_lower)
...
# --- Ball Class ---
# ... (Ball class remains the same as previous version) ...
class Ball:
...
while running:
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_6: # Toggle Flight Mode
                flight_mode_active = not flight_mode_active; print(f"Flight Mode {'ACT' if flight_mode_active else 'DEACT'}IVATED")
    ...
    # --- Process Announcement Sound Queue (Triggered by event or initial call) ---
    # Removed call from main loop update
    ...
    if goal_scored_this_frame:
        ...
        if winner_found_this_match:
            ...
            match_end_sound_played = False # Reset flag to allow queuing
