...
BASE_KICK_FORCE_X = 15
BASE_KICK_FORCE_Y = -2 # Low Angle Kick
...
MATCH_POINT_LIMIT = 5 # <<< Normal match limit is 5
GAME_WIN_LIMIT = 5
MATCH_OVER_DURATION = 3.0
GOAL_MESSAGE_DURATION = 1.5

# --- Helper Functions ---
# ... (draw_polygon_shape, draw_pentagon, draw_hexagon, normalize, draw_rotated_rectangle, draw_goal_isometric, draw_scoreboard are the same) ...
...
def draw_scoreboard(surface, p1_score, p2_score, score_font, name_font, is_goal_active): #... (Updated version)
    score_text = f"{p1_score} - {p2_score}"
    name_text = "Nils vs. Harry"
    ...
    panel_padding_x = 30; panel_padding_y_top = 15; panel_padding_y_bottom = 10; text_spacing = 5
    ...
    panel_rect.centerx = SCREEN_WIDTH // 2; panel_rect.top = 5
    score_rect.centerx = panel_rect.centerx; score_rect.top = panel_rect.top + panel_padding_y_top
    name_rect.centerx = panel_rect.centerx; name_rect.top = score_rect.bottom + text_spacing
...
# --- Draw Past Game Scores (Corrected Order, Added Winner) ---
def draw_game_scores(surface, scores_list, font):
    start_x = 10; start_y = 10; current_y = start_y
    for score_tuple in reversed(scores_list): # Show oldest first
        p1s, p2s = score_tuple
        winner_name_str = ""
        # Check winner based on normal limit first, then debug limit if no one reached normal
        if p1s >= MATCH_POINT_LIMIT and p1s > p2s: winner_name_str = " (Nils)"
        elif p2s >= MATCH_POINT_LIMIT and p2s > p1s: winner_name_str = " (Harry)"
        elif p1s >= DEBUG_MATCH_POINT_LIMIT and p1s > p2s: winner_name_str = " (Nils)" # Check debug limit
        elif p2s >= DEBUG_MATCH_POINT_LIMIT and p2s > p1s: winner_name_str = " (Harry)" # Check debug limit

        score_str = f"{p1s} - {p2s}{winner_name_str}"
        score_surf = font.render(score_str, True, TEXT_COLOR)
        score_rect = score_surf.get_rect(topleft=(start_x, current_y))
        surface.blit(score_surf, score_rect)
        current_y += score_rect.height + 1
# --- Draw Trophy (Moved Down, Engraved Name, Rematch Text) ---
def draw_trophy(surface, winner_name, title_font, name_font):
    cup_color = (255, 215, 0); base_color = (139, 69, 19); engraved_color = BLACK
    trophy_center_x = SCREEN_WIDTH // 2; trophy_base_y = SCREEN_HEIGHT // 2 + 180 # Moved Down
    ...
    title_rect = title_surf.get_rect(center=(trophy_center_x, trophy_base_y - 220)) # Banner position
    name_surf = name_font.render(winner_name, True, engraved_color) # Engraved name
    name_rect = name_surf.get_rect(center=base_rect.center) # Center on base
...
# --- Stick Man Class ---
class StickMan: #... (Fixed get_kick_impact_point) ...
    ...
    def update(self, dt, other_player): # ... (as before) ...
        was_airborne = self.is_jumping or (not self.on_other_player_head and self.y < self.base_y)
        time_ms = pygame.time.get_ticks()
        was_on_head = self.on_other_player_head
        landed_on_head_this_frame = False; landed_on_ground_this_frame = False
...
class Ball: # ... (as before) ...
    ...
    def update(self, dt): # ... (as before) ...
        ...
        if self.y + self.radius >= GROUND_Y:
            if self.vy >= 0:
                hit_ground = True
                if abs(self.vy) > 1.5: play_sound(loaded_sounds['ball_bounce'])
...
screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT)); pygame.display.set_caption("Ciao Kick!"); clock = pygame.time.Clock()

# --- Sound Loading --- (Added number sounds loading)
...
match_winner = None; overall_winner = None; match_end_sound_played = False #<<< Added flag
...
def start_new_match(): # Reset sound flag
    global player1_score, player2_score, match_active, match_winner, match_over_timer, match_end_sound_played
    player1_score = 0; player2_score = 0
    match_active = True; match_winner = None; match_over_timer = 0.0
    match_end_sound_played = False # <<< Reset flag
    reset_positions()
    print("Starting new match.")
def start_new_game(): #... (as before) ...
    global p1_games_won, p2_games_won, game_scores, game_over, overall_winner
    p1_games_won = 0; p2_games_won = 0; game_scores = []
    game_over = False; overall_winner = None
    start_new_match()
    print("Starting new game.")

# --- Collision Handling Function (Player-Ball) ---
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne): # Uses new BASE_KICK_FORCE_Y
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 kick_y = BASE_KICK_FORCE_Y * KICK_FORCE_LEVEL # Uses new default
...
while running:
    ...
    # --- Global Input (Quit, Debug Toggle) ---
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_7: # Toggle Debug
                ...
                print(f"Debug Mode {'ACTIVATED' if debug_mode else 'DEACTIVATED'}")
    ...
    if game_over:
        for event in events:
             if event.type == pygame.KEYDOWN:
                 if event.key == pygame.K_ESCAPE: running = False
                 if event.key == pygame.K_r: # <<< Rematch Check
                     start_new_game()
    ...
    if match_over_timer > 0:
        # --- Play End-of-Match Sounds ONCE ---
        if not match_end_sound_played and not game_over:
            if match_winner == 1: play_sound(loaded_sounds['nils_wins'])
            elif match_winner == 2: play_sound(loaded_sounds['harry_wins'])
            pygame.time.wait(700)
            p1_games = p1_games_won; p2_games = p2_games_won
            if p1_games > p2_games:
                play_sound(loaded_sounds['nils_ahead']); pygame.time.wait(500)
                if 1 <= p1_games <= 5 and loaded_sounds['numbers'].get(p1_games): loaded_sounds['numbers'][p1_games].play()
                pygame.time.wait(500)
                if 1 <= p2_games <= 5 and loaded_sounds['numbers'].get(p2_games): loaded_sounds['numbers'][p2_games].play()
            elif p2_games > p1_games:
                play_sound(loaded_sounds['harry_ahead']); pygame.time.wait(500)
                if 1 <= p2_games <= 5 and loaded_sounds['numbers'].get(p2_games): loaded_sounds['numbers'][p2_games].play()
                pygame.time.wait(500)
                if 1 <= p1_games <= 5 and loaded_sounds['numbers'].get(p1_games): loaded_sounds['numbers'][p1_games].play()
            else: # Tied
                 ...
                 pygame.time.wait(500)
                 if 1 <= p2_games <= 5 and loaded_sounds['numbers'].get(p2_games): loaded_sounds['numbers'][p2_games].play()
            match_end_sound_played = True # Mark as played

        # --- Timer countdown and transition ---
        match_over_timer -= dt
        if match_over_timer <= 0 and not game_over: start_new_match()

        # Allow quit during match over
        for event in events:
    ...
    elif not game_over: # Allow quit during match over screen
         for event in events:
              if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: running = False
    ...
    if goal_scored_this_frame:
        ...
        current_match_limit = DEBUG_MATCH_POINT_LIMIT if debug_mode else MATCH_POINT_LIMIT # Use debug limit if active
        ...
        if winner_found_this_match:
            ...
            match_end_sound_played = False # <<< Allow sounds to play for next match end
        ...
        # Don't 'continue' here if match ended, allow drawing of match end state for one frame before timer takes over
        # 'continue' only if match is NOT over yet.
        if not winner_found_this_match:
             continue
    ...
    # --- Draw UI ---
    draw_scoreboard(screen, player1_score, player2_score, font_large, font_medium, goal_message_timer > 0 or match_over_timer > 0)
    draw_game_scores(screen, game_scores, font_small)
