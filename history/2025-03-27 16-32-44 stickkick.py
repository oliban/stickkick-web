...
# Player Colors
P1_COLOR_MAIN = (220, 220, 220) # White/Light Grey
P1_COLOR_ACCENT = (30, 30, 30)    # Black/Dark Grey
P2_COLOR_MAIN = ITALY_GREEN      # Keep P2 as Italy Green/Red/White
P2_COLOR_ACCENT = ITALY_RED
P2_COLOR_WHITE = ITALY_WHITE
# Other Colors
...
GOAL_EXPLOSION_COLORS = [WHITE, YELLOW, STAR_YELLOW, (255, 215, 0)] # Goldish colors
DEBUG_KICK_ANGLES = False

# Physics
GRAVITY = 0.5; PLAYER_SPEED = 4; JUMP_POWER = -11
BASE_KICK_FORCE_X = 15 # Slightly more horizontal force
BASE_KICK_FORCE_Y = -10 # *** Less steep angle ***
...
HEAD_PLATFORM_RADIUS_BUFFER = 5 # Extra radius for landing on head

# Kick Collision Tweak
KICK_RADIUS_NORMAL = 16 # *** Increased kick radius ***
KICK_RADIUS_FALLING_BONUS = 6 # *** Increased falling bonus ***
BALL_FALLING_VELOCITY_THRESHOLD = 5

# Goal Constants
GOAL_HEIGHT = 135 # *** Higher goals ***
GOAL_POST_THICKNESS = 3; GOAL_Y_POS = GROUND_Y - GOAL_HEIGHT
...
GOAL_PARTICLE_COUNT = 30 # More particles for goal
GOAL_PARTICLE_SPEED_MIN = 200
GOAL_PARTICLE_SPEED_MAX = 350
GOAL_PARTICLE_LIFESPAN = 1.2
...
# --- Particle Class (Added parameters) ---
class Particle:
    ...
    def update(self, dt):
        self.x += self.vx * dt; self.y += self.vy * dt; self.vy += GRAVITY * 20 * dt # Gravity affects particles
        self.lifespan -= dt
        if self.start_life > 0: self.size = PARTICLE_SIZE * max(0, (self.lifespan / self.start_life)) # Ensure size doesn't go negative
        else: self.size = 0
        return self.lifespan > 0 and self.size > 0.5 # Keep slightly larger particles
...
class StickMan:
    def __init__(self, x, y, facing=1):
        ...
        # Default colors (will be overridden for player1)
        ...
        self.on_other_player_head = False # <<< New flag for head standing
    ...
    def jump(self):
        # Allow jump if on ground OR on other player's head
        if not self.is_jumping or self.on_other_player_head:
            self.is_jumping = True
            self.on_other_player_head = False # Jumping off head
            self.vy = JUMP_POWER
    ...
    def update(self, dt, other_player): # <<< Pass other player for head check
        ...
        landed_on_head = False
        if not self.on_other_player_head: # Only apply gravity if not standing on head
            if self.y < self.base_y or self.vy < 0 or self.is_jumping: # Simplified condition
                 self.vy += GRAVITY
                 self.y += self.vy
        ...
        if self.y >= self.base_y and self.vy >= 0:
             if not self.is_kicking: # Don't interrupt kick landing
                 self.y = self.base_y; self.is_jumping = False; self.vy = 0
                 self.on_other_player_head = False # Can't be on ground and head

        # *** Check landing on OTHER PLAYER'S HEAD ***
        other_head_pos, other_head_radius = other_player.get_head_position_radius()
        # Use player's estimated foot position (bottom) for check
        foot_y_check = self.y # Approximation: player's y is roughly where feet land
        dist_x_head = self.x - other_head_pos[0]
        dist_y_head = foot_y_check - (other_head_pos[1] - other_head_radius) # Check against top of head
        platform_radius = other_head_radius + HEAD_PLATFORM_RADIUS_BUFFER # Effective radius to land on

        # Check if feet are roughly above head and falling onto it
        if abs(dist_x_head) < platform_radius and \
           dist_y_head > -5 and dist_y_head < platform_radius / 2 and \
           self.vy >= 0 and not self.on_other_player_head:

            # Landed on head!
            self.y = other_head_pos[1] - other_head_radius - (self.base_y - self.hip_pos[1]) # Place feet on top
            self.vy = 0
            self.is_jumping = False # Allow jump again
            self.on_other_player_head = True
            landed_on_head = True
            # print(f"Player landed on head!") # Debug

        # If currently on head, but moved off horizontally, start falling
        if self.on_other_player_head and not landed_on_head: # Avoid immediate fall if just landed
             if abs(dist_x_head) >= platform_radius or self.y < other_head_pos[1] - other_head_radius * 2 : # Moved off side or somehow below
                  self.on_other_player_head = False
                  self.is_jumping = True # Treat as falling off a platform
        ...
        if self.is_kicking:
            ...
            if self.kick_timer >= self.kick_duration: self.is_kicking = False; self.kick_timer = 0; self.r_thigh_angle = 0; self.l_thigh_angle = 0; self.r_shin_angle = 0; self.l_shin_angle = 0; self.l_upper_arm_angle = 0; self.r_upper_arm_angle = 0; self.l_forearm_angle = 0; self.r_forearm_angle = 0
             ...
             elif self.is_jumping and not self.on_other_player_head: # Jumping anim only if actually jumping/falling
             ...
             else: # Idle or standing on head
                 self.l_upper_arm_angle = 0; self.r_upper_arm_angle = 0; self.l_forearm_angle = 0; self.r_forearm_angle = 0; self.l_thigh_angle = 0; self.r_thigh_angle = 0; self.l_shin_angle = 0; self.r_shin_angle = 0

        # Calculate Joint Positions (Needs update if y changed due to head stand)
        current_y = self.y; current_x = self.x;
        # Approx visual leg height when standing straight
        total_leg_visual_height = self.thigh_length + self.shin_length
        self.hip_pos = (current_x, current_y - total_leg_visual_height) # Recalculate hip based on potentially adjusted y
        upper_body_x = current_x # No wobble
        self.neck_pos = (upper_body_x, self.hip_pos[1] - self.torso_length); self.head_pos = (upper_body_x, self.neck_pos[1] - self.head_radius); self.shoulder_pos = self.neck_pos
        ...
        self.l_foot_pos = (l_foot_x, l_foot_y); self.r_foot_pos = (r_foot_x, r_foot_y) # Keep calculated positions unless grounded

        # Grounding Logic (Simplified, relies on landing check earlier)
        if not self.is_jumping and not self.is_kicking and not self.on_other_player_head:
            self.y = self.base_y # Ensure snapped if supposed to be on ground
...
player1 = StickMan(SCREEN_WIDTH // 4, GROUND_Y, facing=1)
player2 = StickMan(SCREEN_WIDTH * 3 // 4, GROUND_Y, facing=-1)
# --- Set Player 1 Colors ---
player1.torso_colors = [P1_COLOR_MAIN, P1_COLOR_ACCENT, P1_COLOR_MAIN] # Example: White/Black/White
player1.arm_colors = [P1_COLOR_ACCENT, P1_COLOR_MAIN] # Example: Black upper, White lower
player1.leg_colors = [P1_COLOR_MAIN, P1_COLOR_ACCENT] # Example: White thigh, Black shin
# --- End Player 1 Colors ---
...
# Removed combo score tracking variables
...
current_hit_count = 0 # Still needed for particle effect trigger
...
def reset_after_goal():
    ...
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0; player1.is_kicking = False; player1.is_charging = False; player1.facing_direction = 1; player1.on_other_player_head = False
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0; player2.is_kicking = False; player2.is_charging = False; player2.facing_direction = -1; player2.on_other_player_head = False
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 kick_x = kick_x_base * KICK_FORCE_LEVEL; kick_y = BASE_KICK_FORCE_Y * KICK_FORCE_LEVEL
...
while running:
    ...
    # Updates (Pass other player to each update for head check)
    ...
    if p1_rect.colliderect(p2_rect):
        ...
        if overlap > 0:
            push = overlap / 2 + 0.1;
            if dx >= 0: player1.x -= push; player2.x += push # >= to handle exact overlap
            else: player1.x += push; player2.x -= push
            player1.vx *= -0.1; player2.vx *= -0.1

    # --- Goal Detection & Goal Particle Effect ---
    goal_scored = False
    goal_pos_x = -1 # Invalid default
    if ball.x + ball.radius >= GOAL_LINE_X_RIGHT and ball.y > GOAL_Y_POS:
        player1_score += 1; goal_message_timer = GOAL_MESSAGE_DURATION; goal_scored = True
        goal_pos_x = SCREEN_WIDTH # Goal on right
        print(f"GOAL! Player 1 Score: {player1_score}")
    elif ball.x - ball.radius <= GOAL_LINE_X_LEFT and ball.y > GOAL_Y_POS:
        player2_score += 1; goal_message_timer = GOAL_MESSAGE_DURATION; goal_scored = True
        goal_pos_x = 0 # Goal on left
        print(f"GOAL! Player 2 Score: {player2_score}")

    if goal_scored:
        # Create Goal Explosion Particles
        goal_center_y = GOAL_Y_POS + GOAL_HEIGHT / 2
        for _ in range(GOAL_PARTICLE_COUNT):
            particles.append(Particle(goal_pos_x, goal_center_y,
                                      colors=GOAL_EXPLOSION_COLORS,
                                      speed_min=GOAL_PARTICLE_SPEED_MIN,
                                      speed_max=GOAL_PARTICLE_SPEED_MAX,
    ...
    # Trigger Star Explosion (Based on combo count)
