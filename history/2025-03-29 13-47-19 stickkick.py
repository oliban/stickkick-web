...
ROCKET_EXPLOSION_FORCE = 65.0
ROCKET_EXPLOSION_COLOR = (255, 100, 0, 180)
GUN_COLOR = (70, 80, 90)
GUN_SIZE = (25, 8)
GUN_ANIM_SPEED = 3.0
GUN_ANIM_MAGNITUDE = 0.3
LASER_COLOR = (255, 0, 0, 180) # <<< Laser Red, slightly transparent
LASER_LENGTH = 400 # <<< Increased laser length
LASER_WIDTH = 1
...
# Player Impact logic is inside create_explosion
def create_explosion(x, y, radius, players, ball): # Applies force to players and ball
    ...
    play_sound(loaded_sounds['wall_hit']) # Re-using sound, consider adding specific explosion sound

    # Apply force to players
    for p in players:
        dist_sq = (p.x - x)**2 + (p.y - y)**2 # Using player x/y directly
        if dist_sq < radius**2 and dist_sq > 0: # Check if within radius and not exactly at center
            dist = math.sqrt(dist_sq)
            force_magnitude = ROCKET_EXPLOSION_FORCE * (1.0 - (dist / radius)) # Force decreases with distance
            push_vec_x = (p.x - x) / dist # Normalized direction vector away from explosion center
            push_vec_y = (p.y - y) / dist
            # Apply force to player's velocity
            p.vx += push_vec_x * force_magnitude
            p.vy += push_vec_y * force_magnitude * 0.8 - 5 # Added upward boost makes it feel more 'explosive'

    # Apply force to ball (same logic as before)
    ...
    active_explosions.append(Explosion(x, y, radius)) # Create visual effect

class StickMan: # ... (no change to __init__ or most methods) ...
    ...
    def draw(self, screen): # ... (no change) ...
        ...
        nose_tip_x = head_center_x + (self.head_radius * 0.5 + self.current_nose_length) * self.facing_direction; nose_tip_y = head_center_y + self.head_radius * 0.1
        nose_base_x = head_center_x + (self.head_radius * 0.3) * self.facing_direction; nose_base_y1 = nose_tip_y - self.current_nose_width / 2; nose_base_y2 = nose_tip_y + self.current_nose_width / 2
        ...
        if self.current_powerup == "FLIGHT":
            upper_attach_x = self.shoulder_pos[0]; upper_attach_y = self.shoulder_pos[1] + 5; lower_attach_x = self.hip_pos[0]; lower_attach_y = self.hip_pos[1] - 5
            ...
            l_attach_upper = (upper_attach_x - 4, upper_attach_y); l_attach_lower = (lower_attach_x - 4, lower_attach_y); l_upper_poly = create_wing_poly(l_attach_upper, self.l_wing_upper_angle, upper_length, upper_width)
            l_lower_poly = create_wing_poly(l_attach_lower, self.l_wing_lower_angle, lower_length, lower_width); pygame.draw.polygon(screen, self.wing_color, l_upper_poly); pygame.draw.polygon(screen, self.wing_outline_color, l_upper_poly, 1)
            pygame.draw.polygon(screen, self.wing_color, l_lower_poly); pygame.draw.polygon(screen, self.wing_outline_color, l_lower_poly, 1); r_attach_upper = (upper_attach_x + 4, upper_attach_y); r_attach_lower = (lower_attach_x + 4, lower_attach_y)
            r_upper_poly = create_wing_poly(r_attach_upper, self.r_wing_upper_angle, upper_length, upper_width); r_lower_poly = create_wing_poly(r_attach_lower, self.r_wing_lower_angle, lower_length, lower_width)
            pygame.draw.polygon(screen, self.wing_color, r_upper_poly); pygame.draw.polygon(screen, self.wing_outline_color, r_upper_poly, 1); pygame.draw.polygon(screen, self.wing_color, r_lower_poly); pygame.draw.polygon(screen, self.wing_outline_color, r_lower_poly, 1)
        if self.current_powerup == "ROCKET_LAUNCHER":
            gun_attach_x = self.shoulder_pos[0] + 5 * self.facing_direction; gun_attach_y = self.shoulder_pos[1] + 10
            ...
            tip_offset = GUN_SIZE[0] / 2; self.gun_tip_pos = (gun_center_x + tip_offset * math.cos(gun_world_angle), gun_center_y + tip_offset * math.sin(gun_world_angle))
            laser_end_x = self.gun_tip_pos[0] + LASER_LENGTH * math.cos(gun_world_angle); laser_end_y = self.gun_tip_pos[1] + LASER_LENGTH * math.sin(gun_world_angle)
            try:
                 laser_surf = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
                 pygame.draw.aaline(laser_surf, LASER_COLOR, self.gun_tip_pos, (laser_end_x, laser_end_y)) # Use aaline for smoother line
...
while running:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_7: debug_mode = not debug_mode; print(f"Debug Mode {'ACT' if debug_mode else 'DEACT'}IVATED")
                ...
                if not powerup.active and match_active:
                    ...
                    powerup.spawn()
                    powerup_spawn_timer = random.uniform(POWERUP_SPAWN_INTERVAL_MIN, POWERUP_SPAWN_INTERVAL_MAX)
                else: print("DEBUG: Powerup already active or match inactive, cannot force spawn.")
    ...
    # --- Power-up Spawning ---
    if not powerup.active and match_active: # ... (unchanged) ...
    ...
    if match_active: # ... (unchanged - rocket update handles its own collisions) ...
        ...
        # Update rockets and remove inactive/exploded ones
        ...
        active_explosions = [e for e in active_explosions if e.active and e.update(dt)] # Need to call update first
        if powerup.active:
    ...
    # --- Drawing ---
    bg_color = SKY_BLUE # ... (background drawing unchanged) ...
