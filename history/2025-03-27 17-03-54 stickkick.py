...
GOAL_DEPTH_X = 30; GOAL_DEPTH_Y = -15 # Negative Y depth makes back top higher
...
# --- Goal Drawing Function (Revised for Clarity) ---
def draw_goal_isometric(surface, goal_line_x, goal_y, goal_height, depth_x_offset, depth_y, thickness, post_color, net_color):
    # Note: depth_x_offset should be negative for left goal, positive for right goal relative to goal_line_x
    front_top = (goal_line_x, goal_y)
    front_bottom = (goal_line_x, goal_y + goal_height)

    # Calculate back position based directly on the offset passed
    back_x = goal_line_x + depth_x_offset # e.g., 1 + (-30) = -29 for left; 799 + 30 = 829 for right (offscreen ok)
    ...
    # Draw the posts and top/bottom bars
    pygame.draw.line(surface, post_color, bt_int, bb_int, thickness) # Back Post
    pygame.draw.line(surface, post_color, bt_int, ft_int, thickness) # Top Bar
    pygame.draw.line(surface, post_color, bb_int, fb_int, thickness) # Bottom Bar (on ground)
    pygame.draw.line(surface, post_color, ft_int, fb_int, thickness) # Front Post

    # Draw simple net lines (crosses)
    pygame.draw.line(surface, net_color, ft_int, bb_int, 1) # Front top to back bottom
    pygame.draw.line(surface, net_color, fb_int, bt_int, 1) # Front bottom to back top
...
class StickMan:
    ...
    def move(self, direction):
        # Allow setting horizontal velocity even when on head
        if not self.is_kicking:
            self.vx = direction * PLAYER_SPEED
            if direction != 0: self.facing_direction = direction
    def stop_move(self):
        # Allow stopping horizontal velocity even when on head
        self.vx = 0
    def jump(self):
    ...
    def start_kick(self):
        if not self.is_kicking:
            self.is_kicking = True; self.kick_timer = 0; self.vx = 0 # Stop intended horizontal movement
    ...
    def update(self, dt, other_player):
        ...
        # --- Update horizontal position (MODIFIED FOR HEAD MOVEMENT) ---
        intended_vx = self.vx # Velocity set by player input (move/stop_move)

        if not self.is_kicking:
            # Calculate the actual velocity to apply for this frame's movement
            effective_vx = intended_vx
            if self.on_other_player_head:
                # Add the velocity of the player underneath
                effective_vx += other_player.vx

            # Apply the combined velocity for position update
            self.x += effective_vx # Use effective_vx here
            # Clamp position to screen bounds
            self.x = max(self.limb_width / 2, min(self.x, SCREEN_WIDTH - self.limb_width / 2))
        # If kicking, intended_vx is 0. effective_vx will be 0 or other_player.vx
        # self.vx retains the intended value for animation below


        # --- Animation Logic (Use intended_vx for walking check) ---
        # Walk Cycle check uses the *intended* velocity (player input), not the effective one
        ...
        # Kick Animation (No change needed here)
        if self.is_kicking:
            ...
            if self.kick_timer >= self.kick_duration: self.is_kicking = False; self.kick_timer = 0;
             ...
             if is_walking: # Walk animation uses intended_vx via is_walking flag
             ...
             elif self.is_jumping and not self.on_other_player_head: # Jumping anim
             ...
             else: # Idle or standing on head pose
...
while running:
    ...
    # 1. Update players (handles vertical physics, head standing state, animation angles, HORIZONTAL position based on state)
    ...
    # Resolve horizontal overlap *after* vertical states and positions are set
    p1_rect = player1.get_body_rect()
    p2_rect = player2.get_body_rect()

    if p1_rect.colliderect(p2_rect):
        # --- Check Vertical Relationship (using state set in update) ---
        p1_is_on_p2 = player1.on_other_player_head
        p2_is_on_p1 = player2.on_other_player_head
        # Add extra proximity check just in case state is slightly off during rapid movement
        if p1_is_on_p2 and abs(player1.y - (player2.head_pos[1] - player2.head_radius)) > 10: p1_is_on_p2 = False # Too far vertically
        if p2_is_on_p1 and abs(player2.y - (player1.head_pos[1] - player1.head_radius)) > 10: p2_is_on_p1 = False # Too far vertically
        ...
        if not p1_is_on_p2 and not p2_is_on_p1:
            ...
            if overlap_x > 0:
                ...
                if player1.x < player2.x:
                    player1.x -= push
                    player2.x += push
                    if player1.vx > 0: player1.vx = 0 # Stop intended move if pushed back
                    ...
                    if player2.vx > 0: player2.vx = 0
        # Else (head-stand): Do nothing horizontally here. Movement is handled by effective_vx in update().
    ...
    # --- Draw Goals (Using updated function call logic) ---
    # Pass negative depth offset for left goal, positive for right goal
