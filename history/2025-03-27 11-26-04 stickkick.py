...
class StickMan:
    ...
    def get_kick_impact_point(self):
        if self.is_kicking: impact_start = 0.25; impact_end = 0.6; progress = self.kick_timer / self.kick_duration;
    ...
    # Draw Method (Corrected indentation)
    def draw(self, screen):
...
# --- Ball Class (Corrected update method) ---
class Ball:
    ...
    def update(self, dt):
        ...
        if self.y + self.radius >= GROUND_Y:
            if self.vy >= 0: # Only bounce/stop if moving downwards or resting
                 hit_ground = True
            self.y = GROUND_Y - self.radius # Snap back above ground
            self.vy *= -BALL_BOUNCE       # Reverse and dampen vertical velocity
            self.vx *= 0.9                # Apply friction on bounce
            if abs(self.vy) < 1:         # If bounce is very small, stop bouncing
                self.vy = 0

        # Wall collisions
        if self.x + self.radius >= SCREEN_WIDTH:
            self.x = SCREEN_WIDTH - self.radius
            self.vx *= -BALL_BOUNCE * 0.8 # Bounce off walls slightly less
        elif self.x - self.radius <= 0:
            self.x = self.radius
            self.vx *= -BALL_BOUNCE * 0.8

        # Stop rolling on ground if slow
        if abs(self.vx) < 0.1 and self.is_on_ground(): # Use is_on_ground helper
            self.vx = 0
...
while running:
    ...
    if kick_point:
        ...
        if dist_sq < (ball.radius + effective_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6: # Use updated impact window
