...
PLAYER_BODY_COLLISION_FRAMES = 4 # Cooldown frames after a body collision bounce
...
player_body_collision_timer = 0 # <<< Initialize body collision cooldown timer
...
while running:
    ...
    # --- Update Cooldowns ---
    if player_body_collision_timer > 0:
        player_body_collision_timer -= 1
    ...
    if not kick_performed_this_frame and not headbutt_performed_this_frame and player_body_collision_timer == 0: # <<< Check cooldown
        ...
        delta_x = ball.x - closest_x; delta_y = ball.y - closest_y
        ...
        if dist_sq_body < ball.radius**2 and dist_sq_body > 0:
            ...
            # Push-out slightly more
            push_amount = overlap + 0.2 # <<< Increased buffer
            ...
            if vel_along_normal < 0:
                ...
                player_body_collision_timer = PLAYER_BODY_COLLISION_FRAMES # <<< Set cooldown

        elif dist_sq_body == 0:
             ball.y = player_rect.top - ball.radius - 0.1
             if ball.vy > 0: ball.vy *= -PLAYER_BODY_BOUNCE
             player_body_collision_timer = PLAYER_BODY_COLLISION_FRAMES # <<< Set cooldown even for center overlap
    # --- END BODY COLLISION ---
