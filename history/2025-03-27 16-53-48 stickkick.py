...
class StickMan:
    ...
    def update(self, dt, other_player):
        ...
        can_land_on_head = abs(dist_x_head) < platform_radius and self.y >= head_top_y - 5

        if can_land_on_head: platform_y = head_top_y

        next_y = self.y # Start with current y
        if not self.on_other_player_head: # Apply gravity if not on head
            self.vy += GRAVITY
            next_y += self.vy # Update potential next y
        ...
        if next_y >= platform_y and self.vy >= 0:
            if can_land_on_head and platform_y == head_top_y: # Landed on head
                self.y = platform_y # Place feet exactly on platform
                self.vy = 0; self.is_jumping = False; self.on_other_player_head = True; landed_on_head_this_frame = True
            ...
            else: # Still falling, but haven't hit the determined platform yet
                 self.y = next_y
        # Handle falling off a head
        elif self.on_other_player_head and not can_land_on_head:
             self.on_other_player_head = False; self.is_jumping = True; self.y = next_y
...
while running:
    ...
    if screen_flash_timer > 0: screen_flash_timer -= dt # <<< Update flash timer
    ...
    # Player-Player Collision (MODIFIED BLOCK)
    p1_rect = player1.get_body_rect()
    p2_rect = player2.get_body_rect()
    if p1_rect.colliderect(p2_rect):
        # Calculate horizontal overlap more directly using rect edges
        overlap_x = 0
        ...
        # Only resolve if they are actually overlapping horizontally
        if overlap_x > 0:
            push = overlap_x / 2 + 0.1 # Push amount based on the calculated overlap

            # Apply positional correction - push them apart horizontally
            if player1.x < player2.x: # p1 is left, push p1 left, p2 right
                player1.x -= push
                player2.x += push
            else: # p1 is right, push p1 right, p2 left
                player1.x += push
                player2.x -= push

            # --- VELOCITY MODIFICATION REMOVED ---
            # By removing the vx *= -0.1 lines, we eliminate the artificial bounce impulse.
            # The positional push handles the separation. Players will just stop
            # moving towards each other horizontally when they collide.
            # --- ---

            # Optional: If simply stopping feels too abrupt, you could uncomment
            # the lines below to slightly dampen their velocities upon collision,
            # but avoid the negative multiplier.
            # player1.vx *= 0.5 # Example: Halve velocity on impact
            # player2.vx *= 0.5 # Example: Halve velocity on impact
    ...
    if ball.x + ball.radius >= GOAL_LINE_X_RIGHT and ball.y > GOAL_Y_POS:
        ...
        screen_flash_timer = SCREEN_FLASH_DURATION # <<< Trigger flash
    ...
    if screen_flash_timer > 0:
        ...
        flash_alpha = int(255 * (screen_flash_timer / SCREEN_FLASH_DURATION)) # Fade out
    ...
    # Draw Scoreboard (using new function)
