...
class StickMan: # Added wings
    def __init__(self, x, y, facing=1):
        ...
        self.l_wing_angle = math.pi * 0.6; self.r_wing_angle = math.pi * 0.4
        self.wing_flap_timer = 0.0; self.wing_flap_duration = 0.25; self.wing_flapping = False
        self.wing_upper_lobe_size = (20, 15); self.wing_lower_lobe_size = (18, 18)
    ...
    def jump(self): # Conditional mid-air jump based on FLIGHT mode
        can_jump_now = False
        if flight_mode_active: # Use flight_mode_active global
    ...
    def draw(self, screen):
        ...
        # --- Draw Triangle Nose ---
        nose_length = self.head_radius * 0.7; nose_width = self.head_radius * 0.4
        tip_x = head_center_int[0] + nose_length * self.facing_direction; tip_y = head_center_int[1]
        base_y1 = head_center_int[1] - nose_width / 2; base_y2 = head_center_int[1] + nose_width / 2
        base_x = head_center_int[0] + self.head_radius * 0.3 * self.facing_direction
        nose_points = [(base_x, base_y1), (tip_x, tip_y), (base_x, base_y2)]
        pygame.draw.polygon(screen, NOSE_COLOR, nose_points)
        # --- End Nose ---
        draw_pentagon(screen, BLACK, head_center_int, self.head_radius * 0.6, angle=0.1); draw_pentagon(screen, BLACK, head_center_int, self.head_radius * 0.3, angle=math.pi/5 + 0.1); pygame.draw.circle(screen, BLACK, head_center_int, self.head_radius, 1)
        # --- Draw Torso/Limbs ---
        ...
        # --- Draw Wings (Only in Flight Mode) ---
        if flight_mode_active:
            ...
            def draw_rotated_ellipse(target_surface, color, outline_color, center, width, height, angle_rad):
                ...
                max_dim = int(math.hypot(width, height) * 1.5) + 2
                try:
                    size = max_dim * 2
                    ellipse_surf = pygame.Surface((size, size), pygame.SRCALPHA)
                except ValueError: print(f"Warning: Invalid size for ellipse surface ({size}x{size})"); return
                ellipse_rect = pygame.Rect(size//2 - width, size//2 - height, width * 2, height * 2)
                ...
                rotated_rect = rotated_surf.get_rect(center=(int(center[0]), int(center[1])))
                target_surface.blit(rotated_surf, rotated_rect)
            l_attach = (wing_attach_x - self.limb_width * 0.2, wing_attach_y)
            upper_lobe_dist = self.wing_upper_lobe_size[0] * 0.4; lower_lobe_dist = self.wing_upper_lobe_size[0] * 0.3
            l_upper_angle = self.l_wing_angle - 0.3; l_upper_cx = l_attach[0] + upper_lobe_dist * math.cos(l_upper_angle); l_upper_cy = l_attach[1] + upper_lobe_dist * math.sin(l_upper_angle)
            draw_rotated_ellipse(screen, self.wing_color, self.wing_outline_color, (l_upper_cx, l_upper_cy), self.wing_upper_lobe_size[0], self.wing_upper_lobe_size[1], l_upper_angle)
            l_lower_angle = self.l_wing_angle + 0.4; l_lower_cx = l_attach[0] + lower_lobe_dist * math.cos(l_lower_angle); l_lower_cy = l_attach[1] + lower_lobe_dist * math.sin(l_lower_angle)
            draw_rotated_ellipse(screen, self.wing_color, self.wing_outline_color, (l_lower_cx, l_lower_cy), self.wing_lower_lobe_size[0], self.wing_lower_lobe_size[1], l_lower_angle)
            r_attach = (wing_attach_x + self.limb_width * 0.2, wing_attach_y)
            r_upper_angle = self.r_wing_angle + 0.3; r_upper_cx = r_attach[0] + upper_lobe_dist * math.cos(r_upper_angle); r_upper_cy = r_attach[1] + upper_lobe_dist * math.sin(r_upper_angle)
            draw_rotated_ellipse(screen, self.wing_color, self.wing_outline_color, (r_upper_cx, r_upper_cy), self.wing_upper_lobe_size[0], self.wing_upper_lobe_size[1], r_upper_angle)
            r_lower_angle = self.r_wing_angle - 0.4; r_lower_cx = r_attach[0] + lower_lobe_dist * math.cos(r_lower_angle); r_lower_cy = r_attach[1] + lower_lobe_dist * math.sin(r_lower_angle)
            draw_rotated_ellipse(screen, self.wing_color, self.wing_outline_color, (r_lower_cx, r_lower_cy), self.wing_lower_lobe_size[0], self.wing_lower_lobe_size[1], r_lower_angle)

# --- Ball Class ---
class Ball: # ... (Updated update method for bounce sound) ...
...
clock = pygame.time.Clock() # Defined before main loop
...
flight_mode_active = False # Initialize flight mode
...
while running:
    ...
    if match_over_timer > 0:
        ...
        # Sound queue plays independently via events/main loop check
        if match_over_timer <= 0 and not game_over:
             start_new_match()


    # --- Process Announcement Sound Queue (Runs every frame) ---
