...
class StickMan: # Added wing attributes and logic
    def __init__(self, x, y, facing=1):
        ...
        self.wing_upper_lobe_size = (30, 22); self.wing_lower_lobe_size = (18, 18) # Adjusted upper lobe size
        # --- Head Feature Colors ---
        self.eye_color = BLACK
        if P1_COLOR_MAIN == (220, 220, 220): self.cap_color = (50, 50, 50)
    ...
    def update(self, dt, other_player):
        ...
        wing_rest_angle_offset = self.wing_rest_angle_offset; wing_flap_down_angle_offset = math.pi * 0.4
    ...
    def draw(self, screen): # Updated head and wings
        ...
        eye_offset_x = self.head_radius * 0.35 * self.facing_direction; eye_offset_y = -self.head_radius * 0.1; eye_radius = 3 # Bigger eyes
        ...
        nose_length = self.head_radius * 0.8; nose_width = self.head_radius * 0.5 # Bigger nose
        tip_x = head_center_x + nose_length * self.facing_direction; tip_y = head_center_y + self.head_radius * 0.2
        base_y1 = tip_y - nose_width / 2; base_y2 = tip_y + nose_width / 2; base_x = head_center_x + 2 * self.facing_direction
        nose_points = [(int(base_x), int(base_y1)), (int(tip_x), int(tip_y)), (int(base_x), int(base_y2))]; pygame.draw.polygon(screen, NOSE_COLOR, nose_points)
        ...
        if flight_mode_active:
            ...
            def create_wing_poly(attach_point, base_angle, length, width, angle_offset):
                ...
                p1 = attach_point
                p2 = (attach_point[0] + length * 0.8 * cos_a + width*0.5 * cos_w, attach_point[1] + length * 0.8 * sin_a + width*0.5 * sin_w)
                p3 = (attach_point[0] + length * cos_a, attach_point[1] + length * sin_a)
                p4 = (attach_point[0] + length * 0.8 * cos_a - width*0.5 * cos_w, attach_point[1] + length * 0.8 * sin_a - width*0.5 * sin_w)
            ...
            l_attach = (wing_attach_x - 4, wing_attach_y)
            ...
            r_attach = (wing_attach_x + 4, wing_attach_y)
...
class Ball: #... (as before) ...
    ...
    def update(self, dt): # ... (Updated bounce sound logic) ...
...
while running:
    ...
    # --- Process Announcement Sound Queue (Relies on event system) ---
    # This call removed from main loop
