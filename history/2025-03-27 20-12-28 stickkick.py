...
# --- Draw Past Game Scores (Corrected Order, No Numbers) ---
def draw_game_scores(surface, scores_list, font):
    start_x = 10
    start_y = 10 # Start position
    current_y = start_y

    # Iterate in reverse to show oldest game first (game_scores[0] is newest)
    for score_tuple in reversed(scores_list):
        score_str = f"{score_tuple[0]} - {score_tuple[1]}" # Just the score
        score_surf = font.render(score_str, True, TEXT_COLOR)
        score_rect = score_surf.get_rect(topleft=(start_x, current_y)) # Align left
        surface.blit(score_surf, score_rect)
        current_y += score_rect.height + 1 # Line spacing
...
def draw_trophy(surface, winner_name, title_font, name_font):
    ...
    trophy_base_y = SCREEN_HEIGHT // 2 + 180 # <<< MOVED DOWN
...
def load_sounds(sound_dir="sounds"):
    sounds = {}
    sound_files = {
        "kick": ["kick_ball1.wav", "kick_ball2.wav"], "jump": ["jump1.wav"], "land": ["land1.wav"],
        "wall_hit": ["wall_hit1.wav"], "player_bump": ["player_bump1.wav"], "headbutt": ["headbutt1.wav"],
        "body_hit": ["body_hit1.wav"], "combo": ["combo_sparkle1.wav", "combo_sparkle2.wav", "combo_sparkle3.wav", "combo_sparkle4.wav"],
...
while running:
    ...
    if match_active: # Only check goals if match is active
    ...
    if goal_scored_this_frame: # Process goal consequences
        # Play sounds
    ...
    # --- Draw UI ---
    draw_scoreboard(screen, player1_score, player2_score, font_large, font_medium, goal_message_timer > 0 or match_over_timer > 0) # Flash score on goal OR match over
    draw_game_scores(screen, game_scores, font_small)
