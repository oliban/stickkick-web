...
MAX_ACTIVE_POWERUPS = 3 # Limit concurrent powerups
...
ROCKET_EXPLOSION_FORCE = 110.0 # <<< Significantly Increased explosion force for players
ROCKET_PLAYER_UPWARD_BOOST = 10.0 # <<< Increased upward boost for players hit
...
LASER_COLOR_HEX = "#C70E20" # <<< Specific red color
LASER_ALPHA = 230 # <<< Increased alpha slightly
LASER_COLOR = pygame.Color(LASER_COLOR_HEX)
LASER_COLOR.a = LASER_ALPHA
LASER_LENGTH = 200 # <<< Kept shorter laser
LASER_WIDTH = 1

# --- Player Tumbling Constants ---
PLAYER_TUMBLE_ROT_SPEED_MIN = 15.0 # Increased min rot speed
PLAYER_TUMBLE_ROT_SPEED_MAX = 25.0 # Increased max rot speed
PLAYER_TUMBLE_DAMPING = 0.985 # Slightly less damping
...
class ParachutePowerup: # ... (no change) ...
    def __init__(self):
        self.x = -100; self.y = -100; self.vy = POWERUP_DESCEND_SPEED; self.vx = 0; self.width, self.height = POWERUP_BOX_SIZE; self.active = False; self.chute_radius = 35; self.powerup_type = None; self.id = random.randint(1, 1000000)
    def spawn(self):
        self.active = True; self.powerup_type = random.choice(POWERUP_TYPES); self.x = random.randint(GOAL_MARGIN_X + 50, SCREEN_WIDTH - GOAL_MARGIN_X - 50); self.y = -self.chute_radius * 2; self.vx = random.uniform(-POWERUP_DRIFT_SPEED, POWERUP_DRIFT_SPEED); print(f"Powerup spawned: {self.powerup_type} at ({self.x:.0f}, {self.y:.0f})")
    def update(self, dt):
    ...
    def get_box_rect(self): return pygame.Rect(self.x - self.width / 2, self.y, self.width, self.height)
    def check_collision(self, player):
        ...
        if player_rect.colliderect(powerup_box_rect):
            print(f"Powerup collected: {self.powerup_type}"); play_sound(loaded_sounds['combo']); return self.powerup_type
        return None
    def draw(self, screen):
        if not self.active: return
        box_rect = self.get_box_rect(); pygame.draw.rect(screen, POWERUP_BOX_COLOR, box_rect); pygame.draw.rect(screen, BLACK, box_rect, 1); chute_center_x = int(self.x); chute_top_y = int(self.y - self.chute_radius); chute_rect = pygame.Rect(0, 0, self.chute_radius * 2, self.chute_radius * 1.5); chute_rect.center = (chute_center_x, chute_top_y); num_panels = 6; angle_step = math.pi / num_panels
        for i in range(num_panels):
            angle1 = math.pi + i * angle_step; angle2 = math.pi + (i + 1) * angle_step; color = POWERUP_CHUTE_ALT_COLOR if i % 2 == 0 else POWERUP_CHUTE_COLOR
        ...
        pygame.draw.ellipse(screen, BLACK, chute_rect, 1); string_points = [(box_rect.left + 3, box_rect.top), (box_rect.right - 3, box_rect.top), (box_rect.centerx, box_rect.top)]; chute_bottom_center = (chute_center_x, chute_top_y + int(self.chute_radius * 0.75))
...
class Rocket: # ... (no change) ...
    def __init__(self, x, y, vx, vy, owner_player):
        self.x = x; self.y = y; self.vx = vx; self.vy = vy; self.width, self.height = ROCKET_SIZE; self.owner = owner_player; self.active = True; self.angle = math.atan2(vy, vx)
    def update(self, dt, players, ball):
        if not self.active: return False
        self.x += self.vx * dt; self.y += self.vy * dt; self.angle = math.atan2(self.vy, self.vx); exploded = False
        if self.x < -self.width or self.x > SCREEN_WIDTH + self.width or self.y < -self.height or self.y > SCREEN_HEIGHT + self.height: self.active = False; return False
        if self.y + self.height / 2 > GROUND_Y: self.y = GROUND_Y - self.height / 2; exploded = True
        ...
        for p in players:
            ...
            if not exploded:
                head_pos, head_radius = p.get_head_position_radius(); dist_sq_head = (rocket_center_x - head_pos[0])**2 + (rocket_center_y - head_pos[1])**2
        ...
        if exploded: self.active = False; create_explosion(self.x, self.y, ball.radius * ROCKET_BLAST_RADIUS_FACTOR, players, ball); return True
        return False
    def get_rect(self): return pygame.Rect(self.x - max(self.width, self.height)/2, self.y - max(self.width, self.height)/2, max(self.width, self.height), max(self.width, self.height))
    def draw(self, screen):
        if not self.active: return
        draw_rotated_rectangle(screen, ROCKET_COLOR, (self.x, self.y), self.width, self.height, self.angle); flame_length = random.uniform(5, 10); flame_angle = self.angle + math.pi
        flame_x = self.x + math.cos(flame_angle) * (self.width / 2 + 2); flame_y = self.y + math.sin(flame_angle) * (self.width / 2 + 2); end_flame_x = flame_x + math.cos(flame_angle) * flame_length; end_flame_y = flame_y + math.sin(flame_angle) * flame_length
...
class Explosion: # ... (no change) ...
    def __init__(self, x, y, max_radius):
        self.x = x; self.y = y; self.max_radius = max_radius; self.duration = ROCKET_EXPLOSION_DURATION; self.timer = self.duration; self.active = True; self.current_radius = 0
    def update(self, dt):
        if not self.active: return False
        self.timer -= dt
        if self.timer <= 0: self.active = False; return False
        progress = 1.0 - (self.timer / self.duration); self.current_radius = self.max_radius * math.sin(progress * math.pi); return True
    def draw(self, screen):
        if not self.active or self.current_radius <= 0: return
        temp_surf = pygame.Surface((self.current_radius * 2, self.current_radius * 2), pygame.SRCALPHA); pygame.draw.circle(temp_surf, ROCKET_EXPLOSION_COLOR, (self.current_radius, self.current_radius), int(self.current_radius)); screen.blit(temp_surf, (int(self.x - self.current_radius), int(self.y - self.current_radius)))

# Applies force and starts tumbling
def create_explosion(x, y, radius, players, ball): # <<< Increased force application
    global active_explosions
    print(f"Explosion created at ({x:.0f}, {y:.0f}) with radius {radius:.0f}")
    play_sound(loaded_sounds['wall_hit']) # TODO: Add specific explosion sound

    # Apply force to players
    for p in players:
        dist_sq = (p.x - x)**2 + (p.get_body_rect().centery - y)**2 # Use body center y for distance calc
        if dist_sq < radius**2 and dist_sq > 0.1: # Avoid division by zero if exactly at center
            dist = math.sqrt(dist_sq)
            # Apply stronger force closer to the center
            force_scale = (1.0 - (dist / radius))**1.5 # Non-linear falloff
            force_magnitude = ROCKET_EXPLOSION_FORCE * force_scale

            push_vec_x = (p.x - x) / dist
            push_vec_y = (p.get_body_rect().centery - y) / dist # Use body center for direction

            # Apply impulse (change in velocity)
            p.vx += push_vec_x * force_magnitude
            p.vy += push_vec_y * force_magnitude * 0.8 - ROCKET_PLAYER_UPWARD_BOOST # Apply upward boost

            print(f"  Applying force {force_magnitude:.1f} to player ({p.vx:.1f}, {p.vy:.1f})")
            p.start_tumble() # Start tumbling state

    # Apply force to ball
    dist_sq = (ball.x - x)**2 + (ball.y - y)**2
    if dist_sq < radius**2 and dist_sq > 0.1:
        dist = math.sqrt(dist_sq); force_scale = (1.0 - (dist / radius))**1.5
        force_magnitude = ROCKET_EXPLOSION_FORCE * force_scale # Ball also affected strongly
...
class StickMan: # ... (no change to most methods, only update and draw) ...
    def __init__(self, x, y, facing=1):
        ...
        self.current_powerup = None; self.is_flying = False; self.flight_timer = 0.0
    ...
    def start_tumble(self): #... (no change) ...
        if not self.is_tumbling:
             ...
             self.is_kicking = False; self.kick_timer = 0; self.vx *= 0.5 # Dampen current horizontal speed slightly
             print(f"Player {1 if self.facing_direction==1 else 2} starting tumble! Vel: ({self.vx:.1f}, {self.vy:.1f}) RotVel: {self.rotation_velocity:.1f}")
    ...
    def jump(self): # ... (no change) ...
        if self.is_tumbling: return
        can_jump_now = False
        if self.current_powerup == "FLIGHT":
    ...
    def update(self, dt, other_player): # Simplified horizontal physics while tumbling
        if self.is_tumbling:
            self.tumble_timer -= dt
            if self.tumble_timer <= 0:
                self.is_tumbling = False; self.tumble_timer = 0.0; self.rotation_angle = 0.0; self.rotation_velocity = 0.0
                print(f"Player {1 if self.facing_direction==1 else 2} finished tumble.")
            else:
        ...
        if self.current_powerup == "FLIGHT":
            ...
            if self.flight_timer <= 0: self.current_powerup = None; self.is_flying = False; self.flight_timer = 0; print("Flight ended")
        else: self.is_flying = False
        if self.current_powerup == "ROCKET_LAUNCHER":
        ...
        # Vertical Physics (Apply gravity first)
        self.vy += GRAVITY
        next_y = self.y + self.vy # Calculate potential next Y

        # Landing / Vertical Collision checks
        if self.vy >= 0: # Only check landing if moving downwards or still
            ...
            if can_land_on_head_now:
                ...
                if self.is_tumbling: self.vy = -abs(self.vy * 0.3) # Bounce slightly if tumbling
            elif next_y >= self.base_y: # Hit ground
                self.y = self.base_y; self.vy = 0; self.is_jumping = False; self.on_other_player_head = False; landed_on_ground_this_frame = True
                if self.is_tumbling: self.rotation_velocity *= 0.7; self.vx *= 0.7 # Dampen significantly on ground
            else: # Still falling
                 self.y = next_y; self.on_other_player_head = False
        else: # Moving upwards
             self.y = next_y; self.on_other_player_head = False # Cannot be on head if moving up

        # Fall off head check
        if was_on_head and not landed_on_head_this_frame and not is_aligned_for_head:
             self.on_other_player_head = False; self.is_jumping = True # Start jumping state if fell off

        # Sound for landing (only if not tumbling)
        ...
        # Horizontal Physics (Apply vx)
        # Use self.vx directly, as input is disabled during tumble
        # Apply slight air resistance/friction if tumbling
        if self.is_tumbling:
             self.vx *= 0.99 # Air resistance while tumbling
        self.x += self.vx # Apply horizontal velocity (assuming vx is pixels per frame)
        self.x = max(self.limb_width / 2, min(self.x, SCREEN_WIDTH - self.limb_width / 2)) # Clamp X

        # --- Wing Animation Update ---
        # ... (wing animation unchanged) ...
        if self.current_powerup == "FLIGHT":
        ...
        # --- Body Animation ---
        if not self.is_tumbling: # <<< Normal animations only if not tumbling
        ...
        else: # --- Tumbling Animation ---
            tumble_speed = self.rotation_velocity * 1.5
            self.l_upper_arm_angle = math.sin(time_ms * 0.01 + 1) * 0.8 + tumble_speed * 0.05
            self.r_upper_arm_angle = math.sin(time_ms * 0.01 + 2) * 0.8 - tumble_speed * 0.05
            self.l_forearm_angle = math.sin(time_ms * 0.015 + 3) * 1.2
            self.r_forearm_angle = math.sin(time_ms * 0.015 + 4) * 1.2
            self.l_thigh_angle = math.sin(time_ms * 0.01 + 5) * 0.6 - tumble_speed * 0.04
            self.r_thigh_angle = math.sin(time_ms * 0.01 + 6) * 0.6 + tumble_speed * 0.04
            self.l_shin_angle = math.sin(time_ms * 0.015 + 7) * 1.0
            self.r_shin_angle = math.sin(time_ms * 0.015 + 0) * 1.0

        # --- Calculate Joint Positions ---
        # ... (joint calculation unchanged) ...
    ...
    def draw(self, screen): # ... (no change) ...
        all_points = [self.head_pos, self.neck_pos, self.hip_pos, self.shoulder_pos, self.l_elbow_pos, self.r_elbow_pos, self.l_hand_pos, self.r_hand_pos, self.l_knee_pos, self.r_knee_pos, self.l_foot_pos, self.r_foot_pos, self.gun_tip_pos]
        min_x = min(p[0] for p in all_points) - self.head_radius - self.limb_width; max_x = max(p[0] for p in all_points) + self.head_radius + self.limb_width; min_y = min(p[1] for p in all_points) - self.head_radius - self.limb_width; max_y = max(p[1] for p in all_points) + self.head_radius + self.limb_width
        if self.current_powerup == "FLIGHT": min_x = min(min_x, self.shoulder_pos[0] - self.wing_upper_lobe_size[0] - 10); max_x = max(max_x, self.shoulder_pos[0] + self.wing_upper_lobe_size[0] + 10); min_y = min(min_y, self.shoulder_pos[1] - self.wing_upper_lobe_size[1] - 10); max_y = max(max_y, self.hip_pos[1] + self.wing_lower_lobe_size[1] + 10)
        surf_width = max(1, int(max_x - min_x)); surf_height = max(1, int(max_y - min_y)); temp_surf = pygame.Surface((surf_width, surf_height), pygame.SRCALPHA); offset_x = -min_x; offset_y = -min_y
        def offset_pos(pos): return (pos[0] + offset_x, pos[1] + offset_y)
        head_center_int = offset_pos(self.head_pos); cap_rect = pygame.Rect(0, 0, self.head_radius * 1.8, self.head_radius * 0.8); cap_rect.center = (head_center_int[0], head_center_int[1] - self.head_radius * 0.5); pygame.draw.ellipse(temp_surf, self.cap_color, cap_rect); brim_width = self.head_radius * 1.2; brim_height = self.head_radius * 0.4; brim_x = head_center_int[0] + (self.head_radius * 0.5) * self.facing_direction; brim_y = cap_rect.centery + cap_rect.height * 0.1; brim_rect = pygame.Rect(0, 0, brim_width, brim_height); brim_rect.center = (brim_x, brim_y); pygame.draw.rect(temp_surf, self.cap_brim_color, brim_rect); pygame.draw.circle(temp_surf, ITALY_WHITE, head_center_int, self.head_radius, 0); eye_offset_x = self.head_radius * 0.35 * self.facing_direction; eye_offset_y = -self.head_radius * 0.1; eye_radius = 3; eye_pos_x = int(head_center_int[0] + eye_offset_x); eye_y = int(head_center_int[1] + eye_offset_y); pygame.draw.circle(temp_surf, self.eye_color, (eye_pos_x, eye_y - eye_radius // 2 - 1), eye_radius); pygame.draw.circle(temp_surf, self.eye_color, (eye_pos_x, eye_y + eye_radius // 2 + 1), eye_radius); nose_tip_x = head_center_int[0] + (self.head_radius * 0.5 + self.current_nose_length) * self.facing_direction; nose_tip_y = head_center_int[1] + self.head_radius * 0.1; nose_base_x = head_center_int[0] + (self.head_radius * 0.3) * self.facing_direction; nose_base_y1 = nose_tip_y - self.current_nose_width / 2; nose_base_y2 = nose_tip_y + self.current_nose_width / 2; nose_points = [(int(nose_base_x), int(nose_base_y1)), (int(nose_tip_x), int(nose_tip_y)), (int(nose_base_x), int(nose_base_y2))]; pygame.draw.polygon(temp_surf, NOSE_COLOR, nose_points); pygame.draw.circle(temp_surf, BLACK, head_center_int, self.head_radius, 1)
        ...
        def draw_limb_segment_offset(start_pos, end_pos, length, color):
            o_start = offset_pos(start_pos); o_end = offset_pos(end_pos); center_x = (o_start[0] + o_end[0]) / 2; center_y = (o_start[1] + o_end[1]) / 2; dx = o_end[0] - o_start[0]; dy = o_end[1] - o_start[1]; draw_length = math.hypot(dx, dy);
            if draw_length < 1: draw_length = 1; angle = math.atan2(dy, dx); draw_rotated_rectangle(temp_surf, color, (center_x, center_y), draw_length, self.limb_width, angle + math.pi/2)
        draw_limb_segment_offset(self.shoulder_pos, self.l_elbow_pos, self.upper_arm_length, self.arm_colors[0]); draw_limb_segment_offset(self.l_elbow_pos, self.l_hand_pos, self.forearm_length, self.arm_colors[1]); draw_limb_segment_offset(self.shoulder_pos, self.r_elbow_pos, self.upper_arm_length, self.arm_colors[0]); draw_limb_segment_offset(self.r_elbow_pos, self.r_hand_pos, self.forearm_length, self.arm_colors[1]); draw_limb_segment_offset(self.hip_pos, self.l_knee_pos, self.thigh_length, self.leg_colors[0]); draw_limb_segment_offset(self.l_knee_pos, self.l_foot_pos, self.shin_length, self.leg_colors[1]); draw_limb_segment_offset(self.hip_pos, self.r_knee_pos, self.thigh_length, self.leg_colors[0]); draw_limb_segment_offset(self.r_knee_pos, self.r_foot_pos, self.shin_length, self.leg_colors[1])
        if self.current_powerup == "FLIGHT":
             o_shoulder_pos = offset_pos(self.shoulder_pos); o_hip_pos = offset_pos(self.hip_pos); upper_attach_x = o_shoulder_pos[0]; upper_attach_y = o_shoulder_pos[1] + 5; lower_attach_x = o_hip_pos[0]; lower_attach_y = o_hip_pos[1] - 5; upper_length = self.wing_upper_lobe_size[0]; upper_width = self.wing_upper_lobe_size[1]; lower_length = self.wing_lower_lobe_size[0]; lower_width = self.wing_lower_lobe_size[1]
             def create_wing_poly_offset(attach_point, angle, length, width):
                cos_a = math.cos(angle); sin_a = math.sin(angle); cos_w = math.cos(angle + math.pi/2); sin_w = math.sin(angle + math.pi/2); p1 = attach_point; p2 = (attach_point[0] + length * 0.6 * cos_a + width * 0.5 * cos_w, attach_point[1] + length * 0.6 * sin_a + width * 0.5 * sin_w); p3 = (attach_point[0] + length * cos_a, attach_point[1] + length * sin_a); p4 = (attach_point[0] + length * 0.6 * cos_a - width * 0.5 * cos_w, attach_point[1] + length * 0.6 * sin_a - width * 0.5 * sin_w); p5 = (attach_point[0] + length*0.2 * cos_a - width*0.2 * cos_w, attach_point[1] + length*0.2 * sin_a - width*0.2 * sin_w); return [(int(p[0]), int(p[1])) for p in [p1, p2, p3, p4, p5]]
             l_attach_upper = (upper_attach_x - 4, upper_attach_y); l_attach_lower = (lower_attach_x - 4, lower_attach_y); l_upper_poly = create_wing_poly_offset(l_attach_upper, self.l_wing_upper_angle, upper_length, upper_width); l_lower_poly = create_wing_poly_offset(l_attach_lower, self.l_wing_lower_angle, lower_length, lower_width); pygame.draw.polygon(temp_surf, self.wing_color, l_upper_poly); pygame.draw.polygon(temp_surf, self.wing_outline_color, l_upper_poly, 1); pygame.draw.polygon(temp_surf, self.wing_color, l_lower_poly); pygame.draw.polygon(temp_surf, self.wing_outline_color, l_lower_poly, 1); r_attach_upper = (upper_attach_x + 4, upper_attach_y); r_attach_lower = (lower_attach_x + 4, lower_attach_y); r_upper_poly = create_wing_poly_offset(r_attach_upper, self.r_wing_upper_angle, upper_length, upper_width); r_lower_poly = create_wing_poly_offset(r_attach_lower, self.r_wing_lower_angle, lower_length, lower_width); pygame.draw.polygon(temp_surf, self.wing_color, r_upper_poly); pygame.draw.polygon(temp_surf, self.wing_outline_color, r_upper_poly, 1); pygame.draw.polygon(temp_surf, self.wing_color, r_lower_poly); pygame.draw.polygon(temp_surf, self.wing_outline_color, r_lower_poly, 1)
        if self.current_powerup == "ROCKET_LAUNCHER":
            o_shoulder_pos = offset_pos(self.shoulder_pos); gun_attach_x = o_shoulder_pos[0] + 5 * self.facing_direction; gun_attach_y = o_shoulder_pos[1] + 10; base_angle = 0 if self.facing_direction == 1 else math.pi; gun_world_angle = base_angle + self.gun_angle_offset; gun_center_x = gun_attach_x + (GUN_SIZE[0] / 2) * math.cos(gun_world_angle); gun_center_y = gun_attach_y + (GUN_SIZE[0] / 2) * math.sin(gun_world_angle); draw_rotated_rectangle(temp_surf, GUN_COLOR, (gun_center_x, gun_center_y), GUN_SIZE[0], GUN_SIZE[1], gun_world_angle)
            world_gun_attach_x = self.shoulder_pos[0] + 5 * self.facing_direction; world_gun_attach_y = self.shoulder_pos[1] + 10; world_gun_center_x = world_gun_attach_x + (GUN_SIZE[0] / 2) * math.cos(gun_world_angle); world_gun_center_y = world_gun_attach_y + (GUN_SIZE[0] / 2) * math.sin(gun_world_angle); tip_offset = GUN_SIZE[0] / 2; self.gun_tip_pos = (world_gun_center_x + tip_offset * math.cos(gun_world_angle), world_gun_center_y + tip_offset * math.sin(gun_world_angle))
        ...
        if self.is_tumbling and self.rotation_angle != 0:
            ...
            blit_rect = rotated_surf.get_rect(center = offset_pos(self.hip_pos)) # Use hip pos offset for center
            # Convert blit_rect corner back to world coordinates
            blit_pos_world = (blit_rect.left - offset_x, blit_rect.top - offset_y)
            screen.blit(rotated_surf, blit_pos_world)
        else:
...
def reset_positions(): # ... (no change) ...
    ...
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0; player1.is_kicking = False; player1.on_other_player_head = False; player1.facing_direction = 1; player1.is_jumping = False; player1.current_powerup = None; player1.is_flying = False; player1.flight_timer = 0.0; player1.is_tumbling = False; player1.rotation_angle = 0; player1.rotation_velocity = 0
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0; player2.is_kicking = False; player2.on_other_player_head = False; player2.facing_direction = -1; player2.is_jumping = False; player2.current_powerup = None; player2.is_flying = False; player2.flight_timer = 0.0; player2.is_tumbling = False; player2.rotation_angle = 0; player2.rotation_velocity = 0
...
while running:
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_7: debug_mode = not debug_mode; print(f"Debug Mode {'ACT' if debug_mode else 'DEACT'}IVATED")
                ...
                if match_active:
                    if len(active_powerups) < MAX_ACTIVE_POWERUPS:
                         print("DEBUG: Forcing powerup spawn.")
                         new_powerup = ParachutePowerup(); new_powerup.spawn(); active_powerups.append(new_powerup)
                         powerup_spawn_timer = random.uniform(POWERUP_SPAWN_INTERVAL_MIN, POWERUP_SPAWN_INTERVAL_MAX)
                    else: print("DEBUG: Max powerups already active.")
    ...
    # --- Power-up Spawning (Check limit) ---
    if match_active and len(active_powerups) < MAX_ACTIVE_POWERUPS:
    ...
    if match_active: # ... (unchanged physics, collision updates) ...
        ...
        for p in active_powerups[:]:
            p.update(dt)
            if not p.active: active_powerups.remove(p)
        particles = [p for p in particles if p.update(dt)]
        active_rockets = [r for r in active_rockets if r.active and not r.update(dt, player_list, ball)]
        active_explosions = [e for e in active_explosions if e.update(dt)] # Use logical AND

        collected_powerups_indices = []
        for i, pup in enumerate(active_powerups):
            ...
            if collected_type_p1:
                ...
                player1.current_powerup = collected_type_p1
                collected_powerups_indices.append(i); continue
            ...
            if collected_type_p2:
                ...
                player2.current_powerup = collected_type_p2
                collected_powerups_indices.append(i)
        for i in sorted(collected_powerups_indices, reverse=True):
            if 0 <= i < len(active_powerups): active_powerups.pop(i)
        ...
        p1_hit, p1_can_headbutt, p1_body_collision_timer, p1_kick_pt = False, p1_can_headbutt, p1_body_collision_timer, None; p2_hit, p2_can_headbutt, p2_body_collision_timer, p2_kick_pt = False, p2_can_headbutt, p2_body_collision_timer, None
        ...
        score_increased_this_frame = p1_hit or p2_hit; last_kick_point = p1_kick_pt if p1_kick_pt else p2_kick_pt
        if score_increased_this_frame and last_kick_point and current_hit_count > 0 and current_hit_count % 5 == 0:
            play_sound(loaded_sounds['combo']); num_kick_particles = PARTICLE_COUNT // 2
    ...
    # --- Drawing ---
    bg_color = SKY_BLUE # ... (unchanged drawing logic) ...
    ...
    for pup in active_powerups: pup.draw(screen) # Draw all powerups
    ...
    if match_over_timer > 0 and not game_over:
        winner_name = "Nils" if match_winner == 1 else "Harry"; match_win_text = f"{winner_name} Wins the Match!"; match_win_surf = font_large.render(match_win_text, True, YELLOW); match_win_rect = match_win_surf.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 30)); next_match_surf = font_medium.render("Next Match Starting...", True, WHITE); next_match_rect = next_match_surf.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 30)); bg_rect = match_win_rect.union(next_match_rect).inflate(40, 20); bg_surf = pygame.Surface(bg_rect.size, pygame.SRCALPHA); bg_surf.fill((0, 0, 100, 200)); screen.blit(bg_surf, bg_rect.topleft); screen.blit(match_win_surf, match_win_rect); screen.blit(next_match_surf, next_match_rect)
    if match_active:
        ...
        powerup_font = font_small
        if player1.current_powerup == "FLIGHT": timer_text = f"P1 Flight: {player1.flight_timer:.1f}"; timer_surf = powerup_font.render(timer_text, True, (0,100,200)); timer_rect = timer_surf.get_rect(topleft=(10, SCREEN_HEIGHT - 30)); screen.blit(timer_surf, timer_rect)
        elif player1.current_powerup == "ROCKET_LAUNCHER": powerup_text = "P1 ROCKET"; powerup_surf = powerup_font.render(powerup_text, True, (200, 50, 50)); powerup_rect = powerup_surf.get_rect(topleft=(10, SCREEN_HEIGHT - 30)); screen.blit(powerup_surf, powerup_rect)
        if player2.current_powerup == "FLIGHT": timer_text = f"P2 Flight: {player2.flight_timer:.1f}"; timer_surf = powerup_font.render(timer_text, True, (0,100,200)); timer_rect = timer_surf.get_rect(topright=(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 30)); screen.blit(timer_surf, timer_rect)
        elif player2.current_powerup == "ROCKET_LAUNCHER": powerup_text = "P2 ROCKET"; powerup_surf = powerup_font.render(powerup_text, True, (200, 50, 50)); powerup_rect = powerup_surf.get_rect(topright=(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 30)); screen.blit(powerup_surf, powerup_rect)
