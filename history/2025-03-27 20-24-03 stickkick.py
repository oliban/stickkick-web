...
BASE_KICK_FORCE_Y = -2 # <<< NEW Default Kick Y (Lower Angle)
...
# --- Debug Mode ---
debug_mode = False
DEBUG_BG_COLOR = (220, 180, 255) # <<< NEW Light Purple
DEBUG_MATCH_POINT_LIMIT = 1 # <<< NEW Debug Limit (1 goal)

# --- Game State Constants ---
MATCH_POINT_LIMIT = 10 # Normal match limit
GAME_WIN_LIMIT = 5
...
def draw_game_scores(surface, scores_list, font):
    ...
    for score_tuple in reversed(scores_list): # Show oldest first
        p1s, p2s = score_tuple
        winner_name_str = ""
        # Simplified winner check based on who has more points (assuming one reached a limit)
        # This might not be perfect if limits change drastically mid-game history, but ok for now.
        ...
        # Handle draws if limit is reached simultaneously (unlikely with integer scores)
        elif p1s == p2s and p1s >= current_limit: winner_name_str = " (Draw)" # Or handle differently
...
# --- Draw Trophy (Moved Down, Engraved Name) ---
def draw_trophy(surface, winner_name, title_font, name_font):
    ...
    surface.blit(title_surf, title_rect); surface.blit(name_surf, name_rect) # Draw banner and engraved name
...
def load_sounds(sound_dir="sounds"): #... (as before) ...
    sounds = {}
    sound_files = {"kick": ["kick_ball1.wav", "kick_ball2.wav"], "jump": ["jump1.wav"], "land": ["land1.wav"],"wall_hit": ["wall_hit1.wav"], "player_bump": ["player_bump1.wav"], "headbutt": ["headbutt1.wav"], "body_hit": ["body_hit1.wav"], "combo": ["combo_sparkle1.wav", "combo_sparkle2.wav", "combo_sparkle3.wav", "combo_sparkle4.wav"], "ball_bounce": ["ball_bounce1.wav"],}
    ...
    for key in ["goal_p1", "goal_p2", "kick", "jump", "land", "wall_hit", "player_bump", "headbutt", "body_hit", "combo", "ball_bounce"]:
...
match_winner = None; overall_winner = None; goal_message_timer = 0; screen_flash_timer = 0
...
def start_new_match(): #... (as before) ...
    global player1_score, player2_score, match_active, match_winner, match_over_timer
...
# --- Collision Handling Function (Player-Ball) --- (Uses new BASE_KICK_FORCE_Y)
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne): #...
    global current_hit_count # No longer needs debug_mode
    ...
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 ...
                 kick_y = BASE_KICK_FORCE_Y * KICK_FORCE_LEVEL # <<< Simplified
...
while running:
    ...
    if game_over:
        for event in events:
             ...
             # if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE: start_new_game() # Optional restart
    ...
    if match_over_timer > 0:
        ...
        for event in events: # Allow quit during match over
             if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: running = False
    ...
    # --- Drawing ---
    screen.fill(SKY_BLUE if not debug_mode else DEBUG_BG_COLOR) # Use debug color if active
    pygame.draw.rect(screen, GRASS_GREEN, (0, GROUND_Y, SCREEN_WIDTH, SCREEN_HEIGHT - GROUND_Y))
