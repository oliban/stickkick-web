...
# --- Color Definitions (Moved Italy colors up) ---
...
BASE_KICK_FORCE_X = 15; BASE_KICK_FORCE_Y = -10 # Less steep kick
...
class StickMan:
    ...
    def start_kick(self, ball_x): # Renamed, removed charging
        if not self.is_kicking:
    ...
    def update(self, dt, other_player):
        ...
        if not self.on_other_player_head:
            if self.y < self.base_y or self.vy < 0 or self.is_jumping:
        ...
        dist_y_head = foot_y_check - (other_head_pos[1] - other_head_radius)
        ...
        if abs(dist_x_head) < platform_radius and dist_y_head > -5 and dist_y_head < platform_radius / 2 and self.vy >= 0 and not self.on_other_player_head:
            self.y = other_head_pos[1] - other_head_radius - (self.base_y - self.hip_pos[1]) # Adjust y based on hip_pos calculated last frame
            if self.y < other_head_pos[1] - other_head_radius: # Ensure not sinking in
        ...
        if self.on_other_player_head and not landed_on_head:
             if abs(dist_x_head) >= platform_radius or self.y < other_head_pos[1] - other_head_radius * 2 :
                  self.on_other_player_head = False; self.is_jumping = True
        ...
        # Update foot pos based on actual calculation, not just self.y
...
while running:
    ...
    # Updates
    player1.update(dt, player2); player2.update(dt, player1) # Pass other player
