...
POWERUP_SPAWN_INTERVAL_MIN = 20.0 # Min seconds between spawn attempts
POWERUP_SPAWN_INTERVAL_MAX = 45.0 # Max seconds between spawn attempts
# POWERUP_SPAWN_CHANCE = 0.6     # Chance removed, now guaranteed when timer ready
...
# ROCKET_FUSE_TIME = 3.0 # Removed fuse timer, explodes on impact
ROCKET_BLAST_RADIUS_FACTOR = 2.5 # Multiplier of ball radius
ROCKET_EXPLOSION_DURATION = 0.4 # seconds
ROCKET_EXPLOSION_FORCE = 65.0 # Slightly increased force
ROCKET_EXPLOSION_COLOR = (255, 100, 0, 180) # Semi-transparent orange
GUN_COLOR = (70, 80, 90)
GUN_SIZE = (25, 8)
GUN_ANIM_SPEED = 3.0 # Slightly slower animation
GUN_ANIM_MAGNITUDE = 0.3 # Radians - slightly larger arc
LASER_COLOR = (255, 0, 0, 150) # Semi-transparent red
LASER_LENGTH = 150
LASER_WIDTH = 1
...
class ParachutePowerup: # ... (no change from previous version) ...
    def __init__(self):
    ...
    def spawn(self):
        ...
        self.vx = random.uniform(-POWERUP_DRIFT_SPEED, POWERUP_DRIFT_SPEED); print(f"Powerup spawned: {self.powerup_type} at ({self.x:.0f}, {self.y:.0f}) with drift {self.vx:.1f}")
    def update(self, dt):
    ...
    def check_collision(self, player):
        ...
        if player_rect.colliderect(powerup_box_rect):
            collected_type = self.powerup_type; self.active = False; print(f"Powerup collected: {collected_type}")
            play_sound(loaded_sounds['combo']); return collected_type
        return None
    def draw(self, screen):
        if not self.active: return
...
class Rocket: # Removed fuse_timer, explosion on impact
    ...
    def update(self, dt, players, ball):
        ...
        # Player collision (excluding owner)
        for p in players:
            if p != self.owner and p.get_body_rect().colliderect(rocket_rect):
                exploded = True; break
        # Ball collision
        ...
        return False
    def get_rect(self):
        # Basic rect centered on x,y - rotation handled visually
...
class Explosion: # ... (no change) ...
    ...
    def update(self, dt):
        ...
        self.current_radius = self.max_radius * math.sin(progress * math.pi) # Smoother expansion/contraction
    def draw(self, screen):
...
def create_explosion(x, y, radius, players, ball): # ... (no change) ...
    ...
    play_sound(loaded_sounds['wall_hit']) # Maybe add specific explosion sound?
    for p in players:
        ...
        if dist_sq < radius**2 and dist_sq > 0:
            ...
            p.vx += push_vec_x * force_magnitude; p.vy += push_vec_y * force_magnitude * 0.8 - 5
    dist_sq = (ball.x - x)**2 + (ball.y - y)**2
    if dist_sq < radius**2 and dist_sq > 0:
        dist = math.sqrt(dist_sq); force_magnitude = ROCKET_EXPLOSION_FORCE * (1.0 - (dist / radius))
        push_vec_x = (ball.x - x) / dist; push_vec_y = (ball.y - y) / dist
        ball.apply_force(push_vec_x * force_magnitude, push_vec_y * force_magnitude - 5, hitter='explosion')
    active_explosions.append(Explosion(x, y, radius))

class StickMan: # Updated gun/laser logic
    def __init__(self, x, y, facing=1):
        ...
        self.gun_anim_timer = random.uniform(0, 2 * math.pi)
        self.gun_angle_offset = 0.0 # This is the deviation from horizontal (relative to player facing)
        self.gun_tip_pos = (0, 0) # Store gun tip position for laser
    ...
    def fire_rocket(self):
        ...
        play_sound(loaded_sounds['kick']) # TODO: Add rocket fire sound

        # Calculate the world angle based on facing direction and animated offset
        base_angle = 0 if self.facing_direction == 1 else math.pi
        laser_world_angle = base_angle + self.gun_angle_offset

        # Calculate start position from the gun tip
        start_x, start_y = self.gun_tip_pos # Use the calculated tip position

        # Calculate velocity based on the world angle
        rocket_vx = ROCKET_SPEED * math.cos(laser_world_angle)
        rocket_vy = ROCKET_SPEED * math.sin(laser_world_angle)
    ...
    def update(self, dt, other_player): # Updated gun offset calculation
        ...
        # --- Gun Animation Timer & Angle Offset ---
        if self.current_powerup == "ROCKET_LAUNCHER":
            self.gun_anim_timer += dt * GUN_ANIM_SPEED
            # Offset is relative to horizontal, positive up, negative down
            self.gun_angle_offset = math.sin(self.gun_anim_timer) * GUN_ANIM_MAGNITUDE
    ...
    # Draw Method
    def draw(self, screen): # Added laser drawing
        ...
        # --- Draw Wings (Only if FLIGHT powerup active) ---
        if self.current_powerup == "FLIGHT":
             # ... (wing drawing unchanged) ...
        ...
        # --- Draw Gun & Laser (Only if ROCKET_LAUNCHER powerup active) ---
        if self.current_powerup == "ROCKET_LAUNCHER":
            ...
            # World angle includes facing direction and animated offset
            base_angle = 0 if self.facing_direction == 1 else math.pi
            gun_world_angle = base_angle + self.gun_angle_offset

            # Calculate gun center for drawing
            gun_center_x = gun_attach_x + (GUN_SIZE[0] / 2) * math.cos(gun_world_angle)
            gun_center_y = gun_attach_y + (GUN_SIZE[0] / 2) * math.sin(gun_world_angle)
            draw_rotated_rectangle(screen, GUN_COLOR, (gun_center_x, gun_center_y), GUN_SIZE[0], GUN_SIZE[1], gun_world_angle)

            # Calculate gun tip position (used for laser start and rocket spawn)
            tip_offset = GUN_SIZE[0] / 2
            self.gun_tip_pos = (gun_center_x + tip_offset * math.cos(gun_world_angle),
                                gun_center_y + tip_offset * math.sin(gun_world_angle))

            # Calculate laser end point
            laser_end_x = self.gun_tip_pos[0] + LASER_LENGTH * math.cos(gun_world_angle)
            laser_end_y = self.gun_tip_pos[1] + LASER_LENGTH * math.sin(gun_world_angle)

            # Draw the laser using pygame.draw.aaline for antialiasing (looks better for thin lines)
            try:
                 # Use a Surface for transparency
                 laser_surf = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
                 pygame.draw.aaline(laser_surf, LASER_COLOR, self.gun_tip_pos, (laser_end_x, laser_end_y))
                 screen.blit(laser_surf, (0, 0))
                 #pygame.draw.line(screen, LASER_COLOR, self.gun_tip_pos, (laser_end_x, laser_end_y), LASER_WIDTH)
            except ValueError: # Avoid potential errors if coordinates are extreme
                 pass
...
# Powerup timer starts now, reset on successful spawn
...
def reset_positions(): # ... (no change) ...
    ...
    player1.x = SCREEN_WIDTH // 4; player1.y = GROUND_Y; player1.vx = 0; player1.vy = 0; player1.is_kicking = False; player1.on_other_player_head = False; player1.facing_direction = 1; player1.is_jumping = False; player1.current_powerup = None; player1.is_flying = False; player1.flight_timer = 0.0
    player2.x = SCREEN_WIDTH * 3 // 4; player2.y = GROUND_Y; player2.vx = 0; player2.vy = 0; player2.is_kicking = False; player2.on_other_player_head = False; player2.facing_direction = -1; player2.is_jumping = False; player2.current_powerup = None; player2.is_flying = False; player2.flight_timer = 0.0
    ...
    powerup.active = False; active_rockets = []; active_explosions = []
def start_new_match(): # Reset spawn timer
    ...
    powerup_spawn_timer = random.uniform(POWERUP_SPAWN_INTERVAL_MIN, POWERUP_SPAWN_INTERVAL_MAX) # Reset timer for new match
...
while running:
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # --- DEBUG: Force Powerup Spawn ---
            elif event.key == pygame.K_6:
                if not powerup.active and match_active:
                    print("DEBUG: Forcing powerup spawn.")
                    powerup.spawn()
                    # Reset normal timer after debug spawn
                    powerup_spawn_timer = random.uniform(POWERUP_SPAWN_INTERVAL_MIN, POWERUP_SPAWN_INTERVAL_MAX)
                else:
                    print("DEBUG: Powerup already active or match inactive, cannot force spawn.")
            # --- End Debug ---
    ...
    if not powerup.active and match_active:
        ...
        if powerup_spawn_timer <= 0:
            # Spawn is now guaranteed when timer is ready
            powerup.spawn()
    ...
    if match_active:
        ...
        # Pass player_list now
        active_rockets = [r for r in active_rockets if r.active and not r.update(dt, player_list, ball)]
        active_explosions = [e for e in active_explosions if e.update(dt) and e.active]


        # --- Power-up Collection ---
        if powerup.active: # ... (unchanged) ...
    ...
    # Draw Game Elements (order matters for overlap)
    for p in particles: p.draw(screen)
    powerup.draw(screen)
    for r in active_rockets: r.draw(screen)
    player1.draw(screen); player2.draw(screen) # Players drawn before explosions
    ball.draw(screen)
    for e in active_explosions: e.draw(screen) # Explosions drawn over players/ball
