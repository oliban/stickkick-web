...
class StickMan: # Added wing attributes and logic
    def __init__(self, x, y, facing=1):
        ...
        # --- Wing Attributes ---
        self.wing_color = (230, 230, 250); self.wing_outline_color = (150, 150, 180)
        ...
        self.wing_base_size = (15, 5); self.wing_tip_size = (20, 4)
    ...
    def start_wing_flap(self):
         if not self.wing_flapping:
            self.wing_flapping = True
            self.wing_flap_timer = self.wing_flap_duration
            # play_sound(loaded_sounds['wing_flap']) # Optional flap sound
    def update(self, dt, other_player):
        ...
        if self.wing_flapping:
            ...
            if self.wing_flap_timer <= 0:
                self.wing_flapping = False; self.wing_flap_timer = 0.0
                self.l_wing_angle = math.pi + wing_rest_angle_offset; self.r_wing_angle = -wing_rest_angle_offset
            else:
                progress = 1.0 - (self.wing_flap_timer / self.wing_flap_duration); flap_phase = math.sin(progress * math.pi)
                current_offset = wing_rest_angle_offset + (wing_flap_down_angle_offset - wing_rest_angle_offset) * flap_phase
                self.l_wing_angle = math.pi + current_offset; self.r_wing_angle = -current_offset
        else:
    ...
    def draw(self, screen):
        ...
        def draw_wing_segment(attach_pos, angle, size):
            w, h = size; center_x = attach_pos[0] + (w / 2) * math.cos(angle); center_y = attach_pos[1] + (w / 2) * math.sin(angle)
            draw_rotated_rectangle(screen, self.wing_color, (center_x, center_y), w, h, angle) # Use angle directly for rotation

        l_attach = (wing_attach_x - self.limb_width * 0.3, wing_attach_y)
        draw_wing_segment(l_attach, self.l_wing_angle, self.wing_base_size)
        base_end_x = l_attach[0] + self.wing_base_size[0] * math.cos(self.l_wing_angle); base_end_y = l_attach[1] + self.wing_base_size[0] * math.sin(self.l_wing_angle)
        draw_wing_segment((base_end_x, base_end_y), self.l_wing_angle + 0.2, self.wing_tip_size)

        r_attach = (wing_attach_x + self.limb_width * 0.3, wing_attach_y)
        draw_wing_segment(r_attach, self.r_wing_angle, self.wing_base_size)
        base_end_x = r_attach[0] + self.wing_base_size[0] * math.cos(self.r_wing_angle); base_end_y = r_attach[1] + self.wing_base_size[0] * math.sin(self.r_wing_angle)
        draw_wing_segment((base_end_x, base_end_y), self.r_wing_angle - 0.2, self.wing_tip_size)
