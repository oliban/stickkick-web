...
while running:
    ...
    for event in events:
        ...
        if event.type == SOUND_FINISHED_EVENT:
            ...
            play_next_announcement() # Triggered by sound ending

        # --- Key Down Events ---
        # ... (Keydown logic as before) ...
    ...
    if game_over:
        # ... (Game over sound queuing and drawing logic as before) ...
    ...
    if match_over_timer > 0:
        ...
        if not match_end_sound_played and not game_over:
             # ... (Sound queuing logic remains the same) ...
    ...
    # --- Process Announcement Sound Queue (Runs every frame) --- <<< RE-ADDED CHECK
    # This acts as a backup trigger if the event is somehow missed or
    # if the channel becomes free before the event is processed.
    play_next_announcement()

    # --- Updates (Only if match active) ---
    # ... (Rest of game logic: updates, goal check, drawing) ...
    if match_active:
        # ... (updates) ...
    ...
    # ... (Goal detection and win logic remains the same, including sound queuing) ...
