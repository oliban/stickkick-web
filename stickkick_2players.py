...
NOSE_COLOR = (50, 50, 50) # Dark grey for nose
DEBUG_KICK_ANGLES = False

# Physics
GRAVITY = 0.5; PLAYER_SPEED = 4; JUMP_POWER = -11
BASE_KICK_FORCE_X = 15; BASE_KICK_FORCE_Y = -10 # Kick Angle
KICK_FORCE_LEVEL = 1.5 # Fixed kick strength
...
# --- KICK ANIMATION CONSTANTS (Revised Again) ---
KICK_THIGH_WINDUP_ANGLE = -math.pi / 5.0   # Moderate windup
KICK_THIGH_FOLLOW_ANGLE = math.pi * 0.75  # *** Even MORE forward thigh swing ***
KICK_SHIN_WINDUP_ANGLE = math.pi * 0.8   # *** Tighter tuck ***
KICK_SHIN_IMPACT_ANGLE = -math.pi * 0.1  # *** Slightly past straight (hyperextension look) ***
KICK_SHIN_FOLLOW_ANGLE = math.pi * 0.6   # *** More pronounced bend after kick ***
...
# --- Stick Man Class (Ciao - Bidirectional Kick) ---
class StickMan:
    def __init__(self, x, y, facing=1):
        ...
        self.kick_timer = 0; self.kick_duration = 18 # *** Even faster kick ***
        self.walk_cycle_timer = 0.0; # Removed kick_side
        self.head_radius = 12; self.torso_length = 36; self.limb_width = 10; self.upper_arm_length = 12
        self.forearm_length = 12; self.thigh_length = 14; self.shin_length = 14;
        self.torso_colors = [P2_COLOR_MAIN, P2_COLOR_WHITE, P2_COLOR_ACCENT] # Default P2 colors
    ...
    def start_kick(self): # Doesn't need ball_x anymore
        if not self.is_kicking:
            self.is_kicking = True; self.kick_timer = 0; self.vx = 0
            # Kick leg determined by facing_direction in update
    ...
    def update(self, dt, other_player):
        ...
        if not self.on_other_player_head:
            if self.y < self.base_y or self.vy < 0 or self.is_jumping: self.vy += GRAVITY; self.y += self.vy
        landed_on_head = False
        if self.y >= self.base_y and self.vy >= 0:
             if not self.is_kicking: self.y = self.base_y; self.is_jumping = False; self.vy = 0; self.on_other_player_head = False
        other_head_pos, other_head_radius = other_player.get_head_position_radius()
        foot_y_check = self.y; dist_x_head = self.x - other_head_pos[0]; dist_y_head = foot_y_check - (other_head_pos[1] - other_head_radius)
        platform_radius = other_head_radius + HEAD_PLATFORM_RADIUS_BUFFER
        if abs(dist_x_head) < platform_radius and dist_y_head > -5 and dist_y_head < platform_radius / 2 and self.vy >= 0 and not self.on_other_player_head:
            approx_leg_height = self.thigh_length + self.shin_length # Rough height from hip to foot
            self.y = other_head_pos[1] - other_head_radius - approx_leg_height * 0.1 # Adjust landing Y slightly based on leg height estimate
            if self.y < other_head_pos[1] - other_head_radius: self.y = other_head_pos[1] - other_head_radius # Prevent sinking
            self.vy = 0; self.is_jumping = False; self.on_other_player_head = True; landed_on_head = True
        if self.on_other_player_head and not landed_on_head:
             if abs(dist_x_head) >= platform_radius or self.y < other_head_pos[1] - other_head_radius * 2 : self.on_other_player_head = False; self.is_jumping = True
        ...
        # --- VIVID KICK ANIMATION LOGIC (Using facing_direction) ---
        if self.is_kicking:
            ...
            # *** Apply angles based on FACING direction ***
            if self.facing_direction == 1: # Facing right, kick with right leg
                self.r_thigh_angle = thigh_prog_angle; self.r_shin_angle = shin_prog_angle # Kick leg
                self.l_thigh_angle = -thigh_prog_angle * 0.3; self.l_shin_angle = 0.3 # Support leg
            else: # Facing left, kick with left leg
                self.l_thigh_angle = thigh_prog_angle; self.l_shin_angle = shin_prog_angle # Kick leg
                self.r_thigh_angle = -thigh_prog_angle * 0.3; self.r_shin_angle = 0.3 # Support leg

            # Arms react (keep as before, based on thigh angle)
            self.l_upper_arm_angle = -thigh_prog_angle * 0.15 if self.facing_direction == 1 else thigh_prog_angle * 0.12 # Adjust based on facing? Maybe not needed
            self.r_upper_arm_angle = thigh_prog_angle * 0.12 if self.facing_direction == 1 else -thigh_prog_angle * 0.15
    ...
    def get_kick_impact_point(self):
        if self.is_kicking:
            ...
            if impact_start < progress < impact_end:
                # Return the foot based on facing direction, as that's the one kicking forward
    ...
    def draw(self, screen):
        ...
        nose_end_y = head_center_int[1] + 2 # Slightly below center vertically
...
current_hit_count = 0 # For particle effects
...
def handle_player_ball_collisions(player, ball, can_headbutt, body_collision_timer, is_ball_airborne):
    ...
    # 1. Kick
    local_kick_point = player.get_kick_impact_point() # Now returns foot based on facing dir
    if local_kick_point:
        ...
        if dist_sq < (ball.radius + eff_kick_rad)**2:
             ...
             if 0.25 < progress < 0.6:
                 # *** Kick force based on FACING direction ***
                 kick_x = BASE_KICK_FORCE_X * KICK_FORCE_LEVEL * player.facing_direction
                 kick_y = BASE_KICK_FORCE_Y * KICK_FORCE_LEVEL
                 # *** Volley boost adjustment ***
                 if player.vy < 0: kick_y += player.vy * 0.4 # Stronger volley boost maybe?
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_DOWN: player1.start_kick() # Pass ball.x no longer needed
            elif event.key == pygame.K_a: player2.move(-1)
            elif event.key == pygame.K_d: player2.move(1)
            elif event.key == pygame.K_w: player2.jump()
            elif event.key == pygame.K_s: player2.start_kick() # Pass ball.x no longer needed
    ...
    if ball.x + ball.radius >= GOAL_LINE_X_RIGHT and ball.y > GOAL_Y_POS: player1_score += 1; goal_message_timer = GOAL_MESSAGE_DURATION; goal_scored = True; goal_pos_x = SCREEN_WIDTH; print(f"GOAL! Player 1 Score: {player1_score}")
    elif ball.x - ball.radius <= GOAL_LINE_X_LEFT and ball.y > GOAL_Y_POS: player2_score += 1; goal_message_timer = GOAL_MESSAGE_DURATION; goal_scored = True; goal_pos_x = 0; print(f"GOAL! Player 2 Score: {player2_score}")
    if goal_scored:
    ...
    last_kick_point = p1_kick_pt if p1_kick_pt else p2_kick_pt # Prioritize P1 kick point for effect if both hit?
