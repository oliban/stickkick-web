...
def draw_game_scores(surface, scores_list, font): # Corrected Order, Added Winner
    ...
    for score_tuple in reversed(scores_list):
        ...
        # Simplified winner check - check if score reaches *either* limit
        if (p1s >= MATCH_POINT_LIMIT or p1s >= DEBUG_MATCH_POINT_LIMIT) and p1s > p2s: winner_name_str = " (Nils)"
        elif (p2s >= MATCH_POINT_LIMIT or p2s >= DEBUG_MATCH_POINT_LIMIT) and p2s > p1s: winner_name_str = " (Harry)"
...
# --- Sound Loading ---
def load_sounds(sound_dir="sounds"): #... (Added number sound handling) ...
    sounds = {}
...
game_over_sound_played = False # <<< Added flag
announcement_queue = []
...
while running:
    ...
    if game_over:
        ...
        if not game_over_sound_played:
            ...
            game_over_sound_played = True # Mark as played

        # --- Input Handling for Game Over ---
        for event in events:
             if event.type == pygame.QUIT: running = False # Allow quit
             if event.type == pygame.KEYDOWN:
                 if event.key == pygame.K_ESCAPE: running = False
    ...
    if match_over_timer > 0:
        ...
        # --- Transition (Simpler - based only on timer) ---
        if match_over_timer <= 0 and not game_over: # No need to check queue/channel here
             start_new_match()

        # Allow quit during match over
        for event in events:
             if event.type == pygame.QUIT: running = False # Add QUIT check
    ...
    if announcement_queue and not announcement_channel.get_busy():
        ...
        if next_sound:
            # print(f"Playing announcement: {next_sound}") # Optional debug
